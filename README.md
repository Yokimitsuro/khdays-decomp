...
|---|---:|---|
| Real C-decompiled matched functions | **14,348** / ~23,240 (~61.7% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,537,270** / 4,728,790 (~32.51% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. Counts byte-exact C only: `asm_stubs/` and `nonmatching/` are both excluded. |
| Inline ASM / ASM stub matched functions | **2,304** / ~23,240 (~9.9%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |
| SDK/library byte-match identifications | **339** / ~23,240 (~1.5%) | NitroSDK or library functions identified separately by byte matching |
| Named but not decompiled | **6,249** / ~23,240 (~26.9%) | Functions known to the project but not implemented as C |

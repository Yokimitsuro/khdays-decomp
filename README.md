...
|---|---:|---|
| Real C-decompiled matched functions | **3,920** / ~23,240 (~16.9% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **241,556** / 4,728,790 (~5.11% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
| Inline ASM / ASM stub matched functions | **6,133** / ~23,240 (~26.4%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |

...
|---|---:|---|
| Real C-decompiled matched functions | **11,615** / ~23,240 (~50.0% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,120,006** / 4,728,790 (~23.68% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
...
| SDK/library byte-match identifications | **545** / ~23,240 (~2.3%) | NitroSDK or library functions identified separately by byte matching |
| Named but not decompiled | **7,687** / ~23,240 (~33.1%) | Functions known to the project but not implemented as C |

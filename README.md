...
|---|---:|---|
| Real C-decompiled matched functions | **3,659** / ~23,240 (~15.7% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **225,784** / 4,728,790 (~4.77% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
| Inline ASM / ASM stub matched functions | **6,394** / ~23,240 (~27.5%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |

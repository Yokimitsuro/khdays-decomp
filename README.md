...
|---|---:|---|
| Real C-decompiled matched functions | **11,522** / ~23,240 (~49.6% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,077,422** / 4,728,790 (~22.78% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
...
| SDK/library byte-match identifications | **547** / ~23,240 (~2.4%) | NitroSDK or library functions identified separately by byte matching |
| Named but not decompiled | **7,778** / ~23,240 (~33.5%) | Functions known to the project but not implemented as C |

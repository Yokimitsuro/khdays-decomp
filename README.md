...
|---|---:|---|
| Real C-decompiled matched functions | **13,526** / ~23,240 (~58.2% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,396,328** / 4,728,790 (~29.53% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. Counts byte-exact C only: `asm_stubs/` and `nonmatching/` are both excluded. |
| Inline ASM / ASM stub matched functions | **2,527** / ~23,240 (~10.9%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |

...
|---|---:|---|
| Real C-decompiled matched functions | **6,519** / ~23,240 (~28.1% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **635,060** / 4,728,790 (~13.43% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
| Inline ASM / ASM stub matched functions | **3,544** / ~23,240 (~15.2%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |
...
| Named but not decompiled | **12,605** / ~23,240 (~54.2%) | Functions known to the project but not implemented as C |

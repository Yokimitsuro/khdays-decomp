...
|---|---:|---|
| Real C-decompiled matched functions | **11,315** / ~23,240 (~48.7% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,035,356** / 4,728,790 (~21.89% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
...
| SDK/library byte-match identifications | **548** / ~23,240 (~2.4%) | NitroSDK or library functions identified separately by byte matching |
| Named but not decompiled | **7,984** / ~23,240 (~34.4%) | Functions known to the project but not implemented as C |

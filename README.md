...
|---|---:|---|
| Real C-decompiled matched functions | **11,806** / ~23,240 (~50.8% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,161,622** / 4,728,790 (~24.56% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
| Inline ASM / ASM stub matched functions | **3,393** / ~23,240 (~14.6%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |
| SDK/library byte-match identifications | **543** / ~23,240 (~2.3%) | NitroSDK or library functions identified separately by byte matching |
| Named but not decompiled | **7,498** / ~23,240 (~32.3%) | Functions known to the project but not implemented as C |
| Total known functions | **23,240** | Function index for the EU `YKGP` target |
| Region | EU (`YKGP`) | |
| Compiler | CodeWarrior `mwccarm` 3.0 build 139 | Fully-decompiled modules: `ov001`, `ov028`, `ov290`, `ov300`, `ov302`. A few precompiled-middleware translation units (e.g. the `ov028` anti-tamper crypto core) build with an older `mwccarm` via a per-file compiler override (`config/arm9/file_compilers.json`). |

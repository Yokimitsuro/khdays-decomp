...
|---|---:|---|
| Real C-decompiled matched functions | **11,543** / ~23,240 (~49.7% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,086,506** / 4,728,790 (~22.98% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. |
...
| SDK/library byte-match identifications | **546** / ~23,240 (~2.4%) | NitroSDK or library functions identified separately by byte matching |
| Named but not decompiled | **7,758** / ~23,240 (~33.4%) | Functions known to the project but not implemented as C |

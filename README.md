...
|---|---:|---|
| Real C-decompiled matched functions | **15,731** / ~23,240 (~67.7% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **1,715,376** / 4,728,790 (~36.28% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. Counts byte-exact C only: `asm_stubs/` and `nonmatching/` are both excluded. |
| Inline ASM / ASM stub matched functions | **2,055** / ~23,240 (~8.9%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |
...
| Named but not decompiled | **5,250** / ~23,240 (~22.6%) | Functions known to the project but not implemented as C |

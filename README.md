...
|---|---:|---|
| Real C-decompiled matched functions | **16,640** / ~23,240 (~71.6% by function count) | Functions implemented in C and verified byte-exact |
| Real C-decompiled matched **bytes** | **2,006,782** / 4,728,790 (~42.44% by code bytes) | The honest metric — matches how decomp.dev reports progress. Function-count % is inflated because most decomped functions are small trampolines and setters; large semantic functions are still ASM stubs. Counts byte-exact C only: `asm_stubs/` and `nonmatching/` are both excluded. |
| Inline ASM / ASM stub matched functions | **1,757** / ~23,240 (~7.6%) | Temporary ASM-based placeholders under `src/asm_stubs/`; useful for matching, not counted as C decompilation |
...
| Named but not decompiled | **4,660** / ~23,240 (~20.1%) | Functions known to the project but not implemented as C |

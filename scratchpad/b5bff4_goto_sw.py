"""Untried IR shape: keep the jump table but make every case a bare goto to a
label, with the bodies as labelled blocks after it in the ROM's order. The
emitted code should be identical, but the compiler builds the function from
labels and gotos instead of case bodies, which is a different construction
order for the allocator.
"""
import sys
import os
import json
...
# turn every outer case into a goto to a label, bodies after, same order
head, rest = BASE.split("    switch (nClass) {\n", 1)
body, tail = rest.split("    }\n\n    s->bKind = (u8)nTo;", 1)

# split the body into the five case blocks by their top-level labels
parts = re.split(r"\n    (?=case [0-9]+:)", "\n" + body.strip("\n"))
labels = []
blocks = []
for p in parts:
    ...
    blocks.append(re.sub(r"^case [0-9]+:\s*", "", p))
print("outer case labels found:", labels)

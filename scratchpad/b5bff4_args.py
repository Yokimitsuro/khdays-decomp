"""The order in which call arguments are materialised fixes the order the
allocator sees the copies, and therefore which one coalesces first. Vary that in
case 1, whose index is one of the two mis-coalesced values.
"""
import sys
import os
import json
...
report("slot_local", BASE.replace(OLD, """        int nOffset = nColumn * 6;
        int nIndex;
        int nSlotA;
        int nSlotB;

        nSlotA = func_ov002_0205a600(nFrom + nOffset);
        func_ov002_0205afb4(nFrom + nOffset, nFrom, 1, nSlotA, 0);
        nIndex = nColumn * 6 + nTo;
        nSlotB = func_ov002_0205a600(nIndex);
        func_ov002_0205afb4(nIndex, nTo, 1, nSlotB, 1);
        s->bIndex = (u8)nIndex;
"""))

report("index_first", BASE.replace(OLD, """        int nOffset = nColumn * 6;
        int nIndex = nColumn * 6 + nTo;
...
report("store_first", BASE.replace(OLD, """        int nOffset = nColumn * 6;
        int nIndex;
...
report("offset_inline", BASE.replace(OLD, """        int nIndex;

"""Fourth sweep: nCol as an if/else-selected variable, summed inside the pDst
expression with varying associativity."""
...
SEL_IF = "    nCol = 10;\n    if (bWide == 0) {\n        nCol = 9;\n    }\n"
SEL_TERN = "    nCol = bWide ? 10 : 9;\n"

EXPRS = {
    "col+colmn+row": "nCol + nColumn + (nRow * 2 + 0xc) * 0x20",
    "colmn+col+row": "nColumn + nCol + (nRow * 2 + 0xc) * 0x20",
    "row+col+colmn": "(nRow * 2 + 0xc) * 0x20 + nCol + nColumn",
    "paren col+cn": "(nCol + nColumn) + (nRow * 2 + 0xc) * 0x20",
}

tmp = os.path.join(ROOT, "build", "try", "_sweep.c")
rows = []
for sname, sel in (("if", SEL_IF), ("tern", SEL_TERN)):
    for ename, expr in EXPRS.items():
        body = sel + "    pDst = func_ov002_02053bb8(0xb) + (%s) * 2;\n" % expr
        open(tmp, "w").write(HEAD % body)
        try:
            obj = vi.compile_c(tmp, False)
        except subprocess.CalledProcessError:
            print("%-6s %-16s COMPILE FAIL" % (sname, ename))
            continue
        ...
        rows.append((len(real), len(mine), sname, ename))
        print("%-6s %-16s size=%-4d instrs=%-3d real=%d"
              % (sname, ename, len(mine), len(ml), len(real)))
os.remove(tmp)
rows.sort()
print("\nBEST:", rows[0])

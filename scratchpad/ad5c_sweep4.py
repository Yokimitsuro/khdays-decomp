"""All 120 declaration orders of the five locals, with a separate row local."""
...
for perm in itertools.permutations(VARS):
    ...
    for sname, stmt in STMTS.items():
        ...
        rows.append((len(real), ",".join(perm), sname))
rows.sort()
for r in rows[:8]:
    print("real=%-3d %-38s %s%s" % (r[0], r[1], r[2], "  *** MATCH ***" if r[0] == 0 else ""))
os.remove(tmp)

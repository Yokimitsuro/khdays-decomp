...
def main():
    ...
    # order to keep the same winner. Written only when it changes, like
    # file_compilers.json above.
    frags = [BUILD / "file_modes.d" / (unit_of(m) + ".json") for m in MODULES]
    if any(f.exists() for f in frags):
        all_modes = {}
        for f in frags:
            if f.exists():
                all_modes.update(json.loads(f.read_text(encoding="utf-8")))
        modes_text = json.dumps(all_modes, indent=2, sort_keys=True)
        modes_path = BUILD / "file_modes.json"
        if not modes_path.exists() or modes_path.read_text(encoding="utf-8") != modes_text:
            modes_path.write_text(modes_text, encoding="utf-8", newline="\n")

...
extern WMErrCode func_ov105_020bcc20(void *wmSysBuf, u16 dmaNo);   /* WM_Init */

/* func_ov105_020bd9fc -- WMi_InitializeEx: WM_Init plus the asynchronous ARM7 INITIALIZE
 * request carrying the ARM7 buffer, the status block, the 7-to-9 fifo and the misc
 * flags (listen-only / no blink); `callback` fires when the ARM7 side is up. WM_Initialize
 * and WM_InitializeForListening are the wrappers.
 */
WMErrCode func_ov105_020bd9fc(void *wmSysBuf, WMCallbackFunc callback, u16 dmaNo, u32 miscFlags)
{
    WMErrCode result;

    result = func_ov105_020bcc20(wmSysBuf, dmaNo);
    if (result != WM_ERRCODE_SUCCESS) {
        return result;
    }

    func_ov105_020bcea0(WM_APIID_INITIALIZE, callback);

    {
        WMArm9Buf *p = func_ov105_020bd02c();

        result = func_ov105_020bcf10(WM_APIID_INITIALIZE, 4, (u32)(p->WM7), (u32)(p->status), (u32)(p->fifo7to9), miscFlags);
        if (result != WM_ERRCODE_SUCCESS) {
            return result;
        }
    }

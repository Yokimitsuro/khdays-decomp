...
/* func_ov105_020bd674 -- WM_GetMPSendBufferSize: the MP send buffer size the caller
 * must allocate, the ARM7's maximum send size rounded up to 32 bytes; 0 unless the
 * state is PARENT or CHILD and MP has not started yet.
 */
int func_ov105_020bd674(void)
{
    WMErrCode result;
    int maxSendSize;

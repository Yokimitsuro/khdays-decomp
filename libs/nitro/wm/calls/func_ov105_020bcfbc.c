...
extern s32 func_020093e8(int tag, u32 data, BOOL err);   /* PXI_SendWordByFifo */
extern void MI_CpuCopy8(const void *src, void *dst, u32 size);
extern u32 *func_ov105_020bceb8(void);            /* WmGetCommandBuffer4Arm7 */
...
/* func_ov105_020bcfbc -- WMi_SendCommandDirect: post a pre-built ARM7 WM request of
 * `length` bytes through a command buffer and the PXI fifo.
 */
WMErrCode func_ov105_020bcfbc(void *data, u32 length)
{
    int result;
    u32 *tmpAddr;

    tmpAddr = func_ov105_020bceb8();
    if (tmpAddr == NULL) {
    ...
    result = func_020093e8(PXI_FIFO_TAG_WM, (u32)tmpAddr, FALSE);

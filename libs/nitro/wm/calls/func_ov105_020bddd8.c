...
/* func_ov105_020bddd8 -- WM_StartConnectEx: connect to the parent described by
 * `pInfo` (START_CONNECT) from IDLE: the descriptor is flushed, the aid bookkeeping
 * reset, and the request carries the descriptor, the 24-byte ssid (zeroed when none),
 * the power-save flag and the authentication mode.
 */
WMErrCode func_ov105_020bddd8(WMCallbackFunc callback, const WMBssDesc *pInfo, const u8 *ssid, BOOL powerSave, const u16 authMode)
{
    WMErrCode result;
    ...
    {
        WMArm9Buf *w9b = func_ov105_020bd02c();
        w9b->myAid = 0;
        w9b->connectedAidBitmap = 0;
    }

    func_ov105_020bcea0(WM_APIID_START_CONNECT, callback);

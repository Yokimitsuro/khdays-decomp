...
/* func_ov105_020bdc00 -- WMi_StartParentEx: start beaconing as a parent (START_PARENT)
 * from the IDLE state; the connected-aid bookkeeping is reset first. `powerSave` is
 * passed through to the ARM7.
 */
WMErrCode func_ov105_020bdc00(WMCallbackFunc callback, BOOL powerSave)
{
    WMErrCode result;
    ...
    func_ov105_020bcea0(WM_APIID_START_PARENT, callback);

    result = func_ov105_020bcf10(WM_APIID_START_PARENT, 1, (u32)powerSave);
    WM_CHECK_RESULT(result);

...
/* func_ov105_020bdcac -- WM_StartScan: start a single-channel scan (START_SCAN) from
 * IDLE / CLASS1 / SCAN; the request block carries the channel, the scan buffer, the
 * channel time and the bssid filter.
 */
WMErrCode func_ov105_020bdcac(WMCallbackFunc callback, const WMScanParam *param)
{
    WMErrCode result;

    result = func_ov105_020bd0a0(3, WM_STATE_IDLE, WM_STATE_CLASS1, WM_STATE_SCAN);
    ...
    func_ov105_020bcea0(WM_APIID_START_SCAN, callback);

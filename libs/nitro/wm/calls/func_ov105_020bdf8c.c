...
/* func_ov105_020bdf8c -- WMi_StartMP: start the MP (multi-poll) protocol from PARENT /
 ...
 */
WMErrCode func_ov105_020bdf8c(WMCallbackFunc callback, u16 *recvBuf, u16 recvBufSize, u16 *sendBuf, u16 sendBufSize, WMMPTmpParam *tmpParam)
{
    WMErrCode result;
    WMArm9Buf *p = func_ov105_020bd02c();
    WMStatus *status = p->status;

    result = func_ov105_020bd0a0(2, WM_STATE_PARENT, WM_STATE_CHILD);
    ...
    if (status->mp_ignoreSizePrecheckMode == FALSE) {
        if (recvBufSize < func_ov105_020bd6e0()) {
            return WM_ERRCODE_INVALID_PARAM;
        }

        if (sendBufSize < func_ov105_020bd674()) {
            return WM_ERRCODE_INVALID_PARAM;
        }
    }

    func_ov105_020bcea0(WM_APIID_START_MP, callback);

    {
        ...
        result = func_ov105_020bcfbc(&Req, sizeof(Req));
        WM_CHECK_RESULT(result);
    }

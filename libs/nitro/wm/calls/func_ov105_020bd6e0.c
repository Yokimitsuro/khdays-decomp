...
/* func_ov105_020bd6e0 -- WM_GetMPReceiveBufferSize: the double-buffered MP receive
 * buffer size: for the parent the receive header plus one data record per entry, for
 * a child one receive buffer, each rounded up to 32 bytes and doubled.
 */
int func_ov105_020bd6e0(void)
{
    WMErrCode result;
    BOOL isParent;
    int maxReceiveSize;

...
/* func_ov105_020be304 -- WM_SetWEPKey: install the WEP mode (0..3) and, unless mode 0,
 * the 80-byte key block (flushed first) on the ARM7 (SET_WEPKEY).
 */
WMErrCode func_ov105_020be304(WMCallbackFunc callback, u16 wepmode, const u16 *wepkey)
{
    WMErrCode result;

    result = func_ov105_020bd058();
    ...
    func_ov105_020bcea0(WM_APIID_SET_WEPKEY, callback);

    result = func_ov105_020bcf10(WM_APIID_SET_WEPKEY, 2, (u32)wepmode, (u32)wepkey);
    WM_CHECK_RESULT(result);

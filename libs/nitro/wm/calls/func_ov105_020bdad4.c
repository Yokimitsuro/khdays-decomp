...
extern BOOL func_ov105_020bdbb0(const WMParentParam *param);   /* WmCheckParentParameter */

/* func_ov105_020bdad4 -- WM_SetParentParameter: hand the parent parameter block to the
 * ARM7 (SET_P_PARAM) after checking the user game info pointer, the MP frame sizes
 * (with the key-sharing overhead) and the basic ranges; the block and the game info
 * are flushed from the data cache first.
 */
WMErrCode func_ov105_020bdad4(WMCallbackFunc callback, const WMParentParam *pparaBuf)
{
    WMErrCode result;
    ...
    (void)func_ov105_020bdbb0(pparaBuf);

    func_ov105_020bcea0(WM_APIID_SET_P_PARAM, callback);
    ...
    result = func_ov105_020bcf10(WM_APIID_SET_P_PARAM, 1, (u32)pparaBuf);
    WM_CHECK_RESULT(result);

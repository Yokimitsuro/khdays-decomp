...
/* CARDi_ReadFromCache -- NitroSDK card_rom.c: serve the head of the pending ROM read
 * from the one-page cache when it holds the current page; TRUE while bytes remain. */
BOOL CARDi_ReadFromCache(CARDRomStat *p)
{

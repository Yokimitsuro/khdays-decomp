...
extern void func_0201a5b8 (NNSSndSeqPlayer * seqPlayer, int fadeFrame);

/* func_02019f08 -- NitroSystem player.c: NNS_SndPlayerStopSeqBySeqArcIdx. */
void func_02019f08 (int seqArcNo, int index, int fadeFrame)
{
    ...
    for (i = 0; i < SND_PLAYER_NUM; i++) {
        ...
        if (seqPlayer->status != NNS_SND_SEQ_PLAYER_STATUS_STOP &&
            ...
            func_0201a5b8(seqPlayer, fadeFrame);
        }
    }
}

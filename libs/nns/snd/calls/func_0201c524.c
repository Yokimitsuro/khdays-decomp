...
BOOL func_0201c524 (NNSSndHeapHandle heap)
{
    ...
    for (playerNo = 0; playerNo < NNS_SND_PLAYER_NUM; ++playerNo) {
        playerInfo = func_0201b5dc(playerNo);
        if (playerInfo == NULL) continue;

        func_02019dd0(playerNo, playerInfo->seqMax);
        func_02019df0(playerNo, playerInfo->allocChBitFlag);

        if (playerInfo->heapSize > 0 && heap != NNS_SND_HEAP_INVALID_HANDLE) {
            int i;

            for (i = 0; i < playerInfo->seqMax; i++) {
                if (!func_02019e08(playerNo, heap, playerInfo->heapSize)) {
                    return FALSE;
                }
            }
        }

...
void func_02001f10(OSThreadQueue * queue);
struct OSMutex {
    OSThreadQueue queue;
    OSThread * thread;
    s32 count;
...
extern LoadCommand * func_0201d308(NNSFndList * commandList);
extern void func_0201d398(LoadCommand * command);
extern void func_0201d6dc(LoadCommand * command);
extern LoadCommand * func_0201d308 (NNSFndList * commandList);
extern void func_0201d398 (LoadCommand * command);
extern void func_0201d6dc (LoadCommand * command);
...
/* func_0201e09c -- NitroSystem sndarc_stream.c: StrmThread. */
void func_0201e09c (void * arg)
{
    ...
    while (1) {
        ...
        func_02001f10(&thread->threadQ);

        while (1) {
            OS_LockMutex(&thread->mutex);

            command = func_0201d308(&thread->commandList);
            if (command == NULL) {
                OS_UnlockMutex(&thread->mutex);
                break;
            }

            func_0201d6dc(command);

...
BOOL func_0201a9b8(NNSSndStrm * stream, int numChannels, const u8 chNoList[]);
...
/* func_0201d1dc -- NitroSystem sndarc_stream.c: AllocChannel. */
BOOL func_0201d1dc (NNSSndStrmPlayer * player, int numChannels, const u8 chNoList[])
{

    if (player->allocChannelCount == 0) {
        if (!func_0201a9b8(&player->stream, numChannels, chNoList)) {
            return FALSE;
        }
    }

...
void func_0201abe0(NNSSndStrm * stream);
void func_0201ac50(NNSSndStrm * stream, int volume);
...
/* func_0201cc50 -- NitroSystem sndarc_stream.c: NNSi_SndArcStrmMain. */
void func_0201cc50 (void)
{
    ...
    for (playerNo = 0; playerNo < NNS_SND_STRM_PLAYER_NUM; ++playerNo) {
        ...
        func_0201e1a0(&player->fader);

        volume
            = SND_CalcDecibel(func_0201e16c(&player->fader) >> 8)
              + SND_CalcDecibel(player->initVolume)
              + SND_CalcDecibel(player->extVolume)
            ;
        if (volume != player->volume) {
            func_0201ac50(&player->stream, volume);

            player->volume = volume;
        }

        if (player->fadeOutFlag) {
            if (func_0201e1b8(&player->fader)) {
                func_0201d0e0(player);
            }
        }
    }

...
FSFileID func_0201b7c0(void);
u32 func_0201b6a4(u32 fileId);
...
/* func_0201df84 -- NitroSystem sndarc_stream.c: OpenFileStream. */
BOOL func_0201df84 (NNSSndStrmPlayer * player, u32 fileId)
{
    if (func_0201b6f4(fileId, &player->info, sizeof(player->info), 0) != sizeof(player->info)) {
        return FALSE;
    }

    if (!FS_OpenFileFast(&player->file, func_0201b7c0())) {
        return FALSE;
    }
    player->fileOffset = func_0201b6a4(fileId);

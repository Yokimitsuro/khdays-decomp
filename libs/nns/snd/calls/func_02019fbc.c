...
void func_0201a604(NNSSndSeqPlayer * seqPlayer, BOOL flag);
...
extern void func_0201a604 (NNSSndSeqPlayer * seqPlayer, BOOL flag);

/* func_02019fbc -- NitroSystem player.c: NNS_SndPlayerPauseByPlayerNo. */
void func_02019fbc (int playerNo, BOOL flag)
{
    NNSSndSeqPlayer * seqPlayer;
    NNSSndSeqPlayer * next;


    for (seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&data_0204a760[ playerNo ].playerList, NULL);
         seqPlayer != NULL; seqPlayer = next) {
        next = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&data_0204a760[ playerNo ].playerList, seqPlayer);

        func_0201a604(seqPlayer, flag);
    }
}

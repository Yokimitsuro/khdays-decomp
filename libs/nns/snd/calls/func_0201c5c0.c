...
extern BOOL func_0201c734(NNSSndHandle * handle, int playerNo, int bankNo, int playerPrio, const NNSSndArcSeqInfo * info, int seqNo);
extern BOOL func_0201c734 (NNSSndHandle * handle, int playerNo, int bankNo, int playerPrio, const NNSSndArcSeqInfo * info, int seqNo);

/* func_0201c5c0 -- NitroSystem sndarc_player.c: NNS_SndArcPlayerStartSeq. */
BOOL func_0201c5c0 (NNSSndHandle * handle, int seqNo)
{
    const NNSSndArcSeqInfo * info;


    info = func_0201b3e8(seqNo);
    if (info == NULL) return FALSE;

    return func_0201c734(
        handle,
        info->param.playerNo,
        info->param.bankNo,

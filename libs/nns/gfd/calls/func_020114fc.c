...
extern BOOL func_02011378 (const NNSGfdVramTransferTaskQueue * pQueue);

/* func_020114fc -- NitroSystem gfd_VramTransferManager.c: NNSi_GfdPushVramTransferTaskQueue. */
BOOL func_020114fc (NNSGfdVramTransferTaskQueue * pQueue)
{

    if ( !func_02011378(pQueue)) {
        pQueue->idxRear = func_02011358(pQueue, pQueue->idxRear);
        pQueue->numTasks++;
        return TRUE;
    } else {
        return FALSE;

...
/* func_02013814 -- NitroSystem g2d_Font.c: NNSi_G2dFontGetTextWidth. */
int func_02013814 (const NNSG2dFont * pFont, int hSpace, const void * txt)
{
    int width = 0;


    while (txt != NULL) {
        const int line_width = func_02013768(pFont, hSpace, txt, &txt);
        if (line_width > width) {
            width = line_width;
        }
    }

 ...
 * bitfield [6-7] of that node's flags word (+0xc) to `val`. */
void func_02016ae8(int obj, unsigned int idx, unsigned int val, int slot) {
    int target;
    ...
    *(unsigned int *)(target + 0xc) = *(unsigned int *)(target + 0xc) & ~0xc0U | (val << 6);
}

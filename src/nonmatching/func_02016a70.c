 ...
 * bitfield [4-5] of that node's flags word (+0xc) to `val`. */
void func_02016a70(int obj, unsigned int idx, unsigned int val, int slot) {
    int target;
    ...
    *(unsigned int *)(target + 0xc) = *(unsigned int *)(target + 0xc) & ~0x30U | (val << 4);
}

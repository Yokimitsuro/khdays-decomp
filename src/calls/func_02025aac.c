#pragma thumb on
/* func_02025aac -- vsnprintf into a char buffer, MAIN (THUMB). The NitroSDK's STD_TVSNPrintf with
 * one extra conversion: %f prints a 20.12 fixed-point value (int, or s64 with l/ll) with 0..8
 * fraction digits (default 6) through func_020258fc, right-aligned in the width. '+' always sets
...
extern void func_02025800(dst_string *p, char c);                   /* string_put_char */
extern void func_02025830(dst_string *p, char c, int n);            /* string_fill_char */
extern void func_02025894(dst_string *p, const char *s, int n);     /* string_put_string */
extern void func_020258fc(s64 value, int precision, char *pIntText, char *pFracText);
extern int func_0200ff54(const char *str);                          /* STD_GetStringLength */

#define string_put_char func_02025800
#define string_fill_char func_02025830
#define string_put_string func_02025894

int func_02025aac(char *dst, size_t len, const char *fmt, va_list vlist)
{
    char buf[24];
    char frac[14];
    int n_buf;
...
put_invalid:
            ...
            case 'f':
            {
                ...
                func_020258fc(val, precision, buf, frac);
                if (n_prefix + (func_0200ff54(buf) + func_0200ff54(frac)) + 1 < width)
                    string_fill_char(&str, ' ', width - (n_prefix + (func_0200ff54(buf) + func_0200ff54(frac)) + 1));
                if (n_prefix > 0)
                    string_put_char(&str, prefix[0]);
                string_put_string(&str, buf, func_0200ff54(buf));
                if (frac[0] != 0) {
                    string_put_char(&str, '.');
                    string_put_string(&str, frac, func_0200ff54(frac));
                }
                ++s;
            }
            break;

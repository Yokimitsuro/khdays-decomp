...
extern void *func_01ffdb54(u16 *root, CollCastState *state);
extern void *func_01ffd4a4(u16 *root, CollCastState *state);

int func_01ffed08(CollCastState *state, CollisionModel *model)
{
    int hit = 0;
    s32 size;
    int i;
    ...
    if (state->bound34 - model->region84.centerX00 <= size &&
        ...
        if ((state->direction1c.x | state->direction1c.z) == 0) {
            face = func_01ffdb54(model->root9c, state);
        } else {
            face = func_01ffd4a4(model->root9c, state);
        }
        if (face != 0) {
            hit = 1;

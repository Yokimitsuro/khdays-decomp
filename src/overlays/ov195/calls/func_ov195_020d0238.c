/*
 * Hit handler of the ov194 enemy (x3: ov194/195/196), the func_ov181_020cc44c shape. A hit is
 ...
 * +0x24; a hit that charged flips the state's facing bit (+0x52 bit 0) and fires reaction 0x134
...
int func_ov195_020d0238(char *actor, int nParam, struct ActorHitEvent *hit)
{
    struct Ov194ActionState *state = *(struct Ov194ActionState **)(actor + 0x214);
    int delta;
    int rem;
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x134, (state->bFacing & 1) ? 2 : 3, state->pPos);
            } else {
                state->bFacing = state->bFacing + 1;
                func_ov107_020c5af8(actor, 0x134, (state->bFacing & 1) ? 0 : 1, state->pPos);
            }
        }
    }
    return 1;

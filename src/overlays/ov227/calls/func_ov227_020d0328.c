/* func_ov227_020d0328 -- effect command hook of the ov227 enemy (the ov226 d030c hook with the slot
 * table at +0x43c and slots 1/6 swapping kinds).  A type-5 command carries three big-endian
 ...
 * 0203ca30).  Byte 3 picks the effect slot (+0x43c + 8 * slot: resource,
 * handle): slots 0 / 5 / 6 spawn kind 0x15 / 5 / 0x15 at the transform (020c0794) and slot 1 kind 5 on the actor,
...
typedef struct EffectActor {
    ...
    u8  srt[0x43c - 0xa0];    /* 0x0a0 */
    EffectSlot aSlot[11];     /* 0x43c */
} EffectActor;
...
void func_ov227_020d0328(EffectActor *pActor, u8 *pCommand, int nArg)
{
    ...
    if (pCommand[2] == 5) {
        ...
        case 0:
            pActor->aSlot[pCommand[3]].hEffect =
                func_ov107_020c0794(pActor->hList, pActor->aSlot[pCommand[3]].nResource, KIND_EFFECT_B, 0, &transform);
            break;
        case 1:
            pActor->aSlot[pCommand[3]].hEffect =

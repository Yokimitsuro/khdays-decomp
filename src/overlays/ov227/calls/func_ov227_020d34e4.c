/* Leap tick of the ov227 enemy. With a +0x7c lock, losing the target (ov227 14bc below zero)
 * ends the action. The +0x5c timer accumulates the owner's rate and, once it is not negative,
 * reaction 0x14d mode 8 fires at the +8 point (the +0x75 byte marks it). Until the timer
...
extern void func_ov227_020d0a10(int *state, int mode, int b);
extern short data_0203d210[];

#define ANG2IDX(a) ((unsigned short)(((long long)(a) * 0x28be60db9391LL + 0x80000000000LL) >> 44) >> 4)
#define FX_MUL(a, b) ((int)(((long long)(a) * (b) + 0x800) >> 12))

void func_ov227_020d34e4(int *node)
{
    ...
    if (state[0x1f] != 0 && func_ov227_020d14bc(node, 0) < 0) {
        func_0203c634(node, *(signed char *)((char *)node + 0x20), 0);
        return;
    }
    state[0x17] += *(int *)(*node + 0x2c);
    if (*(u8 *)((char *)state + 0x75) == 0 && state[0x17] >= 0) {
        func_ov107_020c5af8(*state, 0x14d, 8, (void *)state[2]);

/* Hover tick of the ov227 enemy (the ov221-ov226 hover with a longer strike). Airborne outside
 * sub-state 6 the +0x50 heading turns towards +0x58 at 0x96 (0x28 without a +0x78 target) per 100 of
 * rate (0203d040), and the owner's +0xa0 pose takes the heading about data_02042264. Sub-states 2 and
 * 6 drain the +0x68 timer. The +0x14 step is copied out and, in sub-states 2/4/5/6 with a target, its
 * y becomes half the gap from the +8 point to the target's +0x194 plus 2.0 (less whatever the owner's
 * +0x13c exceeds 9.0 by, never below the target itself), clamped to [-0x100, 0x60]; in sub-state 9 a
 * cast of the owner's +0x80 radius along the step from its +0x74 clips the step to the first blocking
 * hit. The step lands in the owner's +0xf0 and the +0x14 velocity decays by 0x300 per 0x88 of rate. A
 * +0x60 bit-0 owner then runs the jump handler (020d11d0) and, once it has launched (+0x418 bit 7),
...
extern void func_ov227_020d11d0(int self, int *node);
extern void func_ov227_020d0a10(int *state, int mode, struct Ov221SweepParams *params);
...
void func_ov227_020d1968(int *node)
{
    ...
    if (*(signed char *)(*state + 0x1c6) == 2 || *(signed char *)(*state + 0x1c6) == 6) {
    ...
    *(Vec3 *)(*state + 0xf0) = step;
    ...
    func_ov227_020d11d0(*state, node);
    if ((*(u8 *)(*state + 0x418) & 0x80) == 0) {
        return;
    }
    *(int *)(*state + 0x434) += *(int *)(*node + 0x2c);
    if (*(int *)(*state + 0x434) <= 0x2fd0) {
        ...
        params.nReach = 0x300 + (t * 0x1dccd / 0x3d18 + 0x33);
        params.v0c = data_02042270;
        params.v18 = data_02042258;
        params.v24 = data_02042264;
        params.bFlag = 1;
        func_ov227_020d0a10(state, 6, &params);
        if (*(int *)(*state + 0x434) < 0x1800) {
            return;
        }
        if ((*(u8 *)(*state + 0x418) & 0x40) != 0) {
            return;
        }
        func_ov107_020c0b90(*state, 9, data_02041dc8, 2);
        *(u8 *)(*state + 0x418) |= 0x40;
        return;
    }
    func_ov107_020c0b90(*state, 9, data_02041dc8, 0xff);
    *(u8 *)(*state + 0x418) &= ~0x80;
}

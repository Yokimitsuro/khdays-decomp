 ...
 * 0x12a mode 6 fires at the +8 point, the +0x75 flag and +0x5c timer clear and the tick hands
 * over to func_ov222_020d4750. */
...
extern void func_ov222_020d4750(int *node);

void func_ov222_020d46a8(int *node)
{
    int *state = (int *)node[1];

    func_ov107_020c9264(*state, state[0x1e] != 0 ? 6 : 7, 0);
    func_ov107_020c0b90(*state, 1, data_02041dc8, (unsigned char)(state[0x1e] == 0));
    func_ov107_020c5af8(*state, 0x12a, 6, (void *)state[2]);
    *(unsigned char *)((char *)state + 0x75) = 0;
    state[0x17] = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov222_020d4750);
}

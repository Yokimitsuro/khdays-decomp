 ...
 * zero-vector mode-0xa message with flag 3 and hands over to func_ov222_020d506c, otherwise
 * sub-state 6 ends the state. While busy, a step shorter than 0x80 does nothing; otherwise
 * sub-action 2 (target) or 4 runs (func_ov222_020d284c). */
...
extern void func_ov222_020d506c(int *node);

void func_ov222_020d4e84(int *node)
{
    ...
    *(Vec3 *)(state + 5) = *(Vec3 *)(state + 8);
    ...
    if (*(unsigned char *)(state[1] + 0xad) == 0) {
        if (state[0x1e] != 0) {
            if (((struct Bit0 *)(*state + 0x17a))->bit0 != 0) {
                ...
                func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov222_020d506c);
                return;
            }
    ...
    func_ov222_020d284c(state, state[0x1e] != 0 ? 2 : 4, 0);
}

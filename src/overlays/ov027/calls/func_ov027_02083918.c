/* func_ov027_02083918 -- Ov027_BlinkPanelSlot: run one blinking slot of the game-over panel.  Each
 * slot has a timer and a phase (+0x570 of the scene work, 8 bytes per slot); every 16 frames the
...
void func_ov027_02083918(int nSlot)
{
    data_ov027_02084364->aBlink[nSlot].nTimer++;
    if (data_ov027_02084364->aBlink[nSlot].nTimer >= 16) {
        if (data_ov027_02084364->aBlink[nSlot].nPhase == 1) {
            data_ov027_02084364->aBlink[nSlot].nPhase = 2;
        } else {
            data_ov027_02084364->aBlink[nSlot].nPhase = 1;
        }
        data_ov027_02084364->aBlink[nSlot].nTimer = 0;
        func_ov027_02083c78(&data_ov027_02084364->panel, nSlot, data_ov027_02084364->aBlink[nSlot].nPhase);
    }

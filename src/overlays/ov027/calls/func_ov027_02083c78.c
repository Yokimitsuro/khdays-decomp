/* func_ov027_02083c78 -- Ov027_DrawPanelSlotShifted: draw one slot of the game-over panel with
...
extern void  func_ov027_02083c40(Ov027Panel *pPanel, int nSlot);   /* Ov027_DrawPanelSlot */
/* khdays: shared-bss */
int data_ov027_02084360 = 0;                                        /* the fade-out frame counter */
Ov027Scene *data_ov027_02084364 = 0;                                /* the scene work */

void func_ov027_02083c78(Ov027Panel *pPanel, int nSlot, int nShift)
{
    if (data_ov027_02084364->nSlots == 3 && nSlot == 1 && data_ov027_02084364->bPrompt == 0) {
        return;
    }
    pPanel->aSlot[nSlot].nSrcY += nShift * 2;
    func_ov027_02083c40(pPanel, nSlot);
    pPanel->aSlot[nSlot].nSrcY -= nShift * 2;
}

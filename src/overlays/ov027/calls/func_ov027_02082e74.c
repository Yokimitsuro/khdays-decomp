/* func_ov027_02082e74 -- Ov027_GameOverMenu: the game-over state that shows the two-line panel.
 * Each frame the models animate (Ov027_UpdateModels 02083308), the input is polled
 * (Ov027_PollMenu 02083740) and the hint drawn (Ov027_DrawHintText 02083d50).  With the mode
 * word (+0x24 of the scene work) at 1 only the character object (+0x588; 0208344c) and the
 * camera (+0x4d8; 02023cc0) are updated and the state stays.  With bit 1 of the mode word or a
 * client session (bit 1 of data_0204c240) the main display's BG mode goes to 1, bit 4 of the
 * mode word is dropped and Ov027_GameOverIdle (02083068) follows.  Otherwise the panel gets
...
extern void  func_ov027_02083740(void);                             /* Ov027_PollMenu */
extern void  func_ov027_02083d50(void);                             /* Ov027_DrawHintText */
extern void  func_ov027_02083b94(void);                             /* Ov027_BuildPanel */
...
extern void *func_ov027_02082fb4(void);                             /* Ov027_GameOverChoice */
/* khdays: shared-bss */
...
void *func_ov027_02082e74(void)
{
    ...
    func_ov027_02083d50();
    if (pScene->nMode != 1) {
        if ((data_ov027_02084364->nMode & 2) || (data_0204c240 & 2)) {
        ...
        for (i = 0; i < pScene->nSlots; i++) {
            func_ov027_02083c78(&pScene->panel, i, pScene->aBlink[i].nPhase);
        }
        func_ov027_02083cb8(&pScene->panel);
        func_ov027_0208344c(&pScene->character);
        func_02023cc0(&pScene->camera);
        return func_ov027_02082fb4;
    }
    func_ov027_0208344c(&pScene->character);
    func_02023cc0(&pScene->camera);
    return 0;

...
extern int  data_ov009_02055f6c[];
extern int  data_ov009_020563e4[];

void func_ov009_0204db88(void) {
    int i;
    for (i = 0; i < 7; i++) {
        char *base = (char *)data_ov009_020563e4[1];
        if (*(unsigned char *)(base + 0x963c) & (1 << i)) {
            GFXi_EnqueueCommand(data_ov009_02055f6c[i], 0,
                                ((int *)(base + 0x95a4))[i],
                                *(int *)(base + 0x95d0) << 6);
        }
    }
    *(unsigned char *)((char *)data_ov009_020563e4[1] + 0x963c) = 0;
}

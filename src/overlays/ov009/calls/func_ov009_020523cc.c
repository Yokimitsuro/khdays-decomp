 ...
 * func_ov009_020523a0(ctx, element, arg). base/count reloaded per-iter (callees
 * may mutate *ctx). */
extern void func_ov009_020526d0(int ctx, int arg);
extern void func_ov009_020523a0(int ctx, int element, int arg);
...
void func_ov009_020523cc(int ctx_, int arg) {
    struct Ctx3 *ctx = (struct Ctx3 *)ctx_;
    int i;
    func_ov009_020526d0(ctx_, arg);
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].field_14 != 0) {
            func_ov009_020523a0(ctx_, (int)&ctx->items[i], arg);
        }
    }
}

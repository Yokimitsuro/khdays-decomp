/* Clear the cell buffers for slots 0..2 and mark them dirty, so the next
 * Ov008_FlushDirtyCells re-uploads them. Twin of func_ov008_0204ef90,
...
void func_ov009_0204cc74(void) {
    int i;
    for (i = 0; i <= 2; i++) {
        char *base = (char *)data_ov009_020563e4[1];
        MIi_CpuClear16(0, ((void **)(base + 0x95a4))[i], *(int *)(base + 0x95d0) << 6);
        *(unsigned char *)((char *)data_ov009_020563e4[1] + 0x963c) |= 1 << i;
    }
}

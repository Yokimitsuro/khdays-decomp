...
typedef struct Ov101RadialBurstFrame {
    MtxFx33 mRotation;
    VecFx32 vSpawnPosition;
    VecFx32 vRadialOffset;
    VecFx32 vOrigin;
} Ov101RadialBurstFrame;

typedef struct Ov101HeadingSource {
    u8 pad00[0x80];
    u16 nHeading;
} Ov101HeadingSource;

typedef struct Ov101EffectPart {
    u8 pad00[2];
    s8 nState;
    u8 pad03;
    int nLifetime;
...
typedef struct Ov101SceneFlags {
    u8 bSlotSpawnEnabled : 1;
    u8 rest : 7;
} Ov101SceneFlags;

typedef struct Ov101SceneState {
    u32 nFlags;
    u8 pad04[0x1c];
    Ov101HeadingSource *pHeadingSource;
    u8 pad24[0x456];
    u8 nPendingMode;
    u8 nPendingIndex;
    u8 pad47c[0x218];
    Ov101SceneFlags slotSpawnFlags;
    u8 pad695[0x25eb];
    u8 aSlotPool[4];
} Ov101SceneState;

extern Ov101SceneState *data_ov101_020bc0e0;
...
extern void func_ov101_020bb95c(int slotPool, VecFx32 *position);
extern int func_02030788(void);
extern void func_02033d0c(int slotId, int variant, VecFx32 *position,
                          int flags);

int func_ov101_020bb1d8(void *pContext, Ov101EffectPart *pPart, int nDelta)
{
    const short *pSinCosTable;
    MtxFx33 *pRotation;
    Ov101RadialBurstFrame f;
    int nSpawnVariant;
    int nAngle;
    Ov101SceneState *pScene;

    pScene = data_ov101_020bc0e0;
    pPart->nState = 4;
    ...
    if (pPart->nPhase == 2) {
        ...
        do {
            ...
            func_ov101_020bb95c((int)pScene->aSlotPool, &f.vSpawnPosition);

/* func_ov101_020bb9fc -- enemy state tick, head of a 4-member family (ARM).
 * State 1 accumulates the frame delta at +0x10c and, past 0xa000, notifies func_ov101_020bbba8
 ...
 * r0 at that point (nothing has written r0 since entry) because func_ov101_020bbba8 takes it as
 ...
 */
extern void func_ov101_020bbba8(int *node);
extern int func_02030788(void);
extern void func_ov101_020bbaa4(int *node);
extern int func_0202a818(int a, int b);

int func_ov101_020bb9fc(int *node, int dt) {
    int r = 0;
    switch (node[0]) {
    default:
        break;
    case 1:
        node[0x43] += dt;
        if (node[0x43] >= 0xa000) {
            func_ov101_020bbba8(node);
            node[0] = 2;
        }
        break;
    case 2:
        if (func_02030788() == 0) {
            func_ov101_020bbaa4(node);
        }

...
struct Ov082MotionState {
    char bytes[0x170];
};

struct Ov082Controller {
    char pad000[0x0c];
    int pending0c;
    char pad010[4];
    int animState14;
    ...
    int playerHandle230;
    struct Ov082MotionState motions234[8];
...
extern int func_ov022_020ad718(struct Ov082Actor *actor);
extern void func_ov022_020a4490(struct Ov082Actor *actor, int cue, int mode);
extern void func_ov082_020b8a44(struct Ov082Controller *self);
extern int func_ov082_020b8bd4(struct Ov082Controller *self,
                               struct Ov082MotionState *motion, int delta);
...
extern void func_ov082_020b9290(struct Ov082Controller *self, int delta);
extern void func_ov082_020b91a0(struct Ov082Controller *self);

void func_ov082_020ba160(struct Ov082Controller *self, int delta)
{
    struct Ov082Actor *actor = self->actorDb4;
    int active = 0;
    int i;
    struct Ov082MotionState *motion;
...
afterSpawn:
    ...
    i = 0;
    do {
        if (func_ov082_020b8bd4(self, motion, delta) != 0) {
            active = 1;
        }
        i++;
        motion++;
...
afterAnimation:
    ...
    func_ov082_020b9290(self, delta);
    func_ov082_020b91a0(self);
}

/* func_ov164_020d1fd4: ported from the matched ov202 sibling (same shape: throw-release tick with the 0xc00 guard, a fixed 0x3000 radius and the +0x60 bit-7 set at the end). */
/* func_ov164_020d1fd4: ported from a matched sibling family (same shape, constants and offsets adjusted). */
...
extern int func_ov164_020d01dc(void *world, struct Sphere *shape, void **out);
...
extern const struct Msg data_ov164_020d2c90;
extern const struct Msg data_ov164_020d2cac;
                                     ...
                                     FxVec *vPush, FxVec *vImpact, struct Msg *msgA)
{
    ...
    i = 0;
    if (nHits > 0) {
        tmplA = data_ov164_020d2cac;
        do {
            id = *(u16 *)((char *)aVictims[i] + 2);
            if ((st->uMask5c >> id & 1) == 0) {
                ...
                if (func_ov107_020ca918(aVictims[i], st->pActor, st->pActor, 1,
                    ...
                    *msgA = tmplA;
                    ...
                    id = *(u16 *)((char *)aVictims[i] + 2);
                    st->uMask5c = st->uMask5c | (unsigned long long)1 << id;
                }
            }
            i++;
        } while (i < nHits);
...
void func_ov164_020d1fd4(struct Node *node)
{
    struct State *st;
    void *world;
    void *aVictims[4];
    ...
    if (st->nTimer30 < 0xc00) {
        shape.centre = *st->pPos50;
        shape.radius = 0x3000;
        ...
        nHits = func_ov164_020d01dc(*(void **)((char *)world + 0x7c), &shape, aChildren);
        i = 0;
        if (nHits > 0) {
            do {

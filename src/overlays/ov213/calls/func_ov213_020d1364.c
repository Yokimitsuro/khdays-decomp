/* Sweep-pass hit test (called by the sweep pass ticks 020d0d8c / 020d0f14): the segment of the
...
void func_ov213_020d1364(int *state)
{
    int obj;
    fx64 dist;
    Fx32 scratchZ;
    ...
    for (i = 0; i < n; i++) {
        ...
        *((u8 *)state + 0x84) |= 1 << *(u16 *)(hits[i] + 2);
        func_ov107_020c5af8(*(int *)(state[0] + 0x384), 0x122, 7, (void *)state[1]);
    }
}

/*
 * Hit handler of the ov213 actor, the func_ov191_020d0408 shape. A hit is refused once the
...
int func_ov213_020cd010(char *actor, int nParam, struct ActorHitEvent *hit)
{
    ...
    if (hit->nDealt10 > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 2) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x122, (state->bFacing & 1) ? 2 : 3, state->pPos);
            } else {
                state->bFacing = state->bFacing + 1;
                func_ov107_020c5af8(actor, 0x122, (state->bFacing & 1) ? 0 : 1, state->pPos);
            }
        }
    }
    return 1;

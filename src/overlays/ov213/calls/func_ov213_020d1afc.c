/* func_ov213_020d1afc -- broadcast this node's position. When the caller's flag word has both
 * bit 0 and bit 4 set, build the 14-byte message from the template at data_ov213_020d2f52, pack
 * the actor's +0x74 position vector into bytes 5..13 (three components, each as sign + 23-bit magnitude
 * split high/mid/low), hand it to the object's message hook with length 0xe, clear the pending
 * byte at +0x1c7 and report success. Variant of the ov153 family reading the actor's own position.
 *
...
extern struct Msg data_ov213_020d2f52;

int func_ov213_020d1afc(int self, int a, unsigned int *flags, int d) {

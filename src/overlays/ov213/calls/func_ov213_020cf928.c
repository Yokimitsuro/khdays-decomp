...
extern void func_ov213_020cfbf4(int *node);

void func_ov213_020cf928(int *node)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        *((u8 *)state + 0x69) |= 1 << *(u16 *)(hits[i] + 2);
        func_ov107_020c5af8(*state, 0x122, 7, (void *)(hits[i] + 0x74));
    }
    state[7] += state[0x17];
    if (state[7] <= 0x10000) {
        return;
    ...
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov213_020cfbf4);
}

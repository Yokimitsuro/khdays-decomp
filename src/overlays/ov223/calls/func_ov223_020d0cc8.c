/* func_ov223_020d0cc8 -- the move CHOOSER for ov221's family: pick the next move and re-arm the
 ...
 *            is non-null AND its +0x388 is non-zero) -- i.e. nothing is already in flight
 ...
 *   target, far  : <0x42 && below move 0xb | move 9
...
int func_ov223_020d0cc8(int self, int dist) {
    int *ctx;
    int roll;
    int charged;
    int clear;

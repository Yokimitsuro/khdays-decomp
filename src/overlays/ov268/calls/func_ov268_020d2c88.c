 ...
 * +0x7c grid (func_ov268_020d088c): each struck object (func_ov002_02076dac, power 0xff) gets the
 ...
 * and the tick hands over to func_ov268_020d3264. */
...
extern int func_ov268_020d088c(int grid, Sphere *sphere, int *list);
...
extern void func_ov268_020d3264(int *node);
...
void func_ov268_020d2c88(int *node)
{
    ...
    *(Vec3 *)(state + 5) = *(Vec3 *)(state + 0x15);
    ...
    for (i = 0; i < n; i++) {
        ...
        func_ov107_020c5af8(*state, 0x15f, 9, &pt);
        *((u8 *)state + 0x4e) |= 1 << *(u8 *)(hits[i] + 0x1b4);
    }
    n = func_ov268_020d088c(*(int *)(grid + 0x7c), &sphere, list);
    for (i = 0; i < n; i++) {
        StrikeInfo info = {0};
        Vec3 dir;
        Vec3 pt2;
    ...
    if ((*(u8 *)(*state + 0x1c4) & 0xa) == 0) {
        ...
        func_ov107_020c5af8(*state, 0x15f, 0xc, state + 8);
    }
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov268_020d3264);
}

/* Shockwave tick of the ov268 enemy (x3 with ov208/ov209). While the +0x2c timer (fed by the
 ...
 * reaction 0x15f mode 9 fires there and the kind bit is set. Once the +0x50 idle byte clears
...
void func_ov268_020d3264(int *node)
{
            ...
            do {
                if (((*((u8 *)state + 0x4e) >> *(u8 *)(hits[i] + 0x1b4)) & 1) == 0) {
                    ...
                    if (func_ov107_020ca918(hits[i], *state, *state, 1, &push, 0) != 0) {
                        ...
                        func_ov107_020c5af8(*state, 0x15f, 9, &at);

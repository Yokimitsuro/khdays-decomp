/* Constructor of the ov212 tail sub-actor: installs the handlers (+0xc draw veneer, +0x30
 * callback, +0x1d0 hit), flags 0x4e in the +0x60 high byte and bits 0x14 of +0x1ae, sets the
 * +0x70 latch to 0x1467 with the +0x64/+0x68 scale cleared and +0x6c at -0x1467/2, clears
 * +0x54/+0x58, then creates the +0x388 pool seeded with the camera key and flags it. Codegen:
 * the -latch/2 is computed from the just-stored +0x70 (forwarded, so the ROM negates the 0x1467
...
extern void func_ov212_020d1bfc(void);
extern void func_ov212_020d1cf4(void);
extern void func_ov212_020d1c90(void);
extern PoolEntry *func_01fffca8(void *pool, int elementSize, int capacity);
extern int func_ov107_020c319c(void *camera);

void func_ov212_020d1b24(char *self)
{
    u16 hw;
    int nHalf;

    *(Callback *)(self + 0xc) = func_ov212_020d1bfc;
    *(Callback *)(self + 0x30) = func_ov212_020d1cf4;
    *(Callback *)(self + 0x1d0) = func_ov212_020d1c90;
    hw = *(u16 *)(self + 0x60);
    *(u16 *)(self + 0x60) = (hw & ~0xff00) |
        ((((((u32)hw << 0x10) >> 0x18) | 0x4e) << 0x18) >> 0x10);
    *(u16 *)(self + 0x100 + 0xae) |= 0x14;
    *(int *)(self + 0x70) = 0x1467;

/* Grab-phase machine of the ov212 enemy on its +0x6c phase: 0 and 1 consume a pending request
 * (+0x70, -1 when none) and advance to phase 2 / 4 when it asks so; 2 plays animation 1, fires
 * reaction 0x128 mode 6 at the +8 point and enters 3; 3 waits for the +4 item (+0xaf), then sets
...
void func_ov212_020cd66c(int *state)
{
    switch (state[0x1b]) {
    case 0:
        if (state[0x1c] == -1) {
    ...
    case 2:
        ...
        func_ov107_020c5af8(*state, 0x128, 6, (void *)state[2]);
        state[0x1b] = 3;
        return;
    case 3:
    ...
    case 4:
        ...
        func_ov107_020c5af8(*state, 0x128, 7, (void *)state[2]);
        ((struct w8 *)(*(int *)(*state + 0x4cc) + 8))->lo &= ~2;
        *(unsigned short *)(*state + 0x100 + 0xae) &= ~0x10;
        state[0x1b] = 5;
        return;

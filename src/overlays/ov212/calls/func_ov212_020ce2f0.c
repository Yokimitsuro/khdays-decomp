/* func_ov212_020ce2f0 -- if a dodge or a counter is queued and the opening still holds, fire the
 ...
 *
 * The gate is: a target at ctx[0x11], func_ov212_020cd63c(ctx, 0) still true, and a queued move at
...
int func_ov212_020ce2f0(int self) {
    int *ctx;
    int owner;
    int q;

    ctx = *(int **)(self + 4);
    if (ctx[0x11] != 0 && func_ov212_020cd63c(ctx, 0) != 0) {
        owner = ctx[0];

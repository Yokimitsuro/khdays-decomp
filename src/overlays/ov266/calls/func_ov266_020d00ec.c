/* func_ov266_020d00ec -- if a dodge or a counter is queued and the opening still holds, fire the
 ...
 *
 * The gate is: a target at ctx[0x11], func_ov266_020cf42c(ctx, 0) still true, and a queued move at
...
int func_ov266_020d00ec(int self) {
    int *ctx;
    int owner;
    int q;

    ctx = *(int **)(self + 4);
    if (ctx[0x11] != 0 && func_ov266_020cf42c(ctx, 0) != 0) {
        owner = ctx[0];

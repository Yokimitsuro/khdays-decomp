/* func_ov106_020b825c -- Ov106_CmdStartLightFade: script command that starts a fade of one of
 ...
 * PROVENANCE: byte-identical twin of ov023's Ov023_CmdStartLightFade (func_ov023_02086fac), same code and
...
extern int  func_ov106_020b8a5c(int nLight);                        /* Ov023_GetLightLevel */
...
int func_ov106_020b825c(Ov023ScriptCtx *pCtx, u8 *pOperand)
{
    int nLight;

    nLight = func_02021980(pCtx, pOperand + 0x10);
    pCtx->pEvent->nFadeLevel = func_ov106_020b8a5c(nLight);
    ...
    if (pCtx->pEvent->nFadeFrames == 0) {
        func_ov106_020b8a4c(pCtx->pEvent->nFadeTo, nLight);
        return 1;
    }
    return 0;
}

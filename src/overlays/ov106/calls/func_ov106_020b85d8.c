/* func_ov106_020b85d8 -- Ov106_SetupNoiseDraw: set the G3D globals up for the noise overlay.
 ...
 * PROVENANCE: byte-identical twin of ov023's Ov023_SetupNoiseDraw (func_ov023_02089880), same code and
...
void func_ov106_020b85d8(void)
{
    ...
    nHeight = data_ov106_020b8b68->nHeight << 12;
    nWidth = data_ov106_020b8b68->nWidth << 12;
    vScale.z = 0x1000;
    vScale.x = (int)(((long long)nWidth * 0x1000 + 0x800) >> 12);
    vScale.y = (int)(((long long)nHeight * 0x1000 + 0x800) >> 12);
    func_020158b0(&vScale);
    nScroll = data_ov106_020b8b68->nScroll >> 4;

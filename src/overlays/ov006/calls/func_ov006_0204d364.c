 ...
 * +0x24 bit 1 is set, notify func_ov006_0204d334(ctx, element). base/count are
 * reloaded each iteration because the callee may mutate *ctx. */
extern void func_ov006_0204d334(int ctx, int element);
...
void func_ov006_0204d364(int ctx_) {
    struct Ctx *ctx = (struct Ctx *)ctx_;
    int i;
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].b1 == 1) {
            func_ov006_0204d334(ctx_, (int)&ctx->items[i]);
        }
    }
}

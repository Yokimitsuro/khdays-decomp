...
void func_ov235_020cfc2c(int *node)
{
    int obj;
    ...
    if (*((u8 *)state + 0x65) == 0 && state[0x15] >= 0x800) {
        func_ov107_020c5af8(*state, (short)*(int *)(*state + 0x3c8), 8, (void *)(*(int *)(*state + 0x3b4) + 0x14));
        *((u8 *)state + 0x65) = 1;
    }
    ...
    if (state[0x11] > 0x222 && state[0x11] < 0x911) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (func_ov107_020ca918(hits[i], *state, *state, 4, &push, 0) == 0) {
                continue;
            }

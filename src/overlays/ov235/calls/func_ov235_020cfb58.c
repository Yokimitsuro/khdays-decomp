 ...
 * follows the frame rate and the +0x5c path point is resolved (func_ov235_020cccc0) into the +0x10
 * step. Once the +0xc idle byte clears, animation 0x14 plays, the +0x3a8 part plays motion 0x10,
 * +0x63, the +0x44 timer and +0x65 clear and the tick hands over to func_ov235_020cfc2c. */
...
extern void func_ov235_020cfc2c(int *node);

void func_ov235_020cfb58(int *node)
{
    ...
    func_ov107_020c9ee8(*(int *)(*state + 0x3a8), 0x10, 0);
    *((unsigned char *)state + 0x63) = 0;
    state[0x11] = 0;
    *((unsigned char *)state + 0x65) = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov235_020cfc2c);
}

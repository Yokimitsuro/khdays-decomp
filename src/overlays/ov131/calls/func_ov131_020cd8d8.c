...
extern void func_ov131_020cd9a4(void);

void func_ov131_020cd8d8(char *obj) {
    int *state = *(int **)(obj + 4);
    short pair[2];
    int vec[3];
    int scale;
    ...
    if (*(unsigned char *)state[0x12] == 0) {
        short *pp = pair;
        void (*cb)();
        pp[1] = data_ov131_020cef6c[1];
        pp[0] = data_ov131_020cef6c[0];
        cb = *(void (**)())(*state + 0x24);
        if (cb != 0) cb(*state, pp, 4);
        func_ov107_020c9264(*state, 10, 1);
        state[0x15] = 0;
        state[0xf] = 0x300;
        func_0203c634(obj, *(signed char *)(obj + 0x20), func_ov131_020cd9a4);
    }
}

/* func_ov161_020cce40: hit handler of the ov161 enemy (x2), ported from the matched ov131 sibling; the health always drops here and a kind-4 hit keeps the stored impact point. */
...
int func_ov161_020cce40(char *actor, int source, struct HitMsg *msg)
{
    struct HitState *hs;
    int state;
    int left;
    ...
    if (msg->nDamage28 > 0) {
        if ((msg->uFlags & 8) == 0 || (msg->uFlags & 0x80) == 0 ||
            ...
            if ((msg->uFlags & 0x22) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x152,

...
extern void func_ov165_020d3448(void);

void func_ov165_020d337c(char *obj) {
    int *state = *(int **)(obj + 4);
    short pair[2];
    int vec[3];
    int scale;
    ...
    if (*(unsigned char *)state[0x16] == 0) {
        short *pp = pair;
        void (*cb)();
        pp[1] = data_ov165_020d4aa8[1];
        pp[0] = data_ov165_020d4aa8[0];
        cb = *(void (**)())(*state + 0x24);
        if (cb != 0) cb(*state, pp, 4);
        func_ov107_020c9264(*state, 10, 1);
        state[0x19] = 0;
        state[0x13] = 0x300;
        func_0203c634(obj, *(signed char *)(obj + 0x20), func_ov165_020d3448);
    }
}

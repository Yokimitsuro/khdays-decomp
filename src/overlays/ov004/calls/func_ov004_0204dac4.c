...
extern void func_ov004_0204d640(void *ctx, void *node, unsigned int sx, unsigned int sy, int flag);
extern void func_ov004_0204d91c(DrawParams *p, void *block);

void func_ov004_0204dac4(void *ctx, DrawParams *p, char *block, int flag) {
    int i;
    int sx = p->sx;
    int sy = p->sy;
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (i == 0) {
            func_ov004_0204d640(ctx, *(void **)(row + 0x34),

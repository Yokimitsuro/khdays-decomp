 ...
 * +0x1c anchor follows it (func_ov261_020d0348) and the +0x34 lift eases by a fiftieth towards
 * the carrier's height plus 0x800 above the +4 position; otherwise it eases towards 0x1800 above
 * the actor's +0x13c height. */
extern void func_ov261_020d0348(void *anchor, int *carrier, int *pos);

void func_ov261_020d0b28(int *node)
{
    int *state = (int *)node[1];
    int d;

    if (state[0xb] != 0) {
        func_ov261_020d0348(state + 7, (int *)state[0xb], (int *)state[1]);

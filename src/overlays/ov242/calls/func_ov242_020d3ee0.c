 ...
 * the 0xf74 cosine of the heading, and that func_ov242_020d3dfc accepts with its position and
...
extern int func_ov242_020d3dfc(int *state, Vec3 pos, int radius);
extern const short data_0203d210[];
extern const Vec3 data_02042258;

int func_ov242_020d3ee0(int node)
{
            ...
            if (dy <= 0x1e000) {
                ...
                if (dist < bestDist) {
                    ...
                    if (dist <= 0x1800 || (dist <= 0xf000 && VEC_DotProduct(&d, &fwd) >= 0xf74)) {
                        if (func_ov242_020d3dfc(state, pos, *(int *)(actor + 0x80)) != 0) {
                            bestDist = dist;
                            best = actor;
                        }
                    }

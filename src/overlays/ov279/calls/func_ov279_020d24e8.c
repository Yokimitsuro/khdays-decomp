 ...
 * is committed to +0xc, the +0x50 timer restarts and the tick hands over to func_ov279_020d25e8. */
...
extern void func_ov279_020d25e8(int *node);

void func_ov279_020d24e8(int *node)
{
    int *state = (int *)node[1];
    int mtx[9];

    func_ov107_020c0b90(*state, 0, *(Vec3 *)state[0x13], 0);
    func_ov107_020c5af8(*state, 0x167, 5, (void *)state[0x13]);
    ...
    *(Quat *)(state + 3) = *(Quat *)(state + 7);
    state[0x14] = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov279_020d25e8);
}

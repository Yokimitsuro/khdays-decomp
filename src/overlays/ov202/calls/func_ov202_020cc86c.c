/* func_ov202_020cc86c: hit handler of the ov202 enemy (x2: ov202/203), variant of the matched ov131 sibling with the health always dropping and the state one word shorter. */
...
int func_ov202_020cc86c(char *actor, int source, struct HitMsg *msg)
{
    struct HitState *hs;
    int state;
    int left;
    ...
    if (msg->nDamage28 > 0) {
        if ((msg->uFlags & 8) == 0 || (msg->uFlags & 0x80) == 0 ||
            ...
            if ((msg->uFlags & 0x22) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x156,

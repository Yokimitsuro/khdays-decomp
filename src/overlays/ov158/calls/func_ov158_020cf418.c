/* Send the ov158 enemy's position message: the overlay's 14-byte template (data_ov158_020cf564)
...
extern const PosMsg data_ov158_020cf564;
...
void func_ov158_020cf418(int *state)
{
    PosMsg msg;
    FxVec vContact;
    FxVec *pPos;

    msg = data_ov158_020cf564;
    pPos = (FxVec *)state[1];
    vContact.x = pPos->x;
    ...
    if (*(int *)(*state + 0x13c) - 0xe00 > 0x100) {
        msg.flag = 1;
    }

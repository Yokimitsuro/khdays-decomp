/*
 * Hit handler of the ov158 enemy, the func_ov120_020cc568 shape. A hit is refused once the
 ...
 * 0x150 at the state's +0x4c anchor, modes 2/3 for flag-0x22 hits and 0/1 otherwise, except
...
int func_ov158_020cc790(char *actor, int nParam, struct ActorHitEvent *hit)
{
    struct Ov158ActionState *state = *(struct Ov158ActionState **)(actor + 0x214);
    int delta;
    int rem;
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x150, (state->bFacing & 1) ? 2 : 3, state->pAnchor);
            } else {
                state->bFacing = state->bFacing + 1;
                func_ov107_020c5af8(actor, 0x150, (state->bFacing & 1) ? 0 : 1, state->pAnchor);
            }
        }
    }
    return 1;

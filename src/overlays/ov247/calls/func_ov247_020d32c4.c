/* Attack wait tick of the ov246 enemy: the +0x3c timer accumulates the owner's rate. In phase
 * 0 (+0x54) past 0x22aa (both conditions in one test, which keeps the phase byte live) the data_ov247_020d4f00[4..5] message pair is sent to the +0x24 hook
 * (arg 4), reaction 0x158 mode 6 fires at the +0x398 bone's +0x14 and phase 1 begins; in phase 1
...
void func_ov247_020d32c4(int *node)
{
    ...
    if (*(unsigned char *)((char *)state + 0x54) == 0 && state[0xf] > 0x22aa) {
        pp = pairA;
        pp[1] = data_ov247_020d4f00[5];
        pp[0] = data_ov247_020d4f00[4];
        cb = *(void (**)())(*state + 0x24);
        ...
        *(unsigned char *)((char *)state + 0x54) = 1;
        ...
        pp = pairB;
        pp[1] = data_ov247_020d4f00[3];
        pp[0] = data_ov247_020d4f00[2];
        cb = *(void (**)())(*state + 0x24);
        if (cb != 0) cb(*state, pp, 4);

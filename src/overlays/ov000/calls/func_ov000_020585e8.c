/* func_ov000_020585e8 -- build the menu grid: make sure the three resources are loaded, then walk
 * 3 pages of 40 cells, allocate a node for every populated cell and append it to the list.
 *
 * Each of the three resources (the one func_ov000_020569dc(0) names, plus 0x15 and 0x1b) is
...
extern void func_ov000_0205821c(int a, short *out1, short *out2);
extern void NNS_FndAppendListObject(void *list, void *obj);

void func_ov000_020585e8(int self, void **entries, void *list, unsigned short *ids) {
    ...
    for (page = 0; page < 3; page++, ids += 0x28, entries += 0x28) {
        for (i = 0, entry = entries; i < 0x28; entry++, i++) {
            ...
            *(int *)(node + 0x1c) = -1;
            ...
            func_ov000_0205821c(*(int *)(node + 0x24), (short *)(node + 0x14),
                                (short *)(node + 0x16));

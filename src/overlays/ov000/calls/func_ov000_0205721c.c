/* func_ov000_0205721c -- walk the 24 recipe entries, price each one this owner can actually make,
 ...
 * where a slot with a negative id is empty and skipped, and func_ov000_020571d0 vetoes the entry
...
extern int func_ov000_020571d0(int id, int *owner);
extern void func_ov000_02056b14(int self, Recipe *entry, int units);
extern void func_ov000_02056bac(int self, int a, int units, int b, int c);
extern void func_ov000_02056cb4(int self, int a, int units, int b, int c);

int func_ov000_0205721c(int self, int *owner) {
    int ret;
    Recipe *entry;
    NNSFndList list;
    char buf[0xa0];
            ...
            *(unsigned short *)(buf + 2) = *(unsigned short *)((char *)node + 2);
            ...
            for (j = 0, slot = (char *)owner; j < 5; j++, slot += 0xdc) {
                if (entry->slots[j].id >= 0) {
                    if (func_ov000_020571d0(entry->slots[j].id, owner) == 0) {
                        ok = 0;
                        break;
                    }
            ...
            if (ok != 0) {
                ...
                switch (entry->kind) {
                case 0:
                    func_ov000_02056b14(self, entry, units >> 12);
                    break;
                case 1:
                    func_ov000_02056bac(self, entry->param, units >> 12, 1, 1);
                    break;
                case 2:
                    func_ov000_02056cb4(self, entry->param, units >> 12, 0, 1);
                    break;
                }
                ret = entry->result;
            }

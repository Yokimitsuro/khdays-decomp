 ...
 * func_ov000_020565ec(ctx, element, arg). base/count reloaded per-iter (callees
 * may mutate *ctx). */
extern void func_ov000_020568e0(int ctx, int arg);
extern void func_ov000_020565ec(int ctx, int element, int arg);
...
void func_ov000_02056618(int ctx_, int arg) {
    struct Ctx3 *ctx = (struct Ctx3 *)ctx_;
    int i;
    func_ov000_020568e0(ctx_, arg);
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].field_14 != 0) {
            func_ov000_020565ec(ctx_, (int)&ctx->items[i], arg);
        }
    }
}

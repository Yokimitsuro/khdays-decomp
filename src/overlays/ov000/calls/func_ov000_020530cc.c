/* Ov000_BlitOrClearMainTileRegion -- clipped tile-region blit or clear, main layer.
 *
 ...
 * copies the rows out of the source blob or clears them, and flags the layer dirty.
 * Twin of func_ov000_0205474c against the other layer object; this one returns void
 * and its dirty byte sits at offset 0x4afa rather than 0x10.
 *
...
extern Ov000DirtyFlags *data_ov000_0205ac24;

extern u16 *func_ov000_02053088(int handle, int *pLayer);
extern void MIi_CpuCopy16(const void *src, void *dst, u32 size);
extern void MIi_CpuClear16(u16 value, void *dst, u32 size);

void func_ov000_020530cc(int nRequest, int bCopy)
{
    u16 *blob;
    int i;
    u32 sx;
    ...
    u32 dy;
    {
        ...
        dst = func_ov000_02053088(req->nHandle, &layer);
        ...
        data_ov000_0205ac24->bDirtyMask |= (1 << layer);
    }
}

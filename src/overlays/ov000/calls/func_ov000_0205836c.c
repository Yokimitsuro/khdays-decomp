/* Rebuild the entry view and recount its filled cells (x7: ov000/004/005/008/009/025 + ov069). Load the three message
...
extern void func_ov000_020580fc(int *self);
extern int func_ov000_020569dc(int a);
...
extern void func_ov000_02057850(int *self, int a, void *node, int idx);
extern void func_ov000_02057dc4(int *self, int base, int a, int b, unsigned long long *m);

struct Pair { int a, b; };
struct Self { char pad[0xcc]; struct Pair pairs[3]; };
extern void func_ov000_02056a68(void *p);

void func_ov000_0205836c(int *self, int base, void *list, int p4) {
    ...
    func_ov000_020580fc(self);
    tc = func_ov000_020569dc(0);
    self[0x3d] = 0;
    self[0x3c] = 0;
    self[0x3a] = 0;
    ...
    for (node = NNS_FndGetNextListObject(list, 0); node != 0;
        ...
        func_ov000_02057850(self, *(unsigned short *)((char *)node + 0xa) * 0xa0 + base, node,
    ...
    for (k = 0; k < 3; k++, base += 0xa0) {
        func_ov000_02057dc4(self, base, ((struct Self *)self)->pairs[k].a,
    ...
    func_ov000_02056a68((char *)self + 0x2c);
    if (self[0x3a] != 0) {
        func_02034258(0x13);
        self[0x3a] = 0;
    }

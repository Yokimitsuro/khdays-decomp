/* Animation tick of an ov238 model: a frozen model (+0x5c bit 1) only runs the base tick while it is
 ...
 * finds its track in the first of the owner's two banks (+0x394, 0x24 each) that has it, advances
...
void func_ov238_020cfc08(char *self, int rate)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (*(u8 *)(self + i + 0xad) != 0 && clip >= 0) {
            Track *track = 0;
            AnimBank *bank = (AnimBank *)(owner + 0x394);
            int step;
            int end;

            for (k = 0; k < 2 && track == 0; bank++, k++) {

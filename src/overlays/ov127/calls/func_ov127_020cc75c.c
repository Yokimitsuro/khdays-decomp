...
extern const u8 data_ov127_020cd744[];

int func_ov127_020cc75c(char *actor, int nSource, struct HitMsg *msg)
{
    struct HitState *hs;
    int nLeft;
    int nNext;
    ...
    if (msg->nDamage28 > 0) {
      ...
      if ((hf->lo & 8) == 0 || (hf->lo & 0x80) == 0 || hf->hi != 0x80) {
        if ((hf->lo & 0x22) != 0) {
            ...
            uSlot.sPair = *(const struct ImpactPair *)&data_ov127_020cd744[0];
            hs->bToggle40++;
            ...
            uSlot.sPair = *(const struct ImpactPair *)&data_ov127_020cd744[2];
            hs->bToggle40++;
            func_ov107_020c5af8(actor, 0x118, uSlot.aModes[hs->bToggle40 & 1],
                                hs->pAt10);
        }

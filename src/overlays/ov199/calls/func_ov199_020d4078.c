...
extern const struct ReactionModes data_ov199_020d6114;
...
int func_ov199_020d4078(int owner, int source, struct HitDescriptor *hit)
{
    ...
    modes.normal[0] = data_ov199_020d6114.normal[0];
    modes.normal[1] = data_ov199_020d6114.normal[1];
    modes.flagged[0] = data_ov199_020d6114.flagged[0];
    modes.flagged[1] = data_ov199_020d6114.flagged[1];
    ...
    *(short *)(owner + 0x21a) = (short)remaining;
    ...
    if (hit->damage > 0) {
        ...
        if ((flags->low & 8) == 0 || (flags->low & 0x80) == 0 ||
            flags->kind != 0x80) {
            if ((flags->low & 0x22) != 0) {
                func_ov107_020c5af8(owner, 0x130,

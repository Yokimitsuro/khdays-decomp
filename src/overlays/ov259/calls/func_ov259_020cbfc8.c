/* Animation tick of an ov259 model: a frozen model (+0x5c bit 1) only runs the base tick while it is
...
void func_ov259_020cbfc8(char *self, int rate)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (*(u8 *)(self + i + 0xad) != 0 && clip >= 0) {
            Track *track = 0;
            AnimBank *bank = (AnimBank *)(owner + 0x398);
            int step;
            int end;

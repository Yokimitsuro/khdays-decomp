/* Kick anim 1, notify 020cd524, reset the motion fields, set +0x94=0x19, then dispatch. */
extern int func_ov107_020c9264(int, int, int);
extern int func_0203c634(int, int, void *);
extern int func_ov259_020cd524(int, int, int);
extern int func_ov259_020ceb9c(int);
void func_ov259_020ceb38(int param_1) {
    int owner = *(int *)(param_1 + 4);
    func_ov107_020c9264(*(int *)owner, 1, 0);
    func_ov259_020cd524(param_1, 1, 1);
    *(int *)(owner + 0x68) = 0;
    *(signed char *)(owner + 0xac) = 0;
    *(int *)(owner + 0x98) = 0;
    *(int *)(owner + 0xa8) = 0;
    *(int *)(owner + 0x6c) = 0;
    *(int *)(owner + 0x94) = 0x19;
    func_0203c634(param_1, *(signed char *)(param_1 + 0x20), (void *)&func_ov259_020ceb9c);
}

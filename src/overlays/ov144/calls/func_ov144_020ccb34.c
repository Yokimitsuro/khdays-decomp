...
extern int func_ov144_020cc7fc(int *state, int flag);
...
extern int func_ov144_020cc8e4(int *state, Vecx32 target, int margin);
...
int func_ov144_020ccb34(int *node)
{
    ...
    range = func_ov144_020cc7fc(state, 1);
    if (range < 0) {
        return 0;
    }
    ...
    if (piece != 0) {
        ...
        if (((*(int *)(piece + 0x40) << 30) >> 31) != 0 && (((struct hw60 *)(piece + 0x60))->lo & 1) != 0) {
            ...
            if (dist <= 0x2800 || (range > 0 && dist <= range && VEC_DotProduct(&dir, &facing) >= 0xf74)) {
                if (func_ov144_020cc8e4(state, at, *(int *)(piece + 0x80)) != 0) {

/* Landing step of the mission enemy (same shape as the ov034 x4 family): on the local player
 * both 64-bit flag words at +0x464 and +0x46c get bit 16, the shared sampler's motion is folded
 * into the position at +0x498 on the ground plane (raising bit 46 and clearing the vertical
 * speed unless bit 2 of +0x24 says the enemy is grounded), and the actor's own hook decides
 * bit 1 of +0x694: becoming active raises bit 49, shows the node and, for the local player,
 * sets bit 1 of +0x464. Once the +0x694 bit is set the velocities at +0x498 and +0x698 are
 * cleared, bit 2 of the actor flags is raised and the enemy hands over to state 0 when
...
void *func_ov043_020b43dc(char *self)
{
    int r;
    Vec3 sample;
    Vec3 step;

/* func_ov008_02060b58 -- Ov008_PlaceTrackedNode: place item record pRecord on
...
extern u8    func_ov008_02060944(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode); /* Ov008_AssignTrackedNodeId */
extern int   func_ov008_020604e4(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode, u32 nPage, int nLeft, int nTop, int nArg); /* place the node */
extern void  func_ov008_020609e4(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode); /* unlink a tracked node */
extern void  func_ov008_02061e64(Ov008MenuContext *pCtx, u16 nPage, u16 nX, u16 nY); /* Ov008_ClearGridSlot */
extern void  NNSi_FndFreeFromDefaultHeap(void *pBlock);

int func_ov008_02060b58(Ov008MenuContext *pCtx, Ov008Message15Record *pRecord, u32 nPage, int nCol, int nRow)
{
    int nResult;
    ...
    if (pRecord->nShapeIndex >= 0) {
        ...
        if (func_ov008_02060944(pCtx, pNode) != 0) {
            nLeft = nCol - pRecord->nAnchorColOff;
            nTop = nRow - pRecord->nAnchorRowOff;
            if (func_ov008_020604e4(pCtx, pNode, pCtx->nVisiblePage, nLeft, nTop, 0) != 0) {
                pNode->nPage = nPage;
                pNode->nLeft = nLeft;
                pNode->nTop = nTop;
                    ...
                    for (j = 0; j < GRID_COLS; j++) {
                        if ((pNode->pEntry->nCellMask & (1ULL << (j + i * 8))) != 0) {
                            if (pNode->nAnchorCol != nLeft + j || pNode->nAnchorRow != nTop + i) {
                                func_ov008_02061e64(pCtx, (u16)nPage, (u16)(nLeft + j), (u16)(nTop + i));
                            }
                        }
                    }
                }

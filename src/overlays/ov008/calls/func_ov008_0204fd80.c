...
extern Ov009MenuContext *data_ov008_02090f04[];

#define OV009_CONTEXT (data_ov008_02090f04[1])

extern u16 *func_ov008_0204fd04(int destinationCode, int *slotIndex);
extern int  func_ov008_0204fd48(int position, int stride,
                                int width, int height);

void func_ov008_0204fd80(Ov009TileBlitRequest *request, int copySource)
{
    int slotIndex;
    ...
    destination =
        func_ov008_0204fd04(request->destinationCode, &slotIndex);
    ...
    if (copySource != 0) {
        ...
        for (copyRow = 0; copyRow < (int)clip.copyHeight; copyRow++) {
            for (column = 0; column < (int)copyWidth; column++) {
                int destinationIndex = func_ov008_0204fd48(
                    destinationX + column, destinationY,
                    OV009_CONTEXT->tileGridWidth, 32);
                int sourceIndex = func_ov008_0204fd48(

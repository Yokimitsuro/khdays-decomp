/* func_ov008_02080e74 -- commit a queued Mission Mode sub-state transition, ov006.
 * When a transition is pending (obj+0x956c != 0) and not blocked (bit2 of obj+0x955c set),
 * optionally fires the transition SFX (func_ov008_02080f14(nextState, 600, sfxA, sfxB)) if
...
extern void func_ov008_02080f14(unsigned int state, int a, int sfxA, int sfxB, int e);

#define O ((char *)data_ov008_02090fa4 + 0x9000)

void func_ov008_02080e74(void) {
    if (*(int *)(O + 0x56c) == 0) {
        return;
    }
    ...
    if (*(int *)(O + 0x560) != 0) {
        func_ov008_02080f14(*(unsigned int *)(O + 0x4f8), 600,

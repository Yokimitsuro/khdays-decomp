/* func_ov008_02052360 -- walk the 24 recipe entries, price each one this owner can actually make,
 ...
 * where a slot with a negative id is empty and skipped, and func_ov008_02052314 vetoes the entry
...
extern int func_ov008_02052314(int id, int *owner);
extern void func_ov008_02051c58(int self, Recipe *entry, int units);
extern void func_ov008_02051cf0(int self, int a, int units, int b, int c);
extern void func_ov008_02051df8(int self, int a, int units, int b, int c);

int func_ov008_02052360(int self, int *owner) {
    int ret;
    Recipe *entry;
    NNSFndList list;
    char buf[0xa0];
            ...
            *(unsigned short *)(buf + 2) = *(unsigned short *)((char *)node + 2);
            ...
            for (j = 0, slot = (char *)owner; j < 5; j++, slot += 0xdc) {
                if (entry->slots[j].id >= 0) {
                    if (func_ov008_02052314(entry->slots[j].id, owner) == 0) {
                        ok = 0;
                        break;
                    }
            ...
            if (ok != 0) {
                ...
                switch (entry->kind) {
                case 0:
                    func_ov008_02051c58(self, entry, units >> 12);
                    break;
                case 1:
                    func_ov008_02051cf0(self, entry->param, units >> 12, 1, 1);
                    break;
                case 2:
                    func_ov008_02051df8(self, entry->param, units >> 12, 0, 1);
                    break;
                }
                ret = entry->result;
            }

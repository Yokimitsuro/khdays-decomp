...
extern void func_ov008_0205652c(Ov008MissionListState *list,
                                Ov008MissionResourceDescriptor *descriptor);
extern Ov008MissionListEntry *func_ov008_020565bc(Ov008MissionListState *list,
                                                  Ov008MissionListEntry *entry);
extern void func_ov008_0205658c(Ov008MissionListState *list);
extern int func_020235d0(int field, int selector);
extern int func_02020400(int numerator, int denominator);

u16 func_ov008_0205a778(void *menuState)
{
    ...
    func_ov008_0205652c(&missionList, &resourceDescriptor);
    for (missionEntry = func_ov008_020565bc(&missionList, 0);
         missionEntry != 0;
         missionEntry = func_ov008_020565bc(&missionList, missionEntry)) {
        if ((missionEntry->statusFlags & 3) != 0) {
            qualifyingCount++;
            progressTotal += func_020235d0(missionEntry->missionId * 3 + 0x2a4c, 3);
        }
    }
    func_ov008_0205658c(&missionList);

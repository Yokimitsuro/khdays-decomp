...
extern struct MsgDbHost *data_ov008_02090f04[2];

int func_ov008_02051094(unsigned int id, int p2, int p3, int arg) {
    int result = 1;
    if (data_ov008_02090f04[1]->aRefCount[id] == 0) {
        result = func_02034150(id, 0xe, 1, arg);
    }
    if (result != 0) {
        data_ov008_02090f04[1]->aRefCount[id] += 1;
    }
    return result;
}

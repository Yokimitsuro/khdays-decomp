/* func_ov008_02060d38 -- Ov008_PlaceNodeOnPage: put a tracked node at (col, row)
...
extern int  func_ov008_02060068(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode, u32 nPage, u32 nCol, u16 nRow, int nOne);
extern int  func_ov008_02060b58(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode, u32 nPage, int nCol, int nRow);
extern int *func_ov008_0205da50(Ov008MenuContext *pCtx, u32 nTag);          /* Ov008_FindEntryBy1BasedTag */
extern void func_ov008_02056988(int *pTexture, int *pParams);              /* Ov008_GetTextureParams */

int func_ov008_02060d38(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode, u32 nPage, u32 nCol, u16 nRow)
{
    int nResult;
    int nIndex;
    Ov008GridDisplayCell *pCell;

    if (func_ov008_02060068(pCtx, pNode, nPage, nCol, nRow, 1) == 0) {
        return 0;
    }
    ...
    nResult = func_ov008_02060b58(pCtx, pNode, nPage, nCol, nRow);
    if (nResult != 0) {
        if (pCtx->nVisiblePage == nPage) {
            pCell = &pCtx->gridDisplayCells[nRow][nCol];
            pCell->isActive = 1;
            pCell->displayState = DISPLAY_CELL_ACTIVE_STATE;
            func_ov008_02056988(pCell->aTexture, (int *)func_ov008_0205da50(pCtx, pNode->nTag)[1]);
        }

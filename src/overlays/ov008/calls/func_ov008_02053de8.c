/* func_ov008_02053de8 -- route a fresh press to the first widget under the pointer.
 * The input is sampled (and, if the sampler declines, filled in by func_ov008_02053d7c), then
 * only a NEWLY pressed key counts -- `new & (new ^ previous)` against the copy at +0x4a48.
 * The widget list at +0x4a38 is walked in order and hit-tested with func_ov008_02053cc4; the
...
extern int func_ov008_02055dcc(Ov000_Input *in);
extern void func_ov008_02053d7c(int self, Ov000_Input *in);
extern void *NNS_FndGetNextListObject(void *list, void *obj);
extern int func_ov008_02053cc4(int self, int x, int y, void *obj);
extern void MI_CpuCopy8(const void *src, void *dst, unsigned int n);

int func_ov008_02053de8(int self) {
    Ov000_Input in;
    int hit;
    void *obj;

    hit = 0;
    if (func_ov008_02055dcc(&in) == 0) {
        func_ov008_02053d7c(self, &in);
    }

    if ((in.keys & (in.keys ^ *(unsigned short *)(self + 0x4a48))) != 0) {
        obj = NNS_FndGetNextListObject((void *)(self + 0x4a38), 0);
        while (obj != 0) {
            if (func_ov008_02053cc4(self, in.x, in.y, obj) != 0) {

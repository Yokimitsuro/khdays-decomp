/* func_ov008_02083654 -- Ov008_RefreshFilterRows: move the filter-row
...
extern UiLayoutPos *func_ov008_02054820(void *pGroup, void *pWidget);   /* Ov008_GetEntryPos */
extern void func_ov008_02054858(void *pGroup, void *pWidget, UiLayoutPos *pPos); /* Ov008_SetEntryPos */
extern void func_0203011c(void *pRegion, int nX, int nY, int nW, int nH);
extern void func_ov008_02083588(int nRow, int nLabel, int bOn);         /* Ov008_ShowFilterRow */
extern u16 *func_ov008_02055c84(void *pCache, int nIndex);              /* GetVarRecordByIndex */
extern void func_ov008_02082fa8(u16 *pText, int nArg);                  /* Ov008_DrawDescriptionText */

void func_ov008_02083654(void)
{
    Ov008PanelContext *ctx = data_ov008_02090fac;
    ...
    pos.nX = func_ov008_02054820(pWidgets, func_ov008_02054788(pWidgets, 1))->nX;
    ...
    if (bAllRows != 0) {
        for (i = 0; i < FILTER_ROWS; i++) {
            func_ov008_02083588(i, i, i == pRows->nSelected);
        }
    } else if (bFourRows != 0) {
        for (i = 1; i < FILTER_ROWS; i++) {
            nLabel = i <= 3 ? i - 1 : i;
            func_ov008_02083588(i, nLabel, nLabel == pRows->nSelected);
        }
    } else {
        for (i = 1; i < 4; i++) {
            nLabel = i <= 2 ? i - 1 : i + 1;
    ...
    func_ov008_02082fa8(func_ov008_02055c84(ctx->textLoader, pRows->nSelected + TEXT_DESCRIPTION_BASE), -1);
}

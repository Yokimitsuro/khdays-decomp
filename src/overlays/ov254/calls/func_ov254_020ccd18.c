/* Movement step of the ov254 actor (ov107 020c3e50 variant): while moving (+0x50 == 1, +0x60
 * bit 0) gravity (+0x58 * +0x5c) pulls the +0xfc velocity unless airborne-locked (bit 6) or
 * grounded (+0x17a bit 0); the velocity is scaled by the frame factor plus the +0x16c push over
 * +0x178 frames. With a collision world (+0x7c of +4) a wall probe (radius 0x3d98) from the +0x408
...
void func_ov254_020ccd18(struct AiState *self, int factor)
{
    VecFx32 position;
    VecFx32 direction;
    CollisionHit *hit;
    ...
    if ((((u32)(self->flags60 << 24) >> 24) & 4) == 0) {
        ...
        position = *(VecFx32 *)(*(int *)((char *)self + 0x408) + 0x14);
        hit = func_01fff948(owner->field_7c, &position,
                            (VecFx32 *)&self->field_fc, 0x3d98);
        if (hit != 0) {
            self->field_114.x =
    ...
    if (hit != 0 && hit->field_08 == 0) {
            func_01ffd144(hit->field_0c, (int *)&direction,
                          (int *)&self->field_fc);
            self->field_fc.y += 0x7b31;

 ...
 */
extern void func_ov049_020b3f1c(int self);
extern void func_ov049_020b3b2c(int self, char *p);
...
void func_ov049_020b3728(int self, int mode) {
    int p = self + 0x2cfc;
    int kind = -1;
    int arg = 0;
    int i;
    ...
    switch (mode) {
    case 0x2e:
        if (mode != *(int *)(self + 0x6bc)) { func_ov049_020b3f1c(self); }
        break;
    case 0x2f:
    case 0x30:
        *(int *)(p + 0x118) = (mode == 0x30);
        if (mode != *(int *)(self + 0x6bc)) { func_ov049_020b3b2c(self, (char *)p); }
        break;
    }

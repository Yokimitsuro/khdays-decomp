 ...
 * 0x167 at the anchor, hands the rider to 020ad8e0 and moves on to func_ov272_020d2d84. */
...
extern void func_ov272_020d0854(int world, Vec3 *pos, int rad, int height);
...
extern void func_ov272_020d2d84(int *node);
...
void func_ov272_020d29f0(int *node)
{
    ...
    *(Quat *)(state + 3) = *(Quat *)(state + 7);
    ...
    if (VEC_Mag(&d) > 0x1800 || (*(unsigned int *)(state[2] + 0x1e4) & 0x20)
        ...
        func_ov272_020d0854(*(int *)(*state + 4), &pos, rad, *(int *)(*state + 0x80));
        func_ov107_020c5c54(*state, &pos);
    ...
    if (!(state[0x14] < 0x1000)) {
        ...
        *(int *)(*state + 0x3ac) = *(int *)(state[2] + 0x18c);
        ...
        state[0x14] = 0;
        func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov272_020d2d84);
        return;
    }
}

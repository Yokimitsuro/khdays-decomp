 ...
 * in mode 1 the parts are steered by Ov098_SteerPart, otherwise the shared handler runs and
...
extern void func_ov098_020bb794(char *group, char *part, void *arg);            /* Ov098_SteerPart */
...
extern void (*data_ov098_020bbcf4[])(char *group, char *part, void *arg);

void func_ov098_020bb634(char *group, void *arg)
{
    int state;
    char *part;
    int launched;
    ...
    for (i = 0; i < *(u8 *)(group + 0x19); i++) {
        ...
        if (state != 2) {
            data_ov098_020bbcf4[state](group, part, arg);
        } else if (*(int *)group == 1) {
            func_ov098_020bb794(group, part, arg);
        } else {

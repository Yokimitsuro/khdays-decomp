/* func_ov284_020cc61c -- tick this enemy's stagger timer (+0x21a) and, when a hit lands, flip
 * the node's facing bit and fire reaction 0x16c.
 *
...
int func_ov284_020cc61c(int param_1, int param_2, unsigned int *param_3) {
    ...
    *(short *)(param_1 + 0x21a) = (short)rem;
    ...
    if ((int)param_3[10] > 0) {
        ...
        if ((hf->lo & 8) == 0 || (hf->lo & 0x80) == 0 || hf->hi != 0x80) {
            fb = (struct NodeFacing *)((char *)node + 0x26);
            if ((hf->lo & 0x22) != 0) {
                fb->facing = fb->facing + 1;
                func_ov107_020c5af8(param_1, 0x16c, (fb->facing & 1) ? 2 : 3, node[1]);
            } else {
                fb->facing = fb->facing + 1;
                func_ov107_020c5af8(param_1, 0x16c, (fb->facing & 1) ? 0 : 1, node[1]);
            }
        }
    }
    return 1;

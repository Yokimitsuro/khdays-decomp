/* func_ov162_020cfd64: ported from a matched sibling family (same shape, constants and offsets adjusted). */
...
extern const struct Msg data_ov162_020d10ae;
extern void func_ov162_020d01fc(void);

void func_ov162_020cfd64(struct Node *node)
{
    struct State *st;
    FxVec vFwd;
    FxVec vToTarget;
    ...
    if (nHits > 0) {
        ...
        do {
            ...
            if (func_ov107_020ca918(aVictims[i], st->pActor, st->pActor, 2,
                ...
                hit = 1;
                func_ov107_020c5af8(st->pActor, 0x152, 5, &vImpact);
            }
            i++;
        } while (i < nHits);
    }
    ...
    func_0203c634(node, node->bSlot, (void *)func_ov162_020d01fc);
}

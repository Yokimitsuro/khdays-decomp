 ...
 * template (data_ov275_020d605a for the first foot, data_ov275_020d603e for the second) carries
...
extern const PosMsg data_ov275_020d605a;
extern const PosMsg data_ov275_020d603e;
    ...
    if (n > 0) {
        do {
            ...
            if (func_ov107_020ca918(hits[i], state->pOwner, state->pOwner, 3, &push, 0) != 0) {
                ...
                func_ov107_020c5af8(state->pOwner, 0x163, 0xc, &at);
            }
        } while (++i < n);
    }
}

void func_ov275_020d4b48(int *node)
{
    ...
    if (state->nTimer > 0x2aa && state->nTimer < 0xc44) {
        SweepFoot(state, 0x3bc, data_ov275_020d605a, hits, &sphere);
        SweepFoot(state, 0x3c0, data_ov275_020d603e, hits, &sphere);
    }
    if (*state->pBusy != 0) {
        return;
    }

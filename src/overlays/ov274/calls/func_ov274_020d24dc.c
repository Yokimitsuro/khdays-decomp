 ...
 * acceptance, the overlay's 14-byte template (data_ov274_020d4248 / data_ov274_020d422c)
...
extern const PosMsg data_ov274_020d4248;
extern const PosMsg data_ov274_020d422c;
...
void func_ov274_020d24dc(int *node)
{
    ...
    if (state->nTimer >= 0x1b28 && state->nTimer <= 0x1ee6) {
        SweepFoot(state, 0x3bc, data_ov274_020d4248, hits, &sphere, 1);
    }
    if (state->nTimer >= 0xded && state->nTimer <= 0x1199) {
        SweepFoot(state, 0x3c0, data_ov274_020d422c, hits, &sphere, 2);
    }
    if (*state->pBusy != 0) {
        return;
    }

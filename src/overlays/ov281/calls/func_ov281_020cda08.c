...
extern void func_ov107_020c5af8(Ov281Actor *actor, int effect, int mode, int context);
...
void func_ov281_020cda08(Ov281ActionNode *node)
{
    Ov281ActionState *state;
    MtxFx33 mtx;
    Sphere sphere;
    ...
    if (state->effectStarted == 0) {
        if (state->timer >= 0x2a8) {
            ...
            func_ov107_020c5af8(state->actor, 0x169, 4, state->hitContext);
        }
    }

    if (state->timer >= 0xff0) {
        sphere = *(Sphere *)&state->actor->position;
        if (func_ov281_020ccae4(state, 1, &sphere, 0, 0x1000) != 0) {
    ...
    if (state->subState->active == 0) {
        ...
        func_0203c634(node, node->slot, func_ov281_020cdb9c);
    }
}

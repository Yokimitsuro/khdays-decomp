...
/* Apply a scaled area hit once per actor and emit the corresponding reaction effect. */
void *func_ov281_020ccae4(struct Ov281HitReactionState *state, u32 mode,
                          QuerySphere *query, VecFx32 *position, int scale)
{
    ...
    if (result != 0) {
        ...
        case 1:
        case 2:
            func_ov107_020c5af8(state->owner, 0x169, 5,
                                state->reactionContext0c);
            break;
        }
    }

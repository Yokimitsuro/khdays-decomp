/* Jump wind-up tick of the ov206 enemy. Until flagged (+0x52) the +0x24 timer accumulates the
 * owner's rate and past 0xeee fires reaction 0x116 mode 0xe at the +8 point and sets the
 * flag. Once the +0xc idle byte is clear the overlay's 14-byte position message
 * (data_ov207_020d420c, the +4 point packed as 24-bit values) goes to the owner's +0x24 hook,
 ...
 * heading of the direction and the tick hands over to func_ov207_020d3554. */
...
extern const PosMsg data_ov207_020d420c;
extern const Vec3 data_02041dc8;
extern const short data_0203d210[];
extern void func_ov207_020d3554(int *node);

void func_ov207_020d32c8(int *node)
{
    ...
    if (*(u8 *)((char *)state + 0x52) == 0 && state[9] >= 0xeee) {
        func_ov107_020c5af8(*state, 0x116, 0xe, (void *)state[2]);
        *(u8 *)((char *)state + 0x52) = 1;
    }
    if (*(u8 *)state[3] != 0) {
        return;
    }
    msg = data_ov207_020d420c;
    pPos = (Fx32 *)state[1];
    scratchX = pPos[0];
    ...
    *(Vec3 *)(state + 0xc) = data_02041dc8;
    ...
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov207_020d3554);
}

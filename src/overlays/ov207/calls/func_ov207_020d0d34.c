/*
 * Hit handler of the ov206 enemy. A hit is refused once the stagger timer (+0x21a) has run
 ...
 * +0x28 and, past a tenth of the +0x218 cap (mode 0x3000) or for a flag-8/0x80 hit (mode
 ...
 * the +0x3b0 body from an unmarked attacker fires reaction 0x116 mode 8 at the body's
 * transform for a positive mode word, zeroes the damage, sets result bits 0/3 and returns 1.
 * A hit that charged flips the state's facing bit (+0x51 bit 0) and fires reaction 0x116 at
...
int func_ov207_020d0d34(char *actor, char *attacker, struct ActorHitEvent *hit)
{
    struct Ov206ActionState *state = *(struct Ov206ActionState **)(actor + 0x214);
    struct Vec3 d;
    struct Vec3 v;
    int delta;
    int rem;
    ...
    if ((int)hit->pSource18 == *(int *)(actor + 0x3ac)) {
        ...
        if (*(signed char *)(state->pOwner + 0x1c6) == 7) {
            ...
            *(short *)(actor + 0x21a) = (short)rem;
            ...
            if ((hit->uFlagsLo & 8) != 0 && (hit->uFlagsHi & 0x80) != 0) {
                mode = 0x9000;
            }
            if (mode != 0) {
                state->nCharge = 0;
        ...
        if (state->aSlot[*(u16 *)(attacker + 2)] == 0) {
            ...
            if ((short)hit->uMode10 > 0) {
                func_ov107_020c5af8(state->pOwner, 0x116, 8, (char *)*hit->pSource18 + 4);
            }
            hit->nDamage = 0;
            hit->uResultLo |= 9;
            return 1;
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                func_ov107_020c5af8((int)actor, 0x116, (state->bFacing & 1) ? 2 : 3, (char *)*hit->pSource18 + 4);
            } else {
                state->bFacing = state->bFacing + 1;
                func_ov107_020c5af8((int)actor, 0x116, (state->bFacing & 1) ? 0 : 1, (char *)*hit->pSource18 + 4);
            }
        }
    }

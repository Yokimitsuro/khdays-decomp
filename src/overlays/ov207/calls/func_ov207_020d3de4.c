 ...
 * position is packed into the overlay's 14-byte template (data_ov207_020d426e) for the owner's
...
extern const PosMsg data_ov207_020d426e;

void func_ov207_020d3de4(int *node)
{
    int hit;
            ...
            do {
                if (((state->bHitMask50 >> *(u8 *)(hits[i] + 0x1b4)) & 1) == 0) {
                    ...
                    if (func_ov107_020ca918(hits[i], state->pOwner, state->pOwner, 2, &push, 0) != 0) {
                        ...
                        func_ov107_020c5af8(state->pOwner, 0x116, 0xc, (void *)(hit + 0x74));

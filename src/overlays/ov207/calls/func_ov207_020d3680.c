/* func_ov207_020d3680 = Ov207_RockFlightTick. Thrown-rock flight tick of the ov207 enemy (x4: ov206/ov207/ov274/ov275). The rock falls (the +0x34
 ...
 * data_ov207_020d41fe with the midpoint through the owner's +0x24 hook, fires reaction 0x116 mode
 * 0xc there and records the kind bit. Each breakable in the world's list gets a 0.5 push request;
 * a broken one reports the 0xe message of data_ov207_020d4260 at the sphere's rim. The rock lands
 * on a wall or floor whose plane is not flagged (bounced 0x200 up along it), when it was already
 * resting, or after 3.0 of flight: the landing message of data_ov207_020d41f0 is broadcast with
 * the point, animation 0x10 plays, reaction 0x116 mode 0x10 fires there and the tick hands over
 * to func_ov207_020d3de4. */
...
extern int func_ov207_020d0a54(void *collision, Vec3 *pos, int *out);
...
extern const Cmd14 data_ov207_020d4260;
extern const Cmd14 data_ov207_020d41f0;
extern void func_ov207_020d3de4(int *node);
...
void func_ov207_020d3680(int *node)
{
    ...
    *(Vec3 *)(state + 5) = *(Vec3 *)(state + 0xc);
    ...
    for (i = 0; i < scan[k].nHits; i++) {
        ...
        func_ov107_020c5af8(*state, 0x116, 0xc, &at);
        *(u8 *)((char *)state + 0x50) |= 1 << *(u8 *)(hits[i] + 0x1b4);
    }
    m = func_ov207_020d0a54(*(void **)(scan[k].world + 0x7c), &sphere.c, list);
    for (j = 0; j < m; j++) {
        BreakReq req = {0};
        Vec3 d;
        Vec3 rim;
    ...
    state[9] = 0;
    func_ov107_020c5af8(*state, 0x116, 0x10, &pos);
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov207_020d3de4);
}

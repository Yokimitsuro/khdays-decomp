/* Attack wait tick of the ov158 enemy: the +0x3c timer accumulates the owner's rate. In phase
 * 0 (+0x54) past 0x22aa (both conditions in one test, which keeps the phase byte live) the data_ov159_020d4fa0[4..5] message pair is sent to the +0x24 hook
 * (arg 4), reaction 0x150 mode 6 fires at the +0x398 bone's +0x14 and phase 1 begins; in phase 1
...
void func_ov159_020d32f8(int *node)
{
    ...
    state[5] = *(int *)(node[0] + 0x2c) * 30 / 10;
    state[0xf] += *(int *)(node[0] + 0x2c);
    if (*(unsigned char *)((char *)state + 0x54) == 0 && state[0xf] > 0x22aa) {
        pp = pairA;
        pp[1] = data_ov159_020d4fa0[5];
        pp[0] = data_ov159_020d4fa0[4];
        cb = *(void (**)())(*state + 0x24);
        if (cb != 0) cb(*state, pp, 4);
        func_ov107_020c5af8(*state, 0x150, 6, (void *)(*(int *)(*state + 0x398) + 0x14));
        *(unsigned char *)((char *)state + 0x54) = 1;
    } else if (*(unsigned char *)((char *)state + 0x54) == 1 && state[0xf] > 0x23bb) {
        pp = pairB;
        pp[1] = data_ov159_020d4fa0[3];
        pp[0] = data_ov159_020d4fa0[2];
        cb = *(void (**)())(*state + 0x24);
        if (cb != 0) cb(*state, pp, 4);

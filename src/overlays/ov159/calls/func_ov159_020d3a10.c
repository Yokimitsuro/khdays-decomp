/* Ground probe of the ov158 enemy's descent. With a +0x2c target that has a +0x18c body with a
 * +0x20 collision handle, the +0x1c/+0x24 x/z ease a fifteenth towards the target's +0xb0/+0xb8 and
...
int func_ov159_020d3a10(int *state, Vec3 *dir, Vec3 *out)
{
    ...
    state[7] += (*(int *)(target + 0xb0) - state[7]) / 15;
    state[9] += (*(int *)(target + 0xb8) - state[9]) / 15;
    state[8] = 0;
    origin.x = state[7];
    origin.y = state[5];
    origin.z = state[9];

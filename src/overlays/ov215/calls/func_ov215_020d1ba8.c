/* Per frame step of the bounce (Ghidra: Ov215_StepBounceOffContact).
 *
 * Once the bounce count reaches its maximum the move ends: the actor is parked,
 * its callback cleared and the task handed on. Otherwise the step sweeps ahead
 * for a wall and, on contact, reflects the heading about the contact normal,
 * spends one bounce and stops the actor dead. With no contact it acquires a
 * target and homes -- the delay branch decays speed in fixed 0x88 slices and
 * applies gravity, the search branch spirals the heading through the shared
 * sine table while easing speed toward a target that depends on how many
 * bounces are left.
 *
 * The assignment inside the final `if` is load bearing, not a typo: writing the
...
struct Ov215CollisionResult {
    char pad000[4];
    struct Ov215CollisionShape *pShape04;
    int nType08;
};

struct Ov215CollisionFlags {
    unsigned char bGrounded : 1;
};

extern void func_ov107_020c9264(struct Ov215Actor *actor, int mode, int zero);
extern void func_ov107_020c0b90(struct Ov215Actor *actor, int mode, struct VecFx32 value, int zero);
extern struct Ov215CollisionResult *func_01fff8e8(void *collision, struct VecFx32 *origin,
...
extern int func_ov107_020cab14(struct Ov215Actor *actor, int kind);
...
extern void func_0203c634(struct Ov215Task *task, int slot, void *callback);
extern void func_ov215_020d0a84(struct Ov215State *state, unsigned int kind,
                                struct Ov215Params *params);
extern void func_ov215_020d222c(void);
...
void func_ov215_020d1ba8(struct Ov215Task *task)
{
    ...
    if (state->nPhase6e >= state->nPhaseMax6f) {
        ...
        state->nCallback50 = 0;
        state->bEffect70 = 0;
        func_0203c634(task, task->nSlot20, func_ov215_020d222c);
        return;
    }
    ...
    if (collision != 0) {
        ...
        state->pTarget = (struct Ov215Actor *)func_ov107_020cab14(state->pActor, 0);

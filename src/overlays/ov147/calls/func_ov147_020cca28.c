...
extern void func_ov147_020cdc9c(int node, int arg, Vec3 *dir);
extern Xform data_02042264;
extern const short data_0203d210[];

void func_ov147_020cca28(int *self, int height, int arg) {
    Xform rot;
    Vec3 dir;
    int i;
    int idx;
    ...
    for (i = 0; i < 3; i++) {
        dir.y = height;
        func_ov147_020cdc9c(*(int *)((char *)*self + i * sizeof(int) + 0x398), arg, &dir);
        if (i + 1 < 3) {
            func_0202f384(&dir, &rot, &dir);
        }
    }

/* Engage entry of the ov115 enemy (and its byte-identical twins). Looks for a target in reach:
...
/* data_ov115_020ceb70: the two impact reaction pairs, then the engage message template.
 * The template is copied as a typed struct member (read through a cast pointer the copy is
 * scheduled differently). */
struct Ov115Templates { u8 impactPairs[4]; struct Msg6 engageMsg; };
...
extern void func_ov115_020ce6b0(int *node);

void func_ov115_020ce5dc(int *node)
{
    ...
    *(u8 *)(state + 0x21) = 0;
    ...
    func_0203c634(node, *(signed char *)((char *)node + 0x20), func_ov115_020ce6b0);
}

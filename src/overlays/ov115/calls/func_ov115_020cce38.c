...
extern const u8 data_ov115_020ceb70[];

int func_ov115_020cce38(char *actor, int nParam, struct ActorHitEvent *hit)
{
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                uSlot.sPair = *(const struct ImpactPair *)&data_ov115_020ceb70[2];
                state->nToggle85++;
                func_ov107_020c5af8(actor, 0x114, uSlot.aModes[state->nToggle85 & 1], state->pPos);
            } else {
                union ImpactSlot uSlot;

                uSlot.sPair = *(const struct ImpactPair *)&data_ov115_020ceb70[0];
                state->nToggle85++;
                func_ov107_020c5af8(actor, 0x114, uSlot.aModes[state->nToggle85 & 1], state->pPos);
            }
        }

/* Tick handler of the ov191 enemy (x3: ov191/192/193). If flagged (+0x1c4 & 0xa) and idle
 * (+0x1c7 == -1) and the current sub-state (+0x1c6) is not 0, 1, 3 or 5, force sub-state 5.
 * Then the placements are shuffled: the actor's own +0xa0 placement goes into the +0x390 frame,
 * the +0x398 source's (+4) into the +0x38c pool's first node and into the +0x394 frame, and the
 * +0xa0 placement again into the +0x388 pool's first node. Unless in sub-state 4 the two
...
void func_ov193_020d3eb0(int self)
{

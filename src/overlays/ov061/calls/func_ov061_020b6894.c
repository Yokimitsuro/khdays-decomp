 ...
 * in mode 1 the parts are steered by Ov061_SteerPart, otherwise the shared handler runs and
...
extern void func_ov061_020b69f4(char *group, char *part, void *arg);            /* Ov061_SteerPart */
...
extern void (*data_ov061_020b6f54[])(char *group, char *part, void *arg);

void func_ov061_020b6894(char *group, void *arg)
{
    int state;
    char *part;
    int launched;
    ...
    for (i = 0; i < *(u8 *)(group + 0x19); i++) {
        ...
        if (state != 2) {
            data_ov061_020b6f54[state](group, part, arg);
        } else if (*(int *)group == 1) {
            func_ov061_020b69f4(group, part, arg);
        } else {

/* func_ov232_020d2390 -- lunge tick of the ov231 enemy (x5 with ov232/ov263/ov265/ov280). The +0x30
 ...
 * point. Between 0x1298 and 0x2b90 the target is re-acquired (func_ov232_020d0620) and a probe
 * segment from the +0xc point lifted 2.14 along data_02042240 (length 1.0, radius 2.0) hits kind 2
 * candidates towards the heading lifted 2.0 (func_ov232_020d0888); on a hit reaction 0/0x51 fires at
...
extern int func_ov232_020d0888(int *self, int kind, void *query, void *pt, int flags);
...
void func_ov232_020d2390(int *node)
{
    ...
    *(Vec3 *)(state + 0xc) = v;
    ...
    if (state[0xa] >= 0x1298 && state[0xa] < 0x2b90) {
        ...
        seg.nLength = 0x1000;
        seg.nRadius = 0x200f;
        if (func_ov232_020d0888(node, 0, &seg, &dir, 2) != 0) {

/* func_ov163_020cf808: ported from a matched sibling family (same shape, constants and offsets adjusted). */
...
extern const struct Msg data_ov163_020d0e7e;
extern void func_ov163_020cfc9c(void);

void func_ov163_020cf808(struct Node *node)
{
    struct State *st;
    FxVec vFwd;
    FxVec vToTarget;
    ...
    func_01ffa724(st->nSpeed4c, &st->vStep, &st->vStep);
    ...
    if (nHits > 0) {
        ...
        do {
            ...
            if (func_ov107_020ca918(aVictims[i], st->pActor, st->pActor, 2,
                ...
                hit = 1;
                func_ov107_020c5af8(st->pActor, 0x153, 5, &vImpact);
            }
            i++;
        } while (i < nHits);
    }
    ...
    func_0203c634(node, node->bSlot, (void *)func_ov163_020cfc9c);
}

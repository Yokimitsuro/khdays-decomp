/* func_ov163_020ce6ac: hit handler of the ov163 enemy (x3), ported from the matched ov131 sibling; the health always drops here, and a kind-4 hit in sub-state 7 flattens and normalises the impact direction (falling back to the anchor-to-source direction when it is vertical). */
...
int func_ov163_020ce6ac(char *actor, int source, struct HitMsg *msg)
{
    struct HitState *hs;
    int state;
    int left;

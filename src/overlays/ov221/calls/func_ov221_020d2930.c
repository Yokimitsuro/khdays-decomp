...
extern void func_ov221_020d3f38(int self, int mode, Vec3 at, Vec3 facing, Vec3 dir);
...
void func_ov221_020d2930(int *node)
{
    ...
    if (*(u8 *)((char *)state + 0x75) == 0 && state[0x17] >= 0x17e8) {
        ...
        for (i = 0; i < n; i++) {
            ...
            func_ov221_020d3f38(((struct Ov221Family *)*state)->aChildren[i], bNoTarget, vAt, vFacing, vChildDir);
            func_0202f384(&vDir, &qSpread, &vDir);
            VEC_Add(&vAt, &vStep, &vAt);
        }
        func_ov107_020c0b90(*state, 2, vZero, state[0x1e] == 0);
        func_ov107_020c5af8(*state, 0x12a, 7, (void *)state[2]);

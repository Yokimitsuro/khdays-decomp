 ...
 * blocking hit bursts (ov221 4498 with the sphere's point, flag 1), fires reaction 0x12a mode 9
...
extern int func_ov221_020d44e0(int *node, Sphere *sphere);
...
extern void func_ov221_020d4498(int *state, Vec3 v, int flag);
extern void func_ov107_020c5af8(int owner, int id, int mode, void *at);
extern int VEC_Mag(const Vec3 *v);

void func_ov221_020d4950(int *node, int bStrike, int bBurst)
{
    ...
    if (bStrike != 0 && func_ov221_020d44e0(node, &sphere) != 0) {
    ...
    if (hit != 0) {
        if (bBurst != 0) {
            func_ov221_020d4498(state, sphere.pos, 1);
            func_ov107_020c5af8(*state, 0x12a, 9, (void *)state[2]);
            if (bStrike != 0) {
                func_ov221_020d44e0(node, &sphere);
            }
        }
        *(unsigned char *)(*state + 0x1c7) = 0;
        func_0203c634(node, *(signed char *)((char *)node + 0x20), 0);
    ...
    if (hit != 0 && hit->nBlocked == 0) {
        if (bBurst != 0) {
            func_ov221_020d4498(state, sphere.pos, 0);
            func_ov107_020c5af8(*state, 0x12a, 9, (void *)state[2]);
            if (bStrike != 0) {
                func_ov221_020d44e0(node, &sphere);
            }

 ...
 * rate and once (+0x75 bit 2) past 0x2420 fires reaction 0x12a mode 0x11 at the +8 point when
 ...
 * zero-vector mode-0xa message with flag 3 and hands over to func_ov221_020d324c, otherwise
 * sub-state 6 ends the state. While busy, a step shorter than 0x80 does nothing; otherwise
 * sub-action 2 (target) or 4 runs (func_ov221_020d0a2c). */
...
extern void func_ov221_020d0a2c(int *state, int action, int arg);
extern Vec3 data_02041dc8;
extern void func_ov221_020d324c(int *node);

void func_ov221_020d3064(int *node)
{
    ...
    *(Vec3 *)(state + 5) = *(Vec3 *)(state + 8);
    ...
    if ((*(unsigned char *)((char *)state + 0x75) & 4) == 0 && state[0x17] >= 0x2420) {
        if (state[0x1e] == 0) {
            func_ov107_020c5af8(*state, 0x12a, 0x11, (void *)state[2]);
        }
    ...
    if (*(unsigned char *)(state[1] + 0xad) == 0) {
        if (state[0x1e] != 0) {
            if (((struct Bit0 *)(*state + 0x17a))->bit0 != 0) {
                ...
                func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov221_020d324c);
                return;
            }
    ...
    func_ov221_020d0a2c(state, state[0x1e] != 0 ? 2 : 4, 0);
}

/* Ov119_TickOrbitTarget -- orbit tick of the ov119 enemy (x3 with ov272/ov279). The target is re-acquired into +8 (none:
 * the tick hands over to func_ov119_020ce8a8). The goal point sits on the circle of radius +0x68
 * around the target's +0x74 point at the +0x64 angle, lifted 3.0; the +0x30 step heads from the
 * +0x4c point towards it at up to 1.0. The +0x5c bob phase advances (half sine, lifted 2.0, into
 * +0x60, with the +0x34 climb following the owner's +0x13c height), and once the +0x50 timer
 * reaches 1.0 the tick hands over to func_ov119_020ce8a8. */
...
extern void func_ov119_020ce8a8(int *node);

void func_ov119_020ce674(int *node)
{
    ...
    target = state[2];
    if (target == 0) {
        func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov119_020ce8a8);
        return;
    }

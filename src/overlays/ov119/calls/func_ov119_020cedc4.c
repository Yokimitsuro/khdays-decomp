/* Ov119_TickHoldTarget -- hold tick of the ov279 enemy: keeps itself at the point 1.875 in front
 ...
 * 0x167 at the anchor, hands the rider to 020ad8e0 and moves on to func_ov119_020cf158. */
...
extern void func_ov119_020ccc48(int world, Vec3 *pos, int rad, int height);
...
extern void func_ov119_020cf158(int *node);
...
void func_ov119_020cedc4(int *node)
{
    ...
    pos.y = tpos->y;
    pos.z = tpos->z + FX_MUL(data_0203d210[ANG2IDX(rad) * 2 + 1], 0xf00);
    ...
    *(Quat *)(state + 3) = *(Quat *)(state + 7);
    ...
    if (VEC_Mag(&d) > 0x1800 || (*(unsigned int *)(state[2] + 0x1e4) & 0x20)
        ...
        func_ov119_020ccc48(*(int *)(*state + 4), &pos, rad, *(int *)(*state + 0x80));
        func_ov107_020c5c54(*state, &pos);
    ...
    if (!(state[0x14] < 0x1000)) {
        ...
        *(unsigned short *)(*state + 0x1ae) &= ~1;
        ...
        func_ov107_020c5af8(*state, 0x121, 7, (void *)state[0x13]);
        *(int *)(*state + 0x3ac) = *(int *)(state[2] + 0x18c);
        func_ov022_020ad8e0(*(int *)(*state + 0x3ac), 1);
        state[0x14] = 0;
        func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov119_020cf158);
        return;
    }
}

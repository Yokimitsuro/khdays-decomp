/* Approach tick of the ov119 enemy (x2 with ov279). Gives up (sub-state 2, tick cleared) once
...
extern void func_ov119_020cdc88(int *node);

void func_ov119_020cdaec(int *node)
{
    int target;
    ...
    if (!(gap >= 0x2000)) {
        ...
        *(Quat *)(state + 3) = *(Quat *)(state + 7);
        func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov119_020cdc88);
        return;
    }
}

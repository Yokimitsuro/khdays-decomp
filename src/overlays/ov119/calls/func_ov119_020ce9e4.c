/* Lunge entry of the ov119 enemy (x3 with ov272/ov279): effect 1 spawns at the +0x4c point,
 * reaction 0x121 mode 4 fires there, animation 1 plays (looping), bit 0 of +0x1ae is raised, bit
 * 0 of the +0x388 part's flag byte clears, bits 1 and 2 of the +0x60 high byte are raised, the
 * +0x50 timer restarts and the tick hands over to func_ov119_020ceac8. */
...
extern void func_ov119_020ceac8(int *node);

void func_ov119_020ce9e4(int *node)
{
    int *state = (int *)node[1];
    unsigned short v;

    func_ov107_020c0b90(*state, 1, *(Vec3 *)state[0x13], 0);
    func_ov107_020c5af8(*state, 0x121, 4, (void *)state[0x13]);
    ...
    *(unsigned short *)(*state + 0x60) = (unsigned short)((v & ~0xff00) | (((((unsigned int)v << 0x10) >> 0x18 | 6) << 0x18) >> 0x10));
    state[0x14] = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov119_020ceac8);
}

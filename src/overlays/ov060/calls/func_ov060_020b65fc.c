 ...
 * nothing. */
extern void func_ov060_020b68ac(char *self, char *slot, void *arg);
extern void func_ov060_020b66e4(char *self, char *parts, int angle, int spread);

void func_ov060_020b65fc(char *self, char *parts, void *arg)
{
    int progress;
    int hit;
    int spread;
    ...
    slot = parts + 0x330;
    for (i = 0; i < 3; i++) {
        func_ov060_020b68ac(self, slot, arg);
        slot += 0x110;
    }
    ...
    if (hit) {
        func_ov060_020b66e4(self, parts, angle, spread);
    }
}

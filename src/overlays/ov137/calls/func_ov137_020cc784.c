/*
 * Hit handler of the ov137 enemy, the func_ov120_020cc568 shape. A hit is refused once the
...
int func_ov137_020cc784(char *actor, int nParam, struct ActorHitEvent *hit)
{
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x11d, (state->bFacing & 1) ? 2 : 3, state->pAnchor);
            } else {
                state->bFacing = state->bFacing + 1;
                func_ov107_020c5af8(actor, 0x11d, (state->bFacing & 1) ? 0 : 1, state->pAnchor);
            }
        }
    }
    return 1;

 ...
 * 0 (+0x54) past 0x22aa (both conditions in one test, which keeps the phase byte live) the data_ov137_020cf37c[4..5] message pair is sent to the +0x24 hook
...
void func_ov137_020cd8b4(int *node)
{
    ...
    if (*(unsigned char *)((char *)state + 0x54) == 0 && state[0xf] > 0x22aa) {
        pp = pairA;
        pp[1] = data_ov137_020cf37c[5];
        pp[0] = data_ov137_020cf37c[4];
        cb = *(void (**)())(*state + 0x24);
        ...
        *(unsigned char *)((char *)state + 0x54) = 1;
        ...
        pp = pairB;
        pp[1] = data_ov137_020cf37c[3];
        pp[0] = data_ov137_020cf37c[2];
        cb = *(void (**)())(*state + 0x24);
        if (cb != 0) cb(*state, pp, 4);

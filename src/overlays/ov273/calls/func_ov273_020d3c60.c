 ...
 * +0x70 timer accumulates the frame rate and at 0.8 the flag is set and reaction 0x162 mode 0xe fires
...
void func_ov273_020d3c60(int *node)
{
    int obj;
    ...
    if (*((u8 *)state + 0x6a) == 0) {
        ...
        if (state[0x1c] >= 0xccc) {
            *((u8 *)state + 0x6a) = 1;
            func_ov107_020c5af8(*state, 0x162, 0xe, (void *)(part + 0x14));
        }
    }
    ...
    if (n > 0xc44 && n < 0xf77) {
        ...
        for (i = 0; i < n; i++) {
            ...
            *((u8 *)state + 0x69) |= 1 << *(u16 *)(hits[i] + 2);
            func_ov107_020c5af8(*state, 0x162, 7, (void *)(obj + 0x74));
        }
    }
    if (*(u8 *)state[2] != 0) {
        return;

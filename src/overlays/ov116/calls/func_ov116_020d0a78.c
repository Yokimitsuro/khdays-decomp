/*
 * Hit handler of the ov115 enemy (and its byte-identical twins): copies the hit point into +0x2c and the
 ...
 * +0x85 counter and fires reaction 0x114 at the +8 position with the counter-alternated mode
...
extern const u8 data_ov116_020d27b0[];

int func_ov116_020d0a78(char *actor, int nParam, struct ActorHitEvent *hit)
{
    struct Ov115ActionState *state = *(struct Ov115ActionState **)(actor + 0x214);
    int delta;
    int rem;
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                uSlot.sPair = *(const struct ImpactPair *)&data_ov116_020d27b0[2];
                state->nToggle85++;
                func_ov107_020c5af8(actor, 0x114, uSlot.aModes[state->nToggle85 & 1], state->pPos);
            } else {
                union ImpactSlot uSlot;

                uSlot.sPair = *(const struct ImpactPair *)&data_ov116_020d27b0[0];
                state->nToggle85++;
                func_ov107_020c5af8(actor, 0x114, uSlot.aModes[state->nToggle85 & 1], state->pPos);
            }
        }

/* Target finder of the ov241 enemy (x3: ov241/242/243): from the actor's +0x3b0 heading builds
 ...
 * the 0xb50 cosine of the heading, and that func_ov243_020d3c94 accepts with its position and
...
extern int func_ov243_020d3c94(int *state, Vec3 pos, int radius);
extern const short data_0203d210[];
extern const Vec3 data_02042258;

int func_ov243_020d3d78(int node)
{
    int *state = *(int **)(node + 4);
    int heading = *(int *)(*state + 0x3b0);
    int best = 0;
        ...
        if (((struct flags40 *)(actor + 0x40))->bit1 && (((struct hw60 *)(actor + 0x60))->lo & 1) != 0) {
            ...
            if (dy <= 0x1e000) {
                ...
                if (dist < bestDist) {
                    ...
                    if (dist <= 0x1800 || (dist <= 0x8000 && VEC_DotProduct(&d, &fwd) >= 0xb50)) {
                        if (func_ov243_020d3c94(state, pos, *(int *)(actor + 0x80)) != 0) {
                            bestDist = dist;
                            best = actor;
                        }
                    }

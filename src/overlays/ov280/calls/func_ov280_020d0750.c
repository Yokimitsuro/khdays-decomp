...
extern void  func_ov280_020d06bc(VecFx32 *pIn, int *node, VecFx32 *pOut);
extern void  VEC_Add(VecFx32 *a, VecFx32 *b, VecFx32 *out);

int func_ov280_020d0750(int *node, VecFx32 *dir, int bRay)
{
    void *hit;
    ...
    if (*(void **)(world + 0x7c) != 0) {
        if (bRay) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                func_ov280_020d06bc(&off, node, &off);
                VEC_Add(&pos, &off, &pos);

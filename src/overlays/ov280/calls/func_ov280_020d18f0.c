/* func_ov280_020d18f0 -- per-tick update: refresh the cached aim point, then re-arm the driver.
 ...
 * (+0x18). Either way the driver is re-armed through func_ov280_020d0cf4 with
 * func_ov280_020d199c as the continuation. */
...
extern void func_ov280_020d06bc(Vec3 *out, int self, int rig);
extern int func_ov280_020d0628(int self);
extern void func_ov280_020d0cf4(int self, int a, int b, int c, void (*cb)(void));
extern void func_ov280_020d199c(void);

void func_ov280_020d18f0(int self) {
    int *ctx;
    Vec3 aim;

    ctx = *(int **)(self + 4);
    func_ov280_020d06bc(&aim, self, *(int *)(ctx[0] + 0x388) + 0x2c);
    ...
    *(unsigned char *)((char *)ctx + 0x48) = 0;
    ...
    func_ov280_020d0cf4(self, 3, 3, 1, func_ov280_020d199c);
}

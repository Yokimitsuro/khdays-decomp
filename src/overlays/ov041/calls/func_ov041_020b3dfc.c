 ...
 * nothing. */
extern void func_ov041_020b40ac(char *self, char *slot, void *arg);
extern void func_ov041_020b3ee4(char *self, char *parts, int angle, int spread);

void func_ov041_020b3dfc(char *self, char *parts, void *arg)
{
    int progress;
    int hit;
    int spread;
    ...
    slot = parts + 0x330;
    for (i = 0; i < 3; i++) {
        func_ov041_020b40ac(self, slot, arg);
        slot += 0x110;
    }
    ...
    if (hit) {
        func_ov041_020b3ee4(self, parts, angle, spread);
    }
}

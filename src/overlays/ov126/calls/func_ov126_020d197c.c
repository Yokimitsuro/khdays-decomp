 ...
 * unless the +0x28 target's flag byte is set send the data_ov126_020d400c[4..5] message pair
...
extern void func_ov126_020d1a44(void);

void func_ov126_020d197c(int *node) {
    ...
    pp = pair;
    pp[1] = data_ov126_020d400c[5];
    pp[0] = data_ov126_020d400c[4];
    cb = *(void (**)())(*state + 0x24);
    if (cb != 0) cb(*state, pp, 4);
    func_ov107_020c9264(*state, 4, 1);
    state[0xb] = 0;
    *(unsigned char *)(state + 0xd) &= ~2;
    func_0203c634(node, *(signed char *)(node + 8), func_ov126_020d1a44);
}

/*
 * func_ov126_020d21f0 -- AI-state entry: wire up the sub-nodes and their hit regions, register
 * two handlers. Mark bit1 of *(node+0x5c) on each sub-node state[1..5]. On the first sub-node
 * state[1] also install the physics callback func_ov126_020d209c at +0x6c, back-link the state
...
extern void func_ov126_020d209c(void);
extern void func_ov126_020d239c(void);
extern void func_ov126_020d248c(void);

void func_ov126_020d21f0(int *self) {
    int *state = (int *)self[1];

    *(int *)(state[1] + 0x5c) |= 2;
    *(int *)(state[1] + 0x6c) = (int)&func_ov126_020d209c;
    ...
    *(int *)(state[5] + 0x5c) |= 2;
    ...
    func_0203c634((int)self, 0, (int)&func_ov126_020d239c);
    func_0203c634((int)self, 1, (int)&func_ov126_020d248c);
}

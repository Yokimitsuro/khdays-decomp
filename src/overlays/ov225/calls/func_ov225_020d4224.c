/* func_ov225_020d4224 -- retune, notify, and (in mode 1) start the homing move.
 ...
 * forwarded BY VALUE to func_ov225_020d468c (itself variadic).
 *
 ...
 * `rest` is never read, and leaves `mode` in r1. Its sibling func_ov225_020d468c has the same
...
extern void func_ov225_020d468c(int ctx, int mode, Vec3 a, Vec3 b);
...
void func_ov225_020d4224(int self, int mode, int rest, ...) {
    int m;
    void (*notify)(int, int);
    ...
    *(int *)(self + 0x38c) = 1;
    ...
    func_ov225_020d468c(*(int *)(self + 0x214), m, va_at(rest, 0xc), va_at(rest, 0x18));
}

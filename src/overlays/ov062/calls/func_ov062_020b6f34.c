 ...
 * row heads at +0x12c (stride 0x240), binds data_ov062_020b809c on the +0x10 request, then for
 * each of the 2 rows binds data_ov062_020b80b0 on its 2 requests (+0x130, stride 0x108) and
...
extern void func_ov062_020b79ec(int self);
extern char *data_ov062_020b80e0;
extern int data_ov062_020b809c;
extern int data_ov062_020b80b0;

void func_ov062_020b6f34(int self)
{
    char *owner = data_ov062_020b80e0;
    ...
    *(int *)(blk + 0x11c) = 0;
    ...
    func_0202a634(blk + 0x10, &data_ov062_020b809c, 1, *(unsigned char *)(owner + 9) + 7);
    for (i = 0, row = blk + 0x130; i < 2; i++) {
        for (j = 0, p = row; j < 2; j++, p += 0x108) {
            func_0202a634(p, &data_ov062_020b80b0, 1, *(unsigned char *)(owner + 9) + 7);
        }

/* Tick handler of the ov123 enemy (and its byte-identical twins). If flagged (+0x1c4 & 0xa) and idle
 * (+0x1c7 == -1) and the current sub-state (+0x1c6) is not 0, 1, 3 or 5, force sub-state 5.
 * Unless in sub-state 0x4 the two attachments of the +0x398 set (+0xc, +4) are released; then
 * the actor's own +0xa0 placement is copied into the +0x388 pool's first node and into the
 * +0x38c frame, and the base tick runs (cf. func_ov191_020d0270). */
...
void func_ov123_020cc420(int self)
{
    ...
    if (*(signed char *)(self + 0x1c6) != 0x4) {
        if (*(int *)(*(int *)(self + 0x398) + 0xc) != 0) {
            func_0203c650(list, *(int *)(*(int *)(self + 0x398) + 0xc));
            *(int *)(*(int *)(self + 0x398) + 0xc) = 0;
        }
        if (*(int *)(*(int *)(self + 0x398) + 4) != 0) {
            func_0203c650(list, *(int *)(*(int *)(self + 0x398) + 4));
            *(int *)(*(int *)(self + 0x398) + 4) = 0;
        }
    }
    {

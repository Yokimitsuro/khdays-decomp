/* Glide-to-land tick of an ov257 state: the +0x40 rate is the frame rate x 6 and the +0x5c path
 * point is resolved (func_ov257_020ccf98) into the +0x10 step. Once the +0xc idle byte clears and
 * the owner is grounded (+0x17a bit 0), animation 0xc plays, the +0x3d0 part plays motion 0xb,
 * reaction +0x408 (as a halfword) mode 3 fires at the +4 point and the tick hands over to
 * func_ov257_020ce784. */
typedef struct { int x, y, z; } Vec3;
struct Bits17a { unsigned char b0 : 1; };

extern void func_ov257_020ccf98(int *state, int point, Vec3 *dir, int *speed);
extern void func_01ffa724(int scale, const Vec3 *v, Vec3 *out);
extern void func_ov107_020c9264(int owner, int anim, int mode);
extern void func_ov107_020c9ee8(int part, int motion, int mode);
extern void func_ov107_020c5af8(int owner, int id, int mode, void *at);
extern void func_0203c634(int *node, int slot, void *cb);
extern void func_ov257_020ce784(int *node);

void func_ov257_020ce6ac(int *node)
{
    int *state = (int *)node[1];
    Vec3 dir;
    int speed;

    state[0x10] = *(int *)(node[0] + 0x2c) * 30 / 5;
    func_ov257_020ccf98(state, state[0x18], &dir, &speed);
    func_01ffa724(speed, &dir, (Vec3 *)(state + 4));
    if (*(unsigned char *)state[3] != 0) {
        return;
    }
    if (((struct Bits17a *)(*state + 0x17a))->b0 == 0) {
        return;
    }
    func_ov107_020c9264(*state, 0xc, 0);
    func_ov107_020c9ee8(*(int *)(*state + 0x3d0), 0xb, 0);
    func_ov107_020c5af8(state[0], (short)*(int *)(*state + 0x408), 3, (void *)state[1]);
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov257_020ce784);
}

 ...
 * for the +0x60 target (func_ov257_020ccf98). The +0x44 timer accumulates the frame rate; between
...
void func_ov257_020cf7d4(int *node)
{
    int obj;
    int *pHits;
    ...
    if (*((u8 *)state + 0x76) == 0 && state[0x15] >= 0x2888) {
        func_ov107_020c5af8(state[0], (short)*(int *)(*state + 0x408), 0x19, (void *)state[1]);
        *((u8 *)state + 0x76) = 1;
    }
    state[0x10] = 0;
            ...
            if (i < n) {
                ...
                do {
                    ...
                    if ((*((u8 *)state + 0x73) & (1 << *(u16 *)(obj + 2))) == 0) {
                        ...
                        if (func_ov107_020ca918(pHits[i], *state, *state, 3, &push, 0) != 0) {

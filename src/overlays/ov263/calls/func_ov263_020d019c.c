/* Effect message hook of the ov263 enemy (x3 with ov265/ov280). A "spawned" message (kind 5) picks
 * the +0x3b8 pair named by byte 3: slots 0/2 spawn at the packed position (bytes 5..) with mode 5
 * (slot 0) or 1 and weight 0x189e / 0x1119; the other slots first finish the pending slot-6 effect
...
void func_ov263_020d019c(char *actor, u8 *msg, int param)
{
    int weight;

    if (msg[2] == 5) {
        weight = FX_Inv(0x189e, 0x1119);
        switch (msg[3]) {
        case 0:
        case 2:

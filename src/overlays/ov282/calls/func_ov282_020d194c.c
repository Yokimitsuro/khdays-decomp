/* Leap tick of the ov282 enemy (x3 with ov211/ov282). Once (+0x64) the height of the +0x3d4
 * part's first node (+0x194) is handed to ov282 3ffc. The target is re-acquired into +0x10 --
 * none requests sub-state 2 and ends the action. Until the +0x66 byte marks it, the +0x60 timer
 * accumulates the owner's rate and past 0x199 fires reaction 0x16a mode 5 at the +4 point. The
...
extern void func_ov282_020d4010(int height, int a, int b);
...
void func_ov282_020d194c(int *self) {
    ...
    if (*(unsigned char *)((char *)state + 0x64) == 0) {
        func_ov282_020d4010(*(int *)(**(int **)(state[0] + 0x3d4) + 0x194), 0, 0);
    ...
    if (*(unsigned char *)((char *)state + 0x66) == 0) {
        ...
        if (state[0x18] >= 0x199) {
            *(unsigned char *)((char *)state + 0x66) = 1;
            func_ov107_020c5af8(state[0], 0x16a, 5, state[1]);
        }
    }

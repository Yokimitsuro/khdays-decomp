/*
 * func_ov271_020d1e7c -- x3. AI-state tick: clamp, aim, and transition on a timer.
...
extern void func_ov271_020d1fb0(void);

void func_ov271_020d1e7c(int *self) {
    ...
    if (state[0x14] >= 0x3fc0) {
        ...
        ps->b = data_ov271_020d36a0[4].b;
        ps->a = data_ov271_020d36a0[4].a;
        cb = *(void (**)(int, void *, int))(*state + 0x24);
        if (cb != 0) {
            cb(*state, ps, 4);
        }
        func_0203c634((int)self, *(signed char *)((int)self + 0x20), (int)&func_ov271_020d1fb0);
        return;
    }
    count = 0;
    for (i = 1; i < 3; i++) {
        if (func_ov271_020d23b4(((int *)*state)[i + 0xe4]) == 0) {
            count++;
        }
    }
    if (count <= 0) {

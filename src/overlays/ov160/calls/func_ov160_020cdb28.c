/* func_ov160_020cdb28 -- set the follow-up delay, and if the counter allows, commit the move.
 ...
 * Otherwise a 2-field descriptor is built from data_ov160_020cf794 and passed to the owner's own
 * hook (+0x24) if it has one, the delay is REPLACED with the short value (x * 30 / 10), the owner
 * is retuned (mode 7), the progress fields (+0x3c/+0x54) cleared, the rig at +0x3a0 reset, and
 * the action dispatched with func_ov160_020cdc04.
 *
...
extern void func_ov160_020cdc04(void);
extern unsigned short data_ov160_020cf794[];

void func_ov160_020cdb28(int self) {
    int *ctx;
    Ov137_Desc d;
    Ov137_Desc *p;
    ...
    p = &d;
    d.b = data_ov160_020cf794[1];
    d.a = data_ov160_020cf794[0];
    ...
    *(unsigned char *)((char *)ctx + 0x54) = 0;
    func_ov107_020c9ee8(*(int *)(ctx[0] + 0x3a0), 0, 0);
    func_0203c634(self, *(signed char *)(self + 0x20), func_ov160_020cdc04);
}

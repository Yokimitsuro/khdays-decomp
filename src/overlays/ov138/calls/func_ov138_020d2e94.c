/* Send the ov137 enemy's position message: the overlay's 14-byte template (data_ov138_020d2fe0)
...
extern const PosMsg data_ov138_020d2fe0;
...
void func_ov138_020d2e94(int *state)
{
    PosMsg msg;
    FxVec vContact;
    FxVec *pPos;

    msg = data_ov138_020d2fe0;
    pPos = (FxVec *)state[1];
    vContact.x = pPos->x;
    ...
    if (*(int *)(*state + 0x13c) - 0x800 > 0x100) {
        msg.flag = 1;
    }

/* func_ov138_020d0db4 -- fire the owner's reaction once the target is within half its reach.
 ...
 * The reaction is skipped when the gap is at or beyond the owner's reach (+0x2d8), or at or
 * beyond half of it while the counter at +0x44 is still positive; once that counter runs out
 * the reaction fires anywhere inside the reach. Firing parks the owner in state 4 (+0x1c7) and dispatches through func_0203c634 with the
...
void func_ov138_020d0db4(int self) {
    int *ctx;
    int gap;
    int owner;
    int target;

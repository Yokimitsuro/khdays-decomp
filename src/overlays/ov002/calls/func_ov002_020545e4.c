 ...
 * func_ov002_020545b8(ctx, element, arg). base/count reloaded per-iter (callees
 * may mutate *ctx). */
extern void func_ov002_02054a10(int ctx, int arg);
extern void func_ov002_020545b8(int ctx, int element, int arg);
...
void func_ov002_020545e4(int ctx_, int arg) {
    struct Ctx3 *ctx = (struct Ctx3 *)ctx_;
    int i;
    func_ov002_02054a10(ctx_, arg);
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].field_14 != 0) {
            func_ov002_020545b8(ctx_, (int)&ctx->items[i], arg);
        }
    }
}

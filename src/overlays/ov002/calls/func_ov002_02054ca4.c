/* func_ov002_02054ca4 -- copy the active touch/hit entry into the menu's cursor record, ov002.
...
void func_ov002_02054ca4(void) {
    ...
    for (i = n1; i >= 0; i--) {
        if (buf[i].w[3] == 0) {
            MI_CpuCopy8((unsigned short *)&buf[i], (unsigned short *)data_ov002_0207f610, 8);
            return;
        }
    }
    MI_CpuCopy8((unsigned short *)&buf[n1], (unsigned short *)data_ov002_0207f610, 8);
}

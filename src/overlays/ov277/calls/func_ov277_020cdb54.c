 ...
 * rider's nearest target (func_ov277_020cd300) becomes the actor's +0x390 and the orientation turns
 * towards it by 0.15 of the frame step x 30 (quaternion slerp, renormalised). Grounded or blocked
 * (+0x17a bits 0/1), the dive ends: the message data_ov277_020d3682 carries the +0x1c point
 * lowered by 0.56 to the +0x24 hook, pose 0 is requested and reaction 0x165 mode 0x10 fires there.
...
extern int func_ov277_020cd300(int rider);
...
extern const Cmd14 data_ov277_020d3682;
extern const Cmd14 data_ov277_020d3674;
...
void func_ov277_020cdb54(int *node)
{
    ...
    if (state[0xb] > 0x800) {
        if (state[5] == 0) {
            *(int *)(*state + 0x390) = func_ov277_020cd300(*(int *)(*state + 0x384));
        ...
        if (((struct Bits17a *)(*state + 0x17a))->b0 || ((struct Bits17a *)(*state + 0x17a))->b1) {
            ...
            *(u8 *)(*state + 0x1c7) = 0;
            func_ov107_020c5af8(*state, 0x165, 0x10, (void *)state[7]);
    ...
    for (i = 0; i < n; i++) {
        ...
        func_ov107_020c5af8(*state, 0x165, 0xf, (void *)state[7]);
        *(u8 *)(*state + 0x1c7) = 0;
        func_0203c634(node, *(signed char *)((char *)node + 0x20), 0);
        return;
    }
    if (state[0xb] <= 0x14000) {
        return;
    }
    *(u8 *)(*state + 0x1c7) = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), 0);

...
typedef struct WMStartScanCallback {
    ...
    u16 ssid[16];                 /* 0x16 */
    struct {
        u16 magicNumber;          /* 0x36 */
        u8 ver;                   /* 0x38 */
        u8 platform;              /* 0x39 */
        u32 ggid;                 /* 0x3c */
        u16 tgid;                 /* 0x40 */
        u8 userGameInfoLength;    /* 0x42 */
        u8 attribute;             /* 0x43 */
    } gameInfo;
} WMStartScanCallback;
...
typedef struct WhStatics {
    u16 nChannel;                 /* 0x00: the channel asked for (0 = pick from the allowed set) */
    u16 nMeasuredChannel;         /* 0x02: the channel picked after measuring */
    u16 nMpFreq;                  /* 0x04: MP polls per frame handed to WM_StartMP */
    u16 nConnectChannel;          /* 0x06: nonzero once the scan found a parent to join */
    ...
    int nSysState;                /* 0x24: WH_SYSSTATE_* */
    ...
    int bFilterByGameInfo;        /* 0x48: scan results are vetted by the game info first */
...
extern BOOL func_0200fe88(WMBssDesc *pBssDesc);   /* the game's own vetting of a found parent's game info */
extern void DC_InvalidateRange(void *addr, u32 size);

/* func_ov105_020be96c -- WH_StateInStartScan: issue the next single-channel scan: the
 * requested channel, or the next allowed one after the last scanned (wrapping at 16),
 * with a per-console dispersed channel time into the shared descriptor.
 */
BOOL func_ov105_020be96c(void)
{
    WMErrCode result;
    u16 allowedChannel;
    BOOL searching;
    ...
    if (sWh.nChannel == 0) {
        /* a loop flag rather than while (TRUE): it keeps the exit test as "found -> leave", as in the ROM */
        searching = TRUE;
        while (searching) {
            sWh.scanParam.channel++;
            if (sWh.scanParam.channel > WH_CHANNEL_MAX) {
                sWh.scanParam.channel = 1;
            }
            if (allowedChannel & (1 << (sWh.scanParam.channel - 1))) {
                break;
            }
        }
    } else {
        sWh.scanParam.channel = sWh.nChannel;
    }

    sWh.scanParam.maxChannelTime = func_ov105_020bd8e4();
    sWh.scanParam.scanBuf = &sBssDesc;

    result = func_ov105_020bdcac(func_ov105_020bea34, &sScanParam);

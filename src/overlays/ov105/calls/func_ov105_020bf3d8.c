...
/* func_ov105_020bf3d8 -- WH_StateOutMeasureChannel: record the channel's busy ratio
 * (the quietest channels are collected in the bitmap) and measure the next one; once
 * they run out the helper is MEASURECHANNEL and a channel can be picked.
 */
void func_ov105_020bf3d8(void *arg)
{
    u32 result;
    u16 channel;
    ...
    if (sWh.nChannelBusyRatio > cb->ccaBusyRatio) {
        sWh.nChannelBusyRatio = cb->ccaBusyRatio;
        sWh.nChannelBitmap = (u16)(1 << (channel - 1));
    } else if (sWh.nChannelBusyRatio == cb->ccaBusyRatio) {
        sWh.nChannelBitmap |= 1 << (channel - 1);
    }

    result = func_ov105_020bf33c(++channel);

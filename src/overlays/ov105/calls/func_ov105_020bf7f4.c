...
/* func_ov105_020bf7f4 -- WH_ChildConnect: join the parent described by `bssDesc`:
 * allocate the MP buffers (0x440 receive, 0x80 send), keep a flushed copy of the
 * descriptor and start the connection (through the WEP key step when a child key
 * generator is registered).
 */
BOOL func_ov105_020bf7f4(int mode, WMBssDesc *bssDesc)
{
    sWh_nRecvBufferSize = 0x440;
    sWh_nSendBufferSize = 0x80;

    sWh_pRecvBuffer = func_020236f8(0x440, 0x20, (void **)data_0204c024);
    ...
    case WH_CONNECTMODE_DS_CHILD:
        ...
        if (sWh_pChildWEPKeyGenerator != NULL)
            return func_ov105_020bec64();
        else
            return func_ov105_020becfc();
    default:
        break;
    }

...
/* func_ov105_020bf5a8 -- WH_Initialize: reset the helper's statics, allocate the WM
 * system buffer (0xf00 bytes, 32-byte aligned) from the game heap and bring the WM
 * library up.
 */
enum { WH_RESULT_FAILED = 0, WH_RESULT_OK = 1 };

BOOL func_ov105_020bf5a8(void)
{
    sWh_nRecvBufferSize = 0;
    sWh_nSendBufferSize = 0;

    sWh_pWmBuffer = func_020236f8(0xf00, 0x20, (void **)data_0204c024);

    sWh_pReceiver = NULL;
    sMyAid = 0;
    sConnectBitmap = 1;
    sWh_nErrCode = WM_ERRCODE_SUCCESS;
    sWh_nDisconnectReason = 0;

    sParentParam.userGameInfo = NULL;
    sParentParam.userGameInfoLength = 0;

    MI_CpuClear8(sConnectionSsid, WM_SIZE_CHILD_SSID);
    sWh_pJudgeAccept = NULL;

    return func_ov105_020bf66c() == FALSE ? WH_RESULT_FAILED : WH_RESULT_OK;
}

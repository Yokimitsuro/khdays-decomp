...
/* func_ov105_020be770 -- WH_StateInStartParentMP: start the parent's MP protocol with
 * the helper's buffers and poll frequency unless a session is already up.
 */
BOOL func_ov105_020be770(void)
{
    WMErrCode result;

    if (sWh.nSysState == WH_SYSSTATE_CONNECTED || sWh.nSysState == WH_SYSSTATE_KEYSHARING || sWh.nSysState == WH_SYSSTATE_DATASHARING)
        return TRUE;

    WH_ChangeSysState(WH_SYSSTATE_CONNECTED);
    result = func_ov105_020be0fc(func_ov105_020be7fc, (u16 *)sWh.pRecvBuffer, (u16)sWh.nRecvBufferSize, (u16 *)sWh.pSendBuffer, (u16)sWh.nSendBufferSize, sWh.nMpFreq);

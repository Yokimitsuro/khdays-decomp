...
/* func_ov105_020bebf4 -- WH_StateOutEndScan: the scan is over: back to IDLE, and when a
 * parent was picked start the child connection (through the WEP key step when a child
 * key generator is registered).
 */
void func_ov105_020bebf4(void *arg)
{
    ...
    if (sWh.bAutoConnect != 0) {
        if (sWh.pChildWEPKeyGenerator != NULL) {
            if (!func_ov105_020bec64())
                WH_ChangeSysState(WH_SYSSTATE_ERROR);
        } else {
            if (!func_ov105_020becfc())
                WH_ChangeSysState(WH_SYSSTATE_ERROR);
        }
    }
}

...
/* func_ov105_020beee0 -- WH_StateOutStartChildMP: the child's MP events: a send
 * failure, a timeout or a bad poll bitmap are ignored, other errors end the session;
 * the MP start marks the session connected.
 */
void func_ov105_020beee0(void *arg)
{
    WMStartMPCallback *cb = (WMStartMPCallback *)arg;

    if (cb->errcode != WM_ERRCODE_SUCCESS) {
        if (cb->errcode == WM_ERRCODE_SEND_FAILED || cb->errcode == WM_ERRCODE_TIMEOUT || cb->errcode == WM_ERRCODE_INVALID_POLLBITMAP) {
            return;
        }

...
/* func_ov105_020bed80 -- WH_StateOutStartChild: the child's connection events: a
 * refused entry is an error, a plain failure CONNECT_FAIL; once connected MP is
 * started and the aid recorded; a disconnect raises WH_ERRCODE_DISCONNECTED and keeps
 * the first reason.
 */
void func_ov105_020bed80(void *arg)
{
    ...
    if (cb->state == WM_STATECODE_CONNECTED) {
        ...
        WH_ChangeSysState(WH_SYSSTATE_ERROR);
        if (sWh.nDisconnectReason == 0) {
            sWh.nDisconnectReason = cb->reason;
        }
        return;

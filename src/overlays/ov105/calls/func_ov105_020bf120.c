...
/* func_ov105_020bf120 -- WH_PortReceiveCallback: hand a received data-port frame to the
 * registered receiver (aid, data, length); a station leaving is reported with no data.
 */
void func_ov105_020bf120(void *arg)
{
    WMPortRecvCallback *cb = (WMPortRecvCallback *)arg;

    if (cb->errcode != WM_ERRCODE_SUCCESS) {
        WH_SetError((WMErrCode)cb->errcode);
    } else if (sWh.pReceiver != NULL) {
        if (cb->state == WM_STATECODE_PORT_RECV) {
            (*sWh.pReceiver)(cb->aid, cb->data, cb->length);
        } else if (cb->state == WM_STATECODE_DISCONNECTED) {
            (*sWh.pReceiver)(cb->aid, NULL, 0);
        }
    }
}

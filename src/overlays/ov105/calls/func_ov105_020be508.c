...
/* func_ov105_020be508 -- WH_StateOutSetParentParam: after the parent parameter block is
 * set, install the parent WEP key when a generator was registered, else start the parent.
 */
void func_ov105_020be508(void *arg)
{
    ...
    if (sWh.pParentWEPKeyGenerator != NULL) {
        if (!func_ov105_020be56c())
            WH_ChangeSysState(WH_SYSSTATE_ERROR);
    } else {
        if (!func_ov105_020be604())
            WH_ChangeSysState(WH_SYSSTATE_ERROR);
    }
}

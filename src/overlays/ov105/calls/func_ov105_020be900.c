...
/* func_ov105_020be900 -- WH_StartScan: scan for a parent with the given MAC address
 * filter on `channel` (0 = walk the allowed channels), reporting each hit to
 * `callback`; the scan parameter's channel restarts from the first allowed one.
 */
BOOL func_ov105_020be900(WhScanCallbackFunc callback, const u16 *macAddr, u16 channel)
{
    WH_ChangeSysState(WH_SYSSTATE_SCANNING);

    sWh.pScanCallback = callback;
    sWh.nChannelIndex = channel;
    sWh.scanParam.channel = 0;
    sWh.bAutoConnect = 0;
    *(u16 *)&sWh.scanParam.bssid[4] = macAddr[2];
    *(u16 *)&sWh.scanParam.bssid[2] = macAddr[1];
    *(u16 *)&sWh.scanParam.bssid[0] = macAddr[0];

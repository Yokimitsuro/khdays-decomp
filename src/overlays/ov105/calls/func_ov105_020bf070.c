...
/* func_ov105_020bf070 -- WH_StateInSetMPData: queue `dataSize` bytes for every station
 * on the helper's data port at normal priority; `callback` is told whether it was
 * delivered.
 */
BOOL func_ov105_020bf070(void *data, u16 dataSize, WhSendCallbackFunc callback)
{
    WMErrCode result;

    DC_FlushRange(sWh.pSendBuffer, (u32)sWh.nSendBufferSize);

    result = func_ov105_020be164(func_ov105_020bf0e0, (void *)callback, data, dataSize, 0xFFFF, WH_DATA_PORT, WH_DATA_PRIO);

...
{
    return (gameInfoLength >= WM_SIZE_SYSTEM_GAMEINFO && gameInfo->magicNumber == WM_GAMEINFO_MAGIC_NUMBER);
}
...
/* func_ov105_020bea34 -- WH_StateOutStartScan: a scan step finished. Outside SCANNING
 * the scan is ended; a found parent is handed to the scan callback either after the
 * PictoChat check or after the game-info check (valid header, our ggid, entry
 * allowed), and when the callback asked to connect the scan ends so the connection
 * can start; otherwise the next channel is scanned.
 */
void func_ov105_020bea34(void *arg)
{
    ...
    case WM_STATECODE_PARENT_FOUND:
        ...
        if (sWh.bPictoCatch) {
            if (func_0200fe88(&sBssDesc)) {
                if (sWh.pScanCallback != NULL)
                    sWh.pScanCallback(&sBssDesc);
                break;
            }
        }

        if (WM_IsValidGameInfo(&cb->gameInfo, cb->gameInfoLength) == FALSE) {
            break;
        }
        if (cb->gameInfo.ggid != sParentParam.ggid) {
            break;
        }

        if ((cb->gameInfo.attribute & (WM_ATTR_FLAG_ENTRY | WM_ATTR_FLAG_MB)) != WM_ATTR_FLAG_ENTRY) {
            /* an empty two-term test: it compiles to nothing, but it changes how mwcc lays the
             * checks out (as the SDK's compiled-out WH_TRACE lines do) and keeps them as the ROM's
             * separate branches instead of one predicated chain */
            if (!cb->gameInfoLength && !cb->gameInfoLength) {
            }
            break;
        }

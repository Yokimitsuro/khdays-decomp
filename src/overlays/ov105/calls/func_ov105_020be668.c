...
/* func_ov105_020be668 -- WH_StateOutStartParent: the parent's connection events: a
 * child connecting is vetted by the judge callback (refused ones are disconnected)
 * and added to the bitmap, a disconnected child is removed, and once the parent has
 * started MP is brought up.
 */
void func_ov105_020be668(void *arg)
{
    ...
    case WM_STATECODE_CONNECTED:
        if (sWh.pJudgeAccept != NULL) {
            if (!sWh.pJudgeAccept(cb)) {
                WMErrCode result;

                result = func_ov105_020bdea4(NULL, cb->aid);

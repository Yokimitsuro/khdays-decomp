...
/* func_ov105_020bf928 -- WH_Finalize: tear the session down from whatever state the
 * helper is in: a scan is ended, a connected parent or child stops MP (which resets
 * the helper on completion), anything else resets right away; IDLE needs nothing.
 */
void func_ov105_020bf928(void)
{
    if (sWh.nSysState == WH_SYSSTATE_IDLE) {
        return;
    }

    if (sWh.nSysState == WH_SYSSTATE_SCANNING) {
        if (!func_ov105_020beb98())
            func_ov105_020bf90c();
        return;
    }

    if ((sWh.nSysState != WH_SYSSTATE_KEYSHARING) && (sWh.nSysState != WH_SYSSTATE_DATASHARING) && (sWh.nSysState != WH_SYSSTATE_CONNECTED)) {
    ...
    switch (sWh.nConnectMode) {
    case WH_CONNECTMODE_MP_CHILD:
        if (!func_ov105_020bef44())
            func_ov105_020bf90c();
        break;

    case WH_CONNECTMODE_MP_PARENT:
        if (!func_ov105_020be850())
            func_ov105_020bf90c();
        break;
    }
}

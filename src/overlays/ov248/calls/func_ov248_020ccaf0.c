/* Hit handler of the ov248 enemy: ignored while the +0x21a stamina is spent.
 ...
 * the damage is resolved, and in sub-state 7 doubled and, for an 8-flagged hit, multiplied by 10/3
 * (kind 0x80) and by 5 (kinds 0x800 and 0x4). A 0x4000-flagged hit sets result 1 unless the actor
 * is guarded (+0x1c4 bit 1). The source and the +0x68 flag-4 bit are recorded; a frontal, unguarded
 * hit in sub-states 2/4 (with flag 0x20) or 5 fires reaction 0x146 mode 5, sets result 1 and, for
 * 2/4, requests sub-state 5. In sub-states 8/9 a kind-0x80 throw of weight 2 (+0x58 = 0x6000) or
 ...
 * 0x8000 hit that is not the 8|0x80/0x80 special fires reaction 0x146 with the mode taken from the
...
extern const struct ModeTable data_ov248_020d0bec;

#define ANG2IDX(a) ((unsigned short)(((long long)(a) * 0x28be60db9391LL + 0x80000000000LL) >> 44) >> 4)

int func_ov248_020ccaf0(char *actor, int other, struct ActorHitEvent *hit)
{
    ...
    if (*(signed char *)(*state + 0x1c6) == 7) {
        hit->nDamage <<= 1;
        if ((hit->uFlagsLo & 8) != 0) {
            if ((hit->uFlagsHi & 0x80) != 0) {
                hit->nDamage = hit->nDamage * 100 / 30;
            }
            if ((hit->uFlagsHi & 0x800) != 0) {
                hit->nDamage = hit->nDamage * 50 / 10;
            }
            if ((hit->uFlagsHi & 0x4) != 0) {
                hit->nDamage = hit->nDamage * 50 / 10;
            }
        }
    }
    ...
    case 4:
        if ((hit->uFlagsLo & 0x20) != 0 && !guarded && frontal) {
            func_ov107_020c5af8(actor, 0x146, 5, (void *)state[3]);
            hit->uResultLo |= 1;
            *(u8 *)(*state + 0x1c7) = 5;
            return 1;
        }
    ...
    case 8:
    case 9:
        if ((hit->uFlagsLo & 8) != 0 && (hit->uFlagsHi & 0x80) != 0 && hit->pad014[9] == 2) {
            state[0x16] = 0x6000;
            *(u8 *)(*state + 0x1c7) = 7;
    ...
    if ((hit->uFlagsLo & 0x8000) != 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                pair22 = data_ov248_020d0bec.pair22;
                func_ov107_020c5af8(actor, 0x146, pair22.a[*(u8 *)((char *)state + 0x63)], (void *)state[3]);
            } else {
                pair = data_ov248_020d0bec.pair;
                func_ov107_020c5af8(actor, 0x146, pair.a[*(u8 *)((char *)state + 0x63)], (void *)state[3]);
            }

/* Slam tick of the ov248 enemy: the +0x4c timer accumulates the frame-time, the
 * +0x10 velocity is the +0x490 item's +0x2c vector turned by the +0x40 yaw and, past 0xff0,
 * reaction 0x146 mode 0xf fires once (+0x61) at the +0xc position. Between 0x2a8 and 0x440 the
...
void func_ov248_020ce76c(int *node)
{
    ...
    if (*(u8 *)((char *)state + 0x61) == 0 && state[0x13] >= 0xff0) {
        func_ov107_020c5af8(*state, 0x146, 0xf, (void *)state[3]);

/* func_ov248_020cf9c4 -- the slam tick: the impulse at ctx+0x1c is copied out to
 ...
 * with kind 2 (func_ov248_020cd098). Once the owner lands (+0x17a bit 0) the owner's +0x180 point
 * is kept at ctx+0x34 with its height lowered to the owner's +0x80 floor less 0x200, effect 6
 * spawns there, animation 0x19 plays, reaction 0x146 mode 0xc fires at it, the timer at +0x4c
 * restarts and the tick hands over to func_ov248_020cfb4c. */
...
extern void func_ov248_020cd098(int *ctx, int kind, Sphere *sphere, void *box);
...
extern void func_ov248_020cfb4c(void);

void func_ov248_020cf9c4(int self) {
    int *ctx;
    Sphere sphere;
    int step;
    ...
    *(VecFx32 *)((char *)ctx + 0x10) = *(VecFx32 *)((char *)ctx + 0x1c);
    ...
    func_ov248_020cd098(ctx, 2, &sphere, 0);
    ...
    *(VecFx32 *)((char *)ctx + 0x34) = *(VecFx32 *)(ctx[0] + 0x180);
    ...
    func_ov107_020c5af8(ctx[0], 0x146, 0xc, (char *)ctx + 0x34);
    ctx[0x13] = 0;
    func_0203c634(self, *(signed char *)(self + 0x20), func_ov248_020cfb4c);
}

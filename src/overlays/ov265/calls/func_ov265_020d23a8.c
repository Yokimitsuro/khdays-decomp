/* Lunge tick of the ov265 enemy (variant of ov231/ov232's). The +0x30
 ...
 * point. Between 0x1298 and 0x2b90 the target is re-acquired (func_ov265_020d0628) and a probe
 * segment from the +0xc point lifted 3.85 along data_02042240 (length 2.0, radius 3.61) hits kind 2
 * candidates towards the heading lifted 2.0 (func_ov265_020d0890); on a hit reaction 0/0x51 fires at
...
extern int func_ov265_020d0890(int *self, int kind, void *query, void *pt, int flags);
...
void func_ov265_020d23a8(int *node)
{
    ...
    *(Vec3 *)(state + 0xc) = v;
    ...
    if (state[0xa] >= 0x1298 && state[0xa] < 0x2b90) {
        ...
        seg.nLength = 0x2000;
        seg.nRadius = 0x39b3;
        if (func_ov265_020d0890(node, 0, &seg, &dir, 2) != 0) {

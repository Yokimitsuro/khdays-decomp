/* Ov265_ProbeGround -- cast from the owner's position along `dir` and return the Y of the scaled
 * hit direction (0 when nothing is hit or the world has no collision). With bRay a single ray
 * is cast; otherwise up to four spheres are tried from the owner's position nudged by +-0x1ec6
...
extern void  func_ov265_020d06bc(VecFx32 *pIn, int *node, VecFx32 *pOut);
extern void  VEC_Add(VecFx32 *a, VecFx32 *b, VecFx32 *out);

int func_ov265_020d0750(int *node, VecFx32 *dir, int bRay)
{
    void *hit;
    ...
    if (*(void **)(world + 0x7c) != 0) {
        if (bRay) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                off.y += i == 0 ? 0x1ec6 : (i == 1 ? -0x1ec6 : 0);
                off.x += i == 2 ? 0x1ec6 : (i == 3 ? -0x1ec6 : 0);
                func_ov265_020d06bc(&off, node, &off);
                VEC_Add(&pos, &off, &pos);

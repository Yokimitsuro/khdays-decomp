...
extern int func_ov179_020d2554(int node, Vec3 *out);
...
void func_ov179_020d3628(int node)
{
    ...
    dist = func_ov179_020d2554(node, &dir);
    if (dist > 0x100) {
        dist = 0x200;
    }
    ...
    state[0x17] = 0;
    if (roll < 0x28 && func_ov179_020d25ec(node) != 0) {
        *(unsigned char *)(*state + 0x1c7) = 9;
    } else if (roll < 0x3c && func_ov179_020d25ec(node) != 0) {

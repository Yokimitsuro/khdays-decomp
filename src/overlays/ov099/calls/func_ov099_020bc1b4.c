...
typedef int (*TimelineActorModeCallback)(struct TimelineActor *actor, int mode);
typedef int (*TimelineActorTickCallback)(struct TimelineActor *actor);

struct TimelineActorBits694 {
    u8 bit0 : 1;
    u8 callbackActive : 1;
};
...
struct TimelineActor {
    ...
    u16 flags18;
    char pad01a[6];
    struct TimelineActorNode *node20;
    u32 flags24;
    char pad028[0x30];
    int verticalDelta58;
    char pad05c[0x408];
...
extern int func_ov022_020ad114(struct TimelineActor *actor);
...
extern void func_ov022_0209fe20(struct TimelineActor *actor, struct Vec3 *out);
extern void VEC_Add(const struct Vec3 *a, const struct Vec3 *b,
                    struct Vec3 *out);
extern void func_ov099_020bba38(struct TimelineActorOwner *self);
extern int func_ov022_02095554(void *context);
extern int func_ov022_02095524(void *context);
extern void func_ov022_020acf14(struct TimelineActor *actor, int mode);
extern void func_0202af1c(u16 *nodeFlags);
extern void *func_ov022_020a35f4(struct TimelineActor *actor, int mode);

void *func_ov099_020bc1b4(struct TimelineActorOwner *self)
{
    void *result = 0;
    ...
    if (actor->timeline7b0 % 0x6000 == 0 && angle != -1) {
        struct TimelineActorNode *node = actor->node20;

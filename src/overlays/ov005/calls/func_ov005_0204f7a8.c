...
extern int  func_ov005_0204eda0(int out);
extern void MI_CpuFill8(void *dst, int value, unsigned int size);
extern int  func_ov005_0204eddc(Ov000TallyEntry *groups);
extern void func_ov005_0204f078(Ov000PanelView *self, int id, unsigned int qtyA,
                                unsigned int qtyB, int flag);

int func_ov005_0204f7a8(Ov000PanelView *self, Ov000TallyEntry *groups, int panelIndex,
                        int *flagsOut) {
    int off;
    int j;
    ...
    k2 = 0;
    do {
        (*self->openEntry)(&entry, func_ov005_0204eda0((int)&kind), index, 0xe);
        if (entry != 0) {
    ...
    if (kind == 0 || kind == 0x13) {
        self->comboFlag = func_ov005_0204eddc(groups);
    ...
    if (found->items[0].param != 0) {
        ...
        func_ov005_0204f078(self, found->items[0].param, found->items[0].paramB, 0, 0);
        (*self->closeEntry)(&aux);
    }
            ...
            for (k2 = 1; k2 < 10; k2++, gw++, cw++) {
                if (gw->id >= 0 && *cw < gw->count && item->id == gw->id) {
                    if (item->param != 0) {
                        ...
                        func_ov005_0204f078(self, item->param, item->paramB, 0, 1);
                        (*self->closeEntry)(&aux);
                    }

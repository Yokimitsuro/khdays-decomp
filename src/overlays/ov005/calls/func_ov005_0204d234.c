 ...
 * func_ov005_0204d208(ctx, element, arg). base/count reloaded per-iter (callees
 * may mutate *ctx). */
extern void func_ov005_0204d4fc(int ctx, int arg);
extern void func_ov005_0204d208(int ctx, int element, int arg);
...
void func_ov005_0204d234(int ctx_, int arg) {
    struct Ctx3 *ctx = (struct Ctx3 *)ctx_;
    int i;
    func_ov005_0204d4fc(ctx_, arg);
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].field_14 != 0) {
            func_ov005_0204d208(ctx_, (int)&ctx->items[i], arg);
        }
    }
}

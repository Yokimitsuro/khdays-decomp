 ...
 * word is nonzero, notify func_ov005_0204d3dc(ctx, element). base/count reloaded
 * each iteration because the callee may mutate *ctx. */
extern void func_ov005_0204d3dc(int ctx, int element);
...
void func_ov005_0204d408(int ctx_) {
    struct Ctx2 *ctx = (struct Ctx2 *)ctx_;
    int i;
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].field_c != 0) {
            func_ov005_0204d3dc(ctx_, (int)&ctx->items[i]);
        }
    }
}

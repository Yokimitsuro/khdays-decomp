/* func_ov005_02050188 -- fold one panel into the running tally.
 *
 ...
 * func_ov005_0204eedc, type 1 pushes each of the four non-empty value slots through
 * func_ov005_0204ef70, and types 3, 5 and 6 push them through func_ov005_0204f078 and then
...
extern void func_ov005_0204eedc(Ov000PanelView *self, int payload, int enabled, int reserved);
extern void func_ov005_0204ef70(Ov000PanelView *self, int value, int reserved0, int enabled,
                                int reserved1);
extern void func_ov005_0204f078(Ov000PanelView *self, int value, unsigned int tallyA,
                                unsigned int tallyB, int reserved);

void func_ov005_02050188(Ov000PanelView *self, Ov000GridCell **grid, u64 covered, u64 *out) {
    char *slotRow;
    int row;
    int col;
    int base;
        ...
        for (col = 0; col < 5; col++) {
            ...
            if (cell != 0 && (covered & (1ULL << (col + base))) == 0) {
                *out |= 1ULL << (col + base);
                switch (cell->type) {
                case 0:
                    func_ov005_0204eedc(self, cell->param, 1, 0);
                    break;
                case 1:
                    for (i = 0; i < 4; i++) {
                        slotRow = (char *)cell + i * sizeof(Ov000TallySlot);
                        if (*(int volatile *)(slotRow + 0x34) != 0) {
                            func_ov005_0204ef70(self, *(int *)(slotRow + 0x34), 0, 1, 0);
                        }
                    }
                    break;
                case 3:
                ...
                case 6:
                    for (i = 0; i < 4; i++) {
                        slotRow = (char *)cell + i * sizeof(Ov000TallySlot);
                        if (*(int volatile *)(slotRow + 0x34) != 0) {
                            func_ov005_0204f078(self, *(int *)(slotRow + 0x34),
                                                *(u16 *)(slotRow + 0x38),
                                                *(u16 *)(slotRow + 0x3a), 0);
                        }
                    }

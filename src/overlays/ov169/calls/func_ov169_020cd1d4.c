/* Orbit tick of the ov169 enemy (and its byte-identical twins), variant of the matched ov178 sibling:
...
extern int func_ov169_020cca34(int node, Vec3 *out);
...
void func_ov169_020cd1d4(int node)
{
    ...
    if (dist > *(int *)(*state + 0x2d8)) {
        *(Vec3 *)(state + 8) = data_02041dc8;
        ...
        if (state[0x22] == 0) {
            h = *(int *)(state[3] + 0x78) + 0x2800;
            if (state[0x11] > 0x2800) {
                h -= state[0x11] - 0x2800;
            }
        } else {
            h = state[6];

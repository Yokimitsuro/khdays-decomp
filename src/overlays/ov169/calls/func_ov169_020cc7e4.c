/*
 * Hit handler of the ov169 enemy (and its byte-identical twins): copies the hit point into +0x2c and the
 ...
 * +0x85 counter and fires reaction 0x13f at the +8 position with the counter-alternated mode
...
extern const u8 data_ov169_020cee54[];

int func_ov169_020cc7e4(char *actor, int nParam, struct ActorHitEvent *hit)
{
    struct Ov169ActionState *state = *(struct Ov169ActionState **)(actor + 0x214);
    int delta;
    int rem;
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                uSlot.sPair = *(const struct ImpactPair *)&data_ov169_020cee54[0];
                state->nToggle85++;
                func_ov107_020c5af8(actor, 0x13f, uSlot.aModes[state->nToggle85 & 1], state->pPos);
            } else {
                union ImpactSlot uSlot;

                uSlot.sPair = *(const struct ImpactPair *)&data_ov169_020cee54[2];
                state->nToggle85++;
                func_ov107_020c5af8(actor, 0x13f, uSlot.aModes[state->nToggle85 & 1], state->pPos);
            }
        }

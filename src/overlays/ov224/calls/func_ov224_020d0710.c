/* Hit handler of the ov224 enemy. The hit's source is kept at +0x10 of the work; a locked
 ...
 * +0x218 floor. A damaging hit that is not the 8/0x80/kind-0x80 kind fires reaction 0x14a with the
...
struct Ov224ReactionModes {
    struct Ov224ModePair idle;
    struct Ov224ModePair hurt;
};

struct Ov224HitFlags {
    unsigned int low : 16;
...
struct Ov224ReactionWork {
    struct Ov224ReactionObject *object00;
    char pad004[4];
...
extern int func_ov107_020c89e8(struct Ov224ActorHitState *self, struct Ov224Hit *hit);
extern void func_ov107_020c5af8(struct Ov224ActorHitState *self, int reactionId, u8 mode, int context);
extern int func_ov002_0207298c(void);

int func_ov224_020d0710(struct Ov224ActorHitState *self, int source, struct Ov224Hit *hit)
{
    ...
    flags = (struct Ov224HitFlags *)hit;
    ...
    if (hit->damage28 > 0) {
        if ((flags->low & 8) == 0 || (flags->low & 0x80) == 0 || flags->kind != 0x80) {
            if ((flags->low & 0x22) != 0) {
                union Ov224ModeSlot uSlot;

                uSlot.sPair = data_ov224_020d4e00.hurt;
                func_ov107_020c5af8(self, 0x14a, uSlot.aModes[work->side77], work->reactionContext08);
            } else {
                union Ov224ModeSlot uSlot;

                uSlot.sPair = data_ov224_020d4e00.idle;
                func_ov107_020c5af8(self, 0x14a, uSlot.aModes[work->side77], work->reactionContext08);
            }
            work->side77 ^= 1;
        }

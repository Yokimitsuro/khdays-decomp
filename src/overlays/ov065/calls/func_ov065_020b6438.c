...
typedef struct Ov065RadialBurstFrame {
    MtxFx33 mRotation;
    VecFx32 vSpawnPosition;
    VecFx32 vRadialOffset;
    VecFx32 vOrigin;
} Ov065RadialBurstFrame;

typedef struct Ov065HeadingSource {
    u8 pad00[0x80];
    u16 nHeading;
} Ov065HeadingSource;

typedef struct Ov065EffectPart {
    u8 pad00[2];
    s8 nState;
    u8 pad03;
    int nLifetime;
...
typedef struct Ov065SceneFlags {
    u8 bSlotSpawnEnabled : 1;
    u8 rest : 7;
} Ov065SceneFlags;

typedef struct Ov065SceneState {
    u32 nFlags;
    u8 pad04[0x1c];
    Ov065HeadingSource *pHeadingSource;
    u8 pad24[0x456];
    u8 nPendingMode;
    u8 nPendingIndex;
    u8 pad47c[0x218];
    Ov065SceneFlags slotSpawnFlags;
    u8 pad695[0x25eb];
    u8 aSlotPool[4];
} Ov065SceneState;

extern Ov065SceneState *data_ov065_020b7340;
...
extern void func_ov065_020b6bbc(int slotPool, VecFx32 *position);
extern int func_02030788(void);
extern void func_02033d0c(int slotId, int variant, VecFx32 *position,
                          int flags);

int func_ov065_020b6438(void *pContext, Ov065EffectPart *pPart, int nDelta)
{
    const short *pSinCosTable;
    MtxFx33 *pRotation;
    Ov065RadialBurstFrame f;
    int nSpawnVariant;
    int nAngle;
    Ov065SceneState *pScene;

    pScene = data_ov065_020b7340;
    pPart->nState = 4;
    ...
    if (pPart->nPhase == 2) {
        ...
        do {
            ...
            func_ov065_020b6bbc((int)pScene->aSlotPool, &f.vSpawnPosition);

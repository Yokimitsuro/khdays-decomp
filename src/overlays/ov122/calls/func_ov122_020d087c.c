...
extern void func_ov122_020d0944(void);

void func_ov122_020d087c(int *node) {
    Vec3 delta;
    int *frame = (int *)node[0];
    int *state = (int *)node[1];
    int timer = state[0x10] + frame[0xb];
    int target;
    int angle;
    ...
    state[0x2] = target;
    if (target != 0) {
        VEC_Subtract((Vec3 *)(target + 0x74), (Vec3 *)state[0x3], &delta);
    ...
    func_0203c634(node, *(signed char *)(node + 8), func_ov122_020d0944);
}

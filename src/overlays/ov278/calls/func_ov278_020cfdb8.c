...
extern void func_ov278_020cfef8(void);

void func_ov278_020cfdb8(int *node) {
    ...
    if (*((unsigned char *)state + 0x51) == 0) {
        ...
        if (state[0xa] >= 0x2aaa) {
            *((unsigned char *)state + 0x51) = 1;
            func_ov107_020c5af8(*state, 0x166, 8, (void *)state[0xe]);
        }
    }
    ...
    func_0203c634(node, *(signed char *)((char *)node + 0x20), func_ov278_020cfef8);
}

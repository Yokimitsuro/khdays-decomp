/* Hit filter of the ov278 rider pair (+0x1d0), active while its +0x21a stock is positive and the
 * actor is not in move 8. With the +0x3c0 guard raised the hit only zeroes its damage and raises
 * bit 0 of its result word. Otherwise the first rider (+0x3b0 part, +0x3bc stock) that is still up
 * and was struck takes the full damage (020c89e8, floored at 0); when it falls, the short note
 * data_ov278_020d639c (rider 2 / 3) goes to the actor's +0x24 hook, the 14-byte message
 * data_ov278_020d63c6 (3 when the other rider is down too, else 2) carries the attacker's point to
 * the +0x394 object's hook, reaction 0x166 mode 0xe fires there and the part's bit 1 is raised. The
 * stock becomes the riders' sum (clamped to +0x218) and at 0 the actor requests move 0xc. A damaging
 * hit that is not the 8|0x80/0x80 special flips the +0x75 parity and fires reaction 0x166 with mode
...
struct Ov236 {
    char pad000[0x3b0];
    int riders[2];          /* +0x3b0 */
    char pad3b8[0x3bc - 0x3b8];
    short riderStock[2];    /* +0x3bc */
};
...
extern const Note4 data_ov278_020d639c;
extern const Cmd14 data_ov278_020d63c6;

int func_ov278_020d1848(char *self, int src, unsigned int *hit)
{
    long i;
        ...
        for (i = 0; i < 2; i++) {
            if (RIDERS(state)->riderStock[i] > 0 && hit[6] == (unsigned int)RIDERS(state)->riders[i]) {
                ...
                if (RIDERS(state)->riderStock[i] == 0) {
                    ...
                    func_ov107_020c5af8(self, 0x166, 0xe, (void *)(*(int *)hit[6] + 4));
        ...
        if ((int)hit[10] > 0) {
            if (!((((struct HitWord *)hit)->lo & 8) != 0 && (((struct HitWord *)hit)->lo & 0x80) != 0 && ((struct HitWord *)hit)->hi == 0x80)) {
                if ((((struct HitWord *)hit)->lo & 0x22) != 0) {
                    ((struct Parity *)((u8 *)state + 0x75))->b0++;
                    func_ov107_020c5af8(self, 0x166, (((struct Parity *)((u8 *)state + 0x75))->b0 & 1) ? 2 : 3, (void *)(*(int *)hit[6] + 4));
                } else {
                    ((struct Parity *)((u8 *)state + 0x75))->b0++;
                    func_ov107_020c5af8(self, 0x166, (((struct Parity *)((u8 *)state + 0x75))->b0 & 1) ? 0 : 1, (void *)(*(int *)hit[6] + 4));
                }
            }
        }
    } else {

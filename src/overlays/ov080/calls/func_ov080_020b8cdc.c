 ...
 * nothing. */
extern void func_ov080_020b8f8c(char *self, char *slot, void *arg);
extern void func_ov080_020b8dc4(char *self, char *parts, int angle, int spread);

void func_ov080_020b8cdc(char *self, char *parts, void *arg)
{
    int progress;
    int hit;
    int spread;
    ...
    slot = parts + 0x330;
    for (i = 0; i < 3; i++) {
        func_ov080_020b8f8c(self, slot, arg);
        slot += 0x110;
    }
    ...
    if (hit) {
        func_ov080_020b8dc4(self, parts, angle, spread);
    }
}

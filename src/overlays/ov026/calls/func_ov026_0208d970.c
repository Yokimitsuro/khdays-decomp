/* func_ov026_0208d970 -- Ov008_BuildSellList: collect the sell records
 ...
 * table address r5.
 *
 * PROVENANCE: byte-identical twin of func_ov008_0208aac0 (ov008), propagated with this
...
extern int   func_ov026_0208d498(int nItemId);                          /* item is sellable */
extern void  NNSi_FndFreeFromDefaultHeap(void *pBlock);
extern void  func_ov026_0208d360(Ov008ParamRecord **apRecords, int nCount); /* Ov008_SortParamRecordsByItemOrder */

Ov008ParamRecord **func_ov026_0208d970(int nCategory, int *pCount)
{
    u32 nListed;
    u32 i;
    int nOut;
    ...
    for (i = 0; i < data_ov026_0209136c->nSellCount; i++) {
        if (aOwned[i] != 0 && func_ov026_0208d498(data_ov026_0209136c->pSellRecords[i].pItemDef->nItemId) != 0
            && (nCategory == CATEGORY_ANY || nCategory == data_ov026_0209136c->pSellRecords[i].pItemDef->nCategory)) {
            nListed++;
        }
    }
    *pCount = nListed;
    ...
    for (i = 0; i < data_ov026_0209136c->nSellCount; i++) {
        if (aOwned[i] != 0 && func_ov026_0208d498(data_ov026_0209136c->pSellRecords[i].pItemDef->nItemId) != 0
            && (nCategory == CATEGORY_ANY || nCategory == data_ov026_0209136c->pSellRecords[i].pItemDef->nCategory)) {
            data_ov026_0209136c->apSorted[nOut] = &data_ov026_0209136c->pSellRecords[i];
            nOut++;
        }
    }
    NNSi_FndFreeFromDefaultHeap(aOwned);
    func_ov026_0208d360(data_ov026_0209136c->apSorted, nListed);

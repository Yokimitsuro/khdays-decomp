 ...
 * word is nonzero, notify func_ov026_020834d8(ctx, element). base/count reloaded
 * each iteration because the callee may mutate *ctx. */
extern void func_ov026_020834d8(int ctx, int element);
...
void func_ov026_02083504(int ctx_) {
    struct Ctx2 *ctx = (struct Ctx2 *)ctx_;
    int i;
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].field_c != 0) {
            func_ov026_020834d8(ctx_, (int)&ctx->items[i]);
        }
    }
}

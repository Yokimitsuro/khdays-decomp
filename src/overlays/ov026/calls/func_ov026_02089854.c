/* func_ov026_02089854 -- Ov008_DrawTradePanel: redraw the shop's trade
 ...
 *
 * PROVENANCE: byte-identical twin of func_ov008_020869a4 (ov008), propagated with this
...
extern void  func_ov026_02086af4(Ov008ParamRecord *pRecord, int nCount); /* Ov008_DrawCounterPanel */
...
extern char  func_ov026_0208dff8(int nItemId);                           /* Ov008_CountUnassignedCopies */
...
extern void  func_ov026_02085ca0(void *pSurface, int nValue, int nX, int nY, int nColour); /* Ov008_DrawNumber3Shadowed */

void func_ov026_02089854(void)
{
    ...
    if (pDetail->bReady == 0) {
        func_ov026_020835c4(ctx->tagTracker, func_ov026_02083530(ctx->tagTracker, TAG_TRADE));
        pDetail->bReady = 1;
    }
    if (pView->bListReady != 0) {
        if (pView->apRows == 0) {
            ...
            nOwned = pState->aItemCount[func_ov026_0208df6c(pDetail->pRecord)];
            pRecord = pDetail->pRecord;
            nTotal = nOwned + pRecord->nBonus;
            if (pRecord->nPrice <= pState->nPoints) {
                if (pDetail->pRecord->nBonus + data_0204be18->aItemCount[func_ov026_0208df6c(pRecord)] > (u32)nCap) {
                ...
                func_ov026_02086af4(pDetail->pRecord, pDetail->nQuantity);
            } else {
                if (nOwned < nCap) {
                    nOwned = -1;
                }
                pColumns->nLeftSelected = 0;
                pColumns->nLeftBase = 0;
                func_ov026_02086af4(pDetail->pRecord, nOwned);
                func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, WIDGET_CONFIRM), 0);
            }
        }
        pView->bListReady = 0;
    ...
    for (i = 0; i < INGREDIENTS; i++) {
        ...
        if (pView->apRows != 0 && pIngredient != 0) {
            if (pIngredient->nIconId != 0) {
                ...
                func_02032710(hSlots, pDetail->aRowCell[INGREDIENTS + i], data_0204be18->aItemCount[pIngredient->nItemId] != func_ov026_0208dff8(pIngredient->nItemId));
            }
            nOwned = func_ov026_0208dff8(pIngredient->nItemId);   /* the ingredient's unassigned copies */
            nStyle = (u32)nOwned >= pDetail->pRecord->aIngredient[i].nNeeded ? STYLE_ON : STYLE_OFF;
            nY = (i + 1) * ROW_HEIGHT;
            func_ov026_02085bf4(pSurface, pIngredient->pName, 0x1a, nY + 0xb, nStyle, 8);
            func_ov026_02085bf4(pSurface, data_ov026_02091368->apVarText[4], 0x82, nY + 0xb, nStyle, 8);
            func_ov026_02085c48(pSurface, pDetail->pRecord->aIngredient[i].nNeeded, 0x9a, nY + 0xb, nStyle);
            func_ov026_02085bf4(pSurface, data_ov026_02091338, 0x9e, nY + 0xb, STYLE_LABEL, 8);
            func_ov026_02085ca0(pSurface, nOwned, 0xa2, nY + 0xb, STYLE_LABEL);
            func_ov026_02085bf4(pSurface, data_ov026_0209133c, 0xb2, nY + 0xb, STYLE_LABEL, 8);
        }
    }
}

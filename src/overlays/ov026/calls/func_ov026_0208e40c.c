/* func_ov026_0208e40c -- Ov008_FindNewlyEarnedReward: scan the two reward
...
Ov008RewardRecord *func_ov026_0208e40c(void)
{
    u16 nRank;
    int i;
    ...
    do {
        ...
        for (j = 0; j < data_ov026_0209136c->aCount[nList]; j++) {
            if (func_02023588(nFlagBase + j) == 0
                && (data_ov026_0209136c->apRecords[nList][j].pItemDef == 0
                    || func_ov026_0208d40c(data_ov026_0209136c->apRecords[nList][j].pItemDef->nItemId) != 0)) {
                pRecord = &data_ov026_0209136c->apRecords[nList][j];

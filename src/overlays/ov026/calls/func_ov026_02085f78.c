 ...
 * and the slot is satisfied when the object's current count (func_ov026_0208dff8 of the
...
extern int func_ov026_0208dff8(int obj);
...
int func_ov026_02085f78(struct Ov008ReqHolder *self) {
    int i;
    for (i = 0; i < 4 && self->slots[i].pObj != 0; i++) {
        if (func_ov026_0208dff8(*(int *)(self->slots[i].pObj + 0x14)) < self->slots[i].nMin) {
            return 0;
        }
    }
    return 1;

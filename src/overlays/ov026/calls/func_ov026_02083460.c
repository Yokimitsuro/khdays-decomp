 ...
 * +0x24 bit 1 is set, notify func_ov026_02083430(ctx, element). base/count are
 * reloaded each iteration because the callee may mutate *ctx. */
extern void func_ov026_02083430(int ctx, int element);
...
void func_ov026_02083460(int ctx_) {
    struct Ctx *ctx = (struct Ctx *)ctx_;
    int i;
    for (i = 0; i < ctx->count; i++) {
        if (ctx->items[i].b1 == 1) {
            func_ov026_02083430(ctx_, (int)&ctx->items[i]);
        }
    }
}

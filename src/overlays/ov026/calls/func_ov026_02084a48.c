 ...
 * smallest rec[3]; if one is found, build a node via func_ov026_02084588 and append it to
 * the list at param_1+0x18. Returns whether a record was appended. */
extern int *func_ov026_02084588(unsigned short *rec, short *sub);
extern void NNS_FndAppendListObject(void *list, int *obj);
int func_ov026_02084a48(int param_1, unsigned short *param_2, unsigned int param_3, unsigned int param_4) {
    int i;
    int found = 0;
    short *bestSub = 0;
    ...
    if (best != 0) {
        int *node = func_ov026_02084588(best, bestSub);

/* Hit handler of the ov230 enemy: ignored while the +0x21a stamina is spent.
 ...
 * the damage is resolved, and in sub-state 7 doubled and, for an 8-flagged hit, multiplied by 5
 * (kinds 0x80 and 8) and by 10/3 (kind 0x4). A 0x4000-flagged hit sets result 1 unless the actor
 * is guarded (+0x1c4 bit 1). The source and the +0x68 flag-4 bit are recorded; a frontal, unguarded
 * hit in sub-states 2/4 (with flag 0x20) or 5 fires reaction 0x147 mode 5, sets result 1 and, for
 * 2/4, requests sub-state 5. In sub-states 8/9 a kind-0x4 throw of weight 2 (+0x58 = 0x6000) or
 ...
 * 0x8000 hit that is not the 8|0x80/0x80 special fires reaction 0x147 with the mode taken from the
...
extern const struct ModeTable data_ov230_020d644c;

#define ANG2IDX(a) ((unsigned short)(((long long)(a) * 0x28be60db9391LL + 0x80000000000LL) >> 44) >> 4)

int func_ov230_020d2550(char *actor, int other, struct ActorHitEvent *hit)
{
    ...
    case 4:
        if ((hit->uFlagsLo & 0x20) != 0 && !guarded && frontal) {
            func_ov107_020c5af8(actor, 0x147, 5, (void *)state[3]);
            hit->uResultLo |= 1;
            *(u8 *)(*state + 0x1c7) = 5;
            return 1;
        }
    ...
    case 8:
    case 9:
        if ((hit->uFlagsLo & 8) != 0 && (hit->uFlagsHi & 0x4) != 0 && hit->pad014[9] == 2) {
            state[0x16] = 0x6000;
            *(u8 *)(*state + 0x1c7) = 7;
    ...
    if ((hit->uFlagsLo & 0x8000) != 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                pair22 = data_ov230_020d644c.pair22;
                func_ov107_020c5af8(actor, 0x147, pair22.a[*(u8 *)((char *)state + 0x63)], (void *)state[3]);
            } else {
                pair = data_ov230_020d644c.pair;
                func_ov107_020c5af8(actor, 0x147, pair.a[*(u8 *)((char *)state + 0x63)], (void *)state[3]);
            }

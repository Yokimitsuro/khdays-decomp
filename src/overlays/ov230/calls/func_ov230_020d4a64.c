/* Charge tick of the ov230 enemy: the +0x4c timer accumulates the frame-time and, past 0x550,
 * reaction 0x147 mode 0x10 fires once (+0x61) at the +0xc position. Once the +4 item's +0xad
...
extern void func_ov230_020d4b80(int *node);
...
void func_ov230_020d4a64(int *node)
{
    ...
    if (*(unsigned char *)((char *)state + 0x61) == 0 && state[0x13] >= 0x550) {
        func_ov107_020c5af8(*state, 0x147, 0x10, (void *)state[3]);
        *(unsigned char *)((char *)state + 0x61) = 1;
    }
    if (*(unsigned char *)(state[1] + 0xad) == 0) {
        func_ov107_020c9264(*state, 0x14, 1);
        state[0x13] = state[0x16];
        func_0203c634(node, *(signed char *)((char *)node + 0x20), func_ov230_020d4b80);
        return;
    }

/* func_ov069_020b9734 -- lay out one panel entry and evaluate the group it forms.
 *
 ...
 * list and, for group 0 only, runs the value tally (func_ov069_020b8fbc or 02057138).  A cell
 * that matches no existing group takes the first group whose id is still negative.
 *
 * The tail picks the evaluator by the anchor's type -- 4 goes to func_ov069_020b92c8, 6 to
 * func_ov069_020b8d0c, everything else to func_ov069_020b9100, with bit 0 of node->flags set
...
extern void func_ov069_020b8fbc(void *self, Ov000TallyEntry *entry, Ov000GridCell *cell);
extern void func_ov069_020b901c(void *self, Ov000TallyEntry *entry, Ov000GridCell *cell,
                                int flag);
extern void func_ov069_020b8d0c(void *self, Ov000TallyEntry *entries);
extern int  func_ov069_020b9100(void *self, Ov000TallyEntry *entries, int panelIndex);
extern int  func_ov069_020b92c8(void *self, Ov000TallyEntry *entries, int panelIndex,
                                int *flags);

void func_ov069_020b9734(Ov000PanelView *self, Ov000GridCell **grid, Ov000ViewNode *node,
                         int panel) {
    Ov000GridCell *cell;
    int i;
    int x;
    ...
    if (cell->type == 1) {
        func_ov069_020b8fbc(self, &entries[0], cell);
    } else {
        func_ov069_020b901c(self, &entries[0], cell, 0);
    }
                        ...
                        if (entries[0].type == 4) {
                            ...
                            for (k = 0; k < 10; k++) {
                                ...
                                if (match) {
                                    ...
                                    if (k == 0) {
                                        if (entries[0].type == 1) {
                                            func_ov069_020b8fbc(self, &entries[k], other);
                                        } else {
                                            func_ov069_020b901c(self, &entries[k], other, 1);
                                        }
                                    }
                            ...
                            if (k == 10) {
                                ...
                                entries[slot].count = 1;
                                if (other->type == 1) {
                                    func_ov069_020b8fbc(self, &entries[slot], other);
                                } else {
                                    func_ov069_020b901c(self, &entries[slot], other, 0);
                                }
    ...
    if (entries[0].type != 4) {
        if (entries[0].type == 6) {
            func_ov069_020b8d0c(self, entries);
            node->resultA = -1;
            node->resultB = -1;
            return;
        }
        ...
        node->resultA = -1;
        node->resultB = func_ov069_020b9100(self, entries, node->panelIndex);
        if (node->resultB < 0) {
            return;
        }
        node->flags |= 1;
    ...
    node->resultB = -1;
    node->resultA = func_ov069_020b92c8(self, entries, node->panelIndex, &node->flags);
}

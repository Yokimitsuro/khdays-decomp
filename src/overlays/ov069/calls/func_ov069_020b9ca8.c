/* func_ov069_020b9ca8 -- fold one panel into the running tally.
 *
 ...
 * func_ov069_020b89fc, type 1 pushes each of the four non-empty value slots through
 * func_ov069_020b8a90, and types 3, 5 and 6 push them through func_ov069_020b8b98 and then
...
extern void func_ov069_020b89fc(Ov000PanelView *self, int payload, int enabled, int reserved);
extern void func_ov069_020b8a90(Ov000PanelView *self, int value, int reserved0, int enabled,
                                int reserved1);
extern void func_ov069_020b8b98(Ov000PanelView *self, int value, unsigned int tallyA,
                                unsigned int tallyB, int reserved);

void func_ov069_020b9ca8(Ov000PanelView *self, Ov000GridCell **grid, u64 covered, u64 *out) {
    char *slotRow;
    int row;
    int col;
    int base;
        ...
        for (col = 0; col < 5; col++) {
            ...
            if (cell != 0 && (covered & (1ULL << (col + base))) == 0) {
                *out |= 1ULL << (col + base);
                switch (cell->type) {
                case 0:
                    func_ov069_020b89fc(self, cell->param, 1, 0);
                    break;
                case 1:
                    for (i = 0; i < 4; i++) {
                        slotRow = (char *)cell + i * sizeof(Ov000TallySlot);
                        if (*(int volatile *)(slotRow + 0x34) != 0) {
                            func_ov069_020b8a90(self, *(int *)(slotRow + 0x34), 0, 1, 0);
                        }
                    }
                    break;
                case 3:
                ...
                case 6:
                    for (i = 0; i < 4; i++) {
                        slotRow = (char *)cell + i * sizeof(Ov000TallySlot);
                        if (*(int volatile *)(slotRow + 0x34) != 0) {
                            func_ov069_020b8b98(self, *(int *)(slotRow + 0x34),
                                                *(u16 *)(slotRow + 0x38),
                                                *(u16 *)(slotRow + 0x3a), 0);
                        }
                    }

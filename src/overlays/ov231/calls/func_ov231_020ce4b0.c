 ...
 * (func_ov231_020cc9e0) and a sideways drift is chosen from the +0x20 distance (0x180 out past
 ...
 * (func_ov231_020ccc48); on a hit reaction 0/0x50 fires at the owner's +0x74 point. Past 0x1100 a
 * pending +0x54 flag spawns effect 3 at the origin once. The +0x30 velocity is the +0x388 part's
 * +0x2c vector plus the drift, both turned by the heading (func_ov231_020cca74). When the +0x10
...
extern int func_ov231_020ccc48(int *self, Sphere *sphere, void *query, void *pt, int flags);
...
void func_ov231_020ce4b0(int *node)
{
    ...
    if (state[0xa] >= 0x1100 && state[0xa] < 0x1650) {
        ...
        if (func_ov231_020ccc48(node, &sphere, 0, &dir, 1) != 0) {
    ...
    func_ov231_020cca74(&lift, node, &lift);

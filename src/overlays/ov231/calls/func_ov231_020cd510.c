/* func_ov231_020cd510 -- chase tick of the ov231 enemy (x5 with ov232/ov263/ov265/ov280). The +0x18
 ...
 * func_ov231_020cc9e0) sub-state 2 is requested and the tick ends. The +0x24 delay runs down in
 * sub-states 2/4. In those sub-states a 7.0 floor probe (func_ov231_020ccb08) is cast; with a
...
extern int func_ov231_020ccb08(int *node, Vec3 *dir, int direct);
...
void func_ov231_020cd510(int *node)
{
    ...
    if (*(signed char *)(*state + 0x100 + 0xc6) == 2 || *(signed char *)(*state + 0x100 + 0xc6) == 4) {
        probe = data_ov231_020cfa34;
        probe.y = func_ov231_020ccb08(node, &probe, 1);
        if (state[5] != 0) {

/* Ov231_ProbeGround -- cast from the owner's position along `dir` and return the Y of the scaled
...
extern void  func_ov231_020cca74(VecFx32 *pIn, int *node, VecFx32 *pOut);
extern void  VEC_Add(VecFx32 *a, VecFx32 *b, VecFx32 *out);

int func_ov231_020ccb08(int *node, VecFx32 *dir, int bRay)
{
    void *hit;
    ...
    if (*(void **)(world + 0x7c) != 0) {
        if (bRay) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                func_ov231_020cca74(&off, node, &off);
                VEC_Add(&pos, &off, &pos);

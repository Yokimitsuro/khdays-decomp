/*
 * Hit handler of the ov269 enemy (and its byte-identical twins), the func_ov181_020cc44c shape. A hit is
 ...
 * +0x24; a hit that charged flips the state's facing bit (+0x52 bit 0) and fires reaction 0x160
...
int func_ov269_020d2058(char *actor, int nParam, struct ActorHitEvent *hit)
{
    struct Ov269ActionState *state = *(struct Ov269ActionState **)(actor + 0x214);
    int delta;
    int rem;
    ...
    *(short *)(actor + 0x21a) = (short)rem;
    if (*(short *)(actor + 0x21a) == 0) {
        *(u8 *)(state->pOwner + 0x1c7) = 3;
        ...
        if (*(signed char *)(state->pOwner + 0x1c6) == 7 && (hit->uFlagsLo & 1) != 0 && (hit->uFlagsLo & 0x10) != 0) {
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                ...
                func_ov107_020c5af8(actor, 0x160, (state->bFacing & 1) ? 2 : 3, state->pPos);
            } else {
                state->bFacing = state->bFacing + 1;
                func_ov107_020c5af8(actor, 0x160, (state->bFacing & 1) ? 0 : 1, state->pPos);
            }
        }
    }
    return 1;

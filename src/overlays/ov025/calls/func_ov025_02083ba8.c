...
extern Ov009MenuContext *data_ov025_020b5744[];

#define OV009_CONTEXT (data_ov025_020b5744[1])

extern u16 *func_ov025_02083b2c(int destinationCode, int *slotIndex);
extern int  func_ov025_02083b70(int position, int stride,
                                int width, int height);

void func_ov025_02083ba8(Ov009TileBlitRequest *request, int copySource)
{
    int slotIndex;
    ...
    destination =
        func_ov025_02083b2c(request->destinationCode, &slotIndex);
    ...
    if (copySource != 0) {
        ...
        for (copyRow = 0; copyRow < (int)clip.copyHeight; copyRow++) {
            for (column = 0; column < (int)copyWidth; column++) {
                int destinationIndex = func_ov025_02083b70(
                    destinationX + column, destinationY,
                    OV009_CONTEXT->tileGridWidth, 32);
                int sourceIndex = func_ov025_02083b70(

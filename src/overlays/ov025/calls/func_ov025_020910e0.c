/* func_ov025_020910e0 -- Ov025_ComputeGridChanges: fill the change set of a
 ...
 * (session slot) picks the weapon record of each summary (ov002 020522c8 with the
 * summary's weapon id, +0x74); a kind of 0 or 0x13 falls back to 0 (unlike
 * the ov008 twin, no per-summary +0x2c fallback).  With +0x30 set the record's alternate
 ...
 * (020877e0) and node count (02087844) deltas to +0x8 / +0xc, the two header
 ...
 * failing that, every category but the player's own (020b3f28 by member
...
typedef struct Ov025SummaryEntryA {
    int nId;                  /* 0x00 */
    u8  pad_04[8];
    int bNew;                 /* 0x0c */
} Ov025SummaryEntryA;

typedef struct Ov025SummaryEntryB {
    int nId;                  /* 0x00 */
    u8  pad_04[12];
    int bNew;                 /* 0x10 */
} Ov025SummaryEntryB;
...
typedef struct Ov025GridChanges {
    int nHeaderA;             /* 0x00 */
    int nHeaderB;             /* 0x04 */
    int nEntries;             /* 0x08 */
    int nNodes;               /* 0x0c */
...
typedef struct Ov025KindCategoryTable {
    int aOwnCategory[22];     /* category owned by each member kind */
} Ov025KindCategoryTable;
...
extern int   func_ov025_02087844(Ov025GridSummary *pSummary);     /* Ov025_CountListNodes */
...
int func_ov025_020910e0(Ov025GridChanges *pOut, Ov025GridSummary *pOld, Ov025GridSummary *pNew)
{
    Ov025KindCategoryTable table;
    ...
    int nKindNew;
    int nDelta;
    Ov025SummaryEntryA *pEntryA;
    Ov025SummaryEntryB *pEntryB;
    int nOld;
    int nNew;
    int nSkip;
    int i;
    ...
    pOut->nEntries = func_ov025_020877e0(pNew) - func_ov025_020877e0(pOld);
    pOut->nNodes = func_ov025_02087844(pNew) - func_ov025_02087844(pOld);

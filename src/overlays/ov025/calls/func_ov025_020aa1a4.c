/* func_ov025_020aa1a4 -- Ov025_MissionMenu_DrawMissionText: draw a mission's text (+0x40 of its
...
extern int   func_ov025_020aa168(const u16 *pText);                 /* Ov025_MeasureTextHeight */

void func_ov025_020aa1a4(Ov008MissionMenu *pMenu, Ov008MissionNameNode *pNode, Ov008MissionNameNode *pAltNode)
{
    int nHeight;
    const u16 *pText;
    ...
    if (func_ov025_02084e38() == 0) {
        ...
        nHeight += func_ov025_020aa168(pText);
    } else if (*pText != 0) {
        func_020302ec(pMenu->surface, 3, 3, 0xf5, 0xfb, pText);
        nHeight += func_ov025_020aa168(pText);
    } else if (pAltNode != 0) {
        func_020302ec(pMenu->surface, 3, 3, 0xf5, 0xfb, pAltNode->pText);
        nHeight += func_ov025_020aa168(pAltNode->pText);
    }

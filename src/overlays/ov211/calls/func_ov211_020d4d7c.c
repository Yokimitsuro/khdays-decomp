/*
 * func_ov211_020d4d7c -- x3. AI-state tick: strafe the same target (negative sweep), fire on a timer.
 ...
 * state[5..7] = data_02042264 x dir scaled -0x100 (opposite sweep). Advance timer state[0xb] +=
 * owner_delta; while it stays under state[0xc], return. Once past: fire attack 0x11 (020c9264) and hand
 * off to the 020d3050 state.
 */
...
extern void func_ov211_020d4e70(void);

void func_ov211_020d4d7c(int *self) {
    ...
    func_0203c634((int)self, *(signed char *)((int)self + 0x20), (int)&func_ov211_020d4e70);
}

/* Burst tick of the ov210 enemy (x3 with ov211/ov282). The +0x2c timer accumulates the owner's
 * rate; between 0.5 and 0xaaa a sphere at the +4 point, its radius growing to 0x3555 with the
 ...
 * (data_ov211_020d6554, low nibble of byte 4 = 6) carries its +0x74 point to the owner's +0x24
...
extern const Cmd14 data_ov211_020d6554;

void func_ov211_020d415c(int *node)
{
    ...
    if (state[0xb] >= 0x800 && state[0xb] <= 0xaaa) {
        ...
        sphere.nRadius = FX_MUL(t, 0x3555);
        n = func_ov107_020c8eb8(*state, &sphere, hits);
        i = 0;
        if (n > 0) {
            tmpl = data_ov211_020d6554;
            do {

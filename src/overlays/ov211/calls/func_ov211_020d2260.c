...
int func_ov211_020d2260(char *actor, int nParam, struct ActorHitEvent *hit)
{
    ...
    if (hit->nDamage > 0) {
        if ((hit->uFlagsLo & 8) == 0 || (hit->uFlagsLo & 0x80) == 0 || hit->uFlagsHi != 0x80) {
            if ((hit->uFlagsLo & 0x22) != 0) {
                state->bToggle++;
                func_ov107_020c5af8(actor, 0x117, (state->bToggle & 1) ? 2 : 3, state->pPos);
            } else {
                state->bToggle++;
                func_ov107_020c5af8(actor, 0x117, (state->bToggle & 1) == 0 ? 1 : 0, state->pPos);
            }
        }
    }
    return 1;

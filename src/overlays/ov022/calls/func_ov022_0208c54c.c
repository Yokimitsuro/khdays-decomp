/* Ov022_StepSlotPartsC -- drive every part of a slot forward by a frame.
 *
 * The third of the multi-part drivers. This one gives states 1 and 2 routines
 * of their own and sends only 3 and 4 through the shared step table, where a
 * step that reports it finished takes one off the slot's sequence count. A part
...
extern void func_ov022_0208c744(struct ReactionCtx *pCtx, struct SlotPart *pPart,
                                int nDelta);
extern void func_ov022_0208c644(struct ReactionCtx *pCtx, struct SlotPart *pPart,
                                int nDelta);

int func_ov022_0208c54c(struct ReactionCtx *pCtx, int nDelta)
{
    ...
    for (i = 0; i < pSlot->nParts; i++) {
        ...
        if (pPart->nState != 0) {
            ...
            switch (pPart->nState) {
            case 2:
                func_ov022_0208c744(pCtx, pPart, nDelta);
                break;
            case 1:
                func_ov022_0208c644(pCtx, pPart, nDelta);
                break;
            case 3:
            case 4:

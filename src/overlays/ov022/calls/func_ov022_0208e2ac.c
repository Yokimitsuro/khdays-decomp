...
/* Ov022_CreateSlotKind06 -- create a slot of pool kind 6 from a template.
 *
 * Kind 6 owns five parts. The slot is built and filled like the other kinds, its
 * sequence registered on its own animation block, and then the parts are
 * allocated in one run: each takes the same sequence, the template's entry value
 * in all three of its slots, the slot's tail as its owner, an idle state and its
 * own place in the run as its index. What is left is the arming set the part
 * driver reads.
 */
...
void func_ov022_0208e2ac(struct ReactionCtx *pCtx, void *pSeq, int nSubKind,
                         const struct SlotTemplate *pTpl)
{
    ...
    pSlot->nMarkNext = 0;
    pSlot->nField180 = pTpl->nWord2;
    pSlot->nArmMax = (u8)(pTpl->nWord3 >> 12);
    pSlot->nArmDelay = pTpl->nWord4;
}

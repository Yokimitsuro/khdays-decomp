...
/* Ov022_CreateSlotKind04 -- create a slot of pool kind 4 from a template.
 *
 * The same shape and the same tail as pool kinds 0 and 3, with this kind's own
 * constants: it starts in state 4, scales its spawn value the way kind 3 does,
...
/* the part of the slot from 0x118 on, shared with pool kinds 0 and 3 */
struct SlotTail {
    u8 nState;                   /* 0x00 */
    u8 pad01;
    u16 nField02;                /* 0x02 */
...
void func_ov022_0208cf18(struct ReactionCtx *pCtx, void *pSeq, int nSubKind,

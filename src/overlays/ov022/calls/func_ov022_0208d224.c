/* Ov022_StepSlotPartsB -- drive every part of a slot forward by a frame.
 *
 * The twin of the other multi-part driver, instruction for instruction: it
 * walks every part, states 1, 3 and 4 go through the same step table, and a
 * step that reports it finished takes one off the slot's sequence count. Only
 * state 2 differs, running this kind's own routine instead. A part in state 0
 * is free and is skipped.
 *
...
extern void func_ov022_0208d310(struct ReactionCtx *pCtx, struct SlotPart *pPart,
                                int nDelta);

int func_ov022_0208d224(struct ReactionCtx *pCtx, int nDelta)
{
    ...
    for (i = 0; i < pSlot->nParts; i++) {
        ...
        if (pPart->nState != 0) {
            ...
            case 4:
                ...
                break;
            case 2:
                func_ov022_0208d310(pCtx, pPart, nDelta);
                break;
            }
        }
    }

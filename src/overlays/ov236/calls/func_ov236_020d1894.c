 ...
 * data_ov236_020d63e8 (rider 2 / 3) goes to the actor's +0x24 hook, the 14-byte message
 * data_ov236_020d6412 (3 when the other rider is down too, else 2) carries the attacker's point to
 * the +0x394 object's hook, reaction 0x127 mode 0xe fires there and the part's bit 1 is raised. The
...
extern const Note4 data_ov236_020d63e8;
extern const Cmd14 data_ov236_020d6412;

int func_ov236_020d1894(char *self, int src, unsigned int *hit)
{
    long i;
        ...
        if ((int)hit[10] > 0) {
            if (!((((struct HitWord *)hit)->lo & 8) != 0 && (((struct HitWord *)hit)->lo & 0x80) != 0 && ((struct HitWord *)hit)->hi == 0x80)) {
                if ((((struct HitWord *)hit)->lo & 0x22) != 0) {
                    ...
                    func_ov107_020c5af8(self, 0x127, (((struct Parity *)((u8 *)state + 0x75))->b0 & 1) ? 2 : 3, (void *)(*(int *)hit[6] + 4));
                } else {
                    ((struct Parity *)((u8 *)state + 0x75))->b0++;
                    func_ov107_020c5af8(self, 0x127, (((struct Parity *)((u8 *)state + 0x75))->b0 & 1) ? 0 : 1, (void *)(*(int *)hit[6] + 4));
                }
            }
        }
    } else {

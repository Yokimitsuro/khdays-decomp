/* Stomp tick of the ov206 enemy. While the +0x24 timer (fed by the owner's rate) is between
 ...
 * template (data_ov206_020d05f6 for the first foot, data_ov206_020d05da for the second) carries
 * the entity's +0x74 position added to the transform and scaled 0.5 to the owner's +0x24
 * hook, where reaction 0x116 mode 0xc fires. Once the +0xc idle byte clears sub-state 0xa is
...
extern const PosMsg data_ov206_020d05f6;
extern const PosMsg data_ov206_020d05da;

static inline void SweepFoot(struct Ov206State *state, int nFoot, PosMsg tmpl, int *hits, Sphere *psphere)
{
    Vec3 at;
    PosMsg msg;
    Vec3 push;
    ...
    *psphere = *(Sphere *)(*(int *)(state->pOwner + nFoot) + 0x68);
    ...
    if (n > 0) {
        do {
            ...
            if (func_ov107_020ca918(hits[i], state->pOwner, state->pOwner, 3, &push, 0) != 0) {
                msg = tmpl;
                VEC_Add((void *)(hits[i] + 0x74), (char *)*(int *)(state->pOwner + nFoot) + 4, &at);
                func_01ffa724(0x800, &at, &at);
                SendPos(state, &msg, &at);
                func_ov107_020c5af8(state->pOwner, 0x116, 0xc, &at);
            }
        } while (++i < n);
    }
}

void func_ov206_020cf0e4(int *node)
{
    ...
    if (state->nTimer > 0x2aa && state->nTimer < 0xc44) {
        SweepFoot(state, 0x3bc, data_ov206_020d05f6, hits, &sphere);
        SweepFoot(state, 0x3c0, data_ov206_020d05da, hits, &sphere);
    }
    if (*state->pBusy != 0) {
        return;
    }

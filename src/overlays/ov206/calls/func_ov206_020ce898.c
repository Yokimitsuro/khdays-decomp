 ...
 * acceptance, the overlay's 14-byte template (data_ov206_020d0604 / data_ov206_020d05e8)
...
extern const PosMsg data_ov206_020d0604;
extern const PosMsg data_ov206_020d05e8;
...
static inline void SweepFoot(struct Ov206State *state, int nFoot, PosMsg tmpl, int *hits, Sphere *psphere, int nBit)
{
    ...
    *psphere = *(Sphere *)(*(int *)(state->pOwner + nFoot) + 0x68);
    ...
    if ((state->nFlags52 & nBit) == 0) {
        func_ov107_020c5af8(state->pOwner, 0x116, 9, psphere);
        state->nFlags52 |= nBit;
    }
    ...
    if (n > 0) {
        do {
            ...
            if (func_ov107_020ca918(hits[i], state->pOwner, state->pOwner, 1, &push, 0) != 0) {
                ...
                func_ov107_020c5af8(state->pOwner, 0x116, 0xa, &at);
            }
        } while (++i < n);
    }
}

void func_ov206_020ce898(int *node)
{
    ...
    if (state->nTimer >= 0x1b28 && state->nTimer <= 0x1ee6) {
        SweepFoot(state, 0x3bc, data_ov206_020d0604, hits, &sphere, 1);
    }
    if (state->nTimer >= 0xded && state->nTimer <= 0x1199) {
        SweepFoot(state, 0x3c0, data_ov206_020d05e8, hits, &sphere, 2);
    }
    if (*state->pBusy != 0) {
        return;
    }

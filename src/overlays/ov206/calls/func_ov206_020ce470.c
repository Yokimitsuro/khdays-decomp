/* Charge tick of the ov206 enemy. The +0x14 velocity is the sine/cosine of the +0x40 heading
 ...
 * acceptance the overlay's 14-byte template (data_ov206_020d0612) carries the entity's +0x74
 * position added to the +0x3b0 body's transform and scaled 0x800 to the owner's
 * +0x24 hook and reaction 0x116 mode 6 fires at the +8 point; every accepted or rejected
 * candidate bumps the +0x52 count. Four hits end the charge (animation 5 mode 0, hand-over to
 * func_ov206_020ce820); otherwise the +0x20 step counter fires reaction 0x116 mode 4 at 5 and
...
extern const PosMsg data_ov206_020d0612;
extern void func_ov206_020ce820(int *node);

void func_ov206_020ce470(int *node)
{
        ...
        do {
            ...
            if (VEC_DotProduct(&d, &dir) > 0) {
                if (func_ov107_020ca918(hits[i], state->pOwner, state->pOwner, 0, &dir, 0) != 0) {
                    ...
                    func_ov107_020c5af8(state->pOwner, 0x116, 6, state->pPos);
                }
                state->nHits52++;
            }
        } while (++i < n);
    ...
    if (state->nHits52 >= 4) {
        ...
        func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov206_020ce820);
        return;
    }
    state->nStep++;
    if (state->nStep == 5) {
        func_ov107_020c5af8(state->pOwner, 0x116, 4, state->pPos);
    } else if (state->nStep == 0xa) {
        func_ov107_020c5af8(state->pOwner, 0x116, 5, state->pPos);
        state->nStep = 0;
    }

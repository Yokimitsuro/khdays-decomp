/* Recoil wind-up tick of the ov233 enemy. The +0x4c timer accumulates the owner's
 * rate; past 0x770 reaction 0x164 mode 0xb fires at the +0xc position (once, flag +0x61). When
 ...
 * (0, 1.5, 0) and the tick hands over to func_ov233_020cf6f0. */
...
extern void func_ov233_020cf6f0(int *node);

void func_ov233_020cf5f8(int *node)
{
    ...
    if (*((unsigned char *)state + 0x61) == 0 && state[0x13] >= 0x770) {
        func_ov107_020c5af8(*state, 0x164, 0xb, (void *)state[3]);
    ...
    *(unsigned short *)(*state + 0x60) = (unsigned short)((v & ~0xff00) | (((((unsigned int)v << 0x10) >> 0x18 | 0x40) << 0x18) >> 0x10));
    ...
    state[8] = 0x1800;
    state[9] = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov233_020cf6f0);
}

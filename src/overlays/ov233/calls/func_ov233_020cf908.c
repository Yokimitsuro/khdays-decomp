/* func_ov233_020cf908 -- the slam tick: the impulse at ctx+0x1c is copied out to
 ...
 * with kind 2 (func_ov233_020cd028). Once the owner lands (+0x17a bit 0) the owner's +0x180 point
 * is kept at ctx+0x34 with its height lowered to the owner's +0x80 floor less 0x200, effect 6
 * spawns there, animation 0x19 plays, reaction 0x164 mode 0xc fires at it, the timer at +0x4c
 * restarts and the tick hands over to func_ov233_020cfa8c. */
...
extern void func_ov233_020cd028(int *ctx, int kind, Sphere *sphere, void *box);
...
extern void func_ov233_020cfa8c(void);

void func_ov233_020cf908(int self) {
    int *ctx;
    Sphere sphere;
    int step;
    ...
    *(VecFx32 *)((char *)ctx + 0x10) = *(VecFx32 *)((char *)ctx + 0x1c);
    ...
    func_ov233_020cd028(ctx, 2, &sphere, 0);
    ...
    *(VecFx32 *)((char *)ctx + 0x34) = *(VecFx32 *)(ctx[0] + 0x180);
    ...
    func_ov107_020c5af8(ctx[0], 0x164, 0xc, (char *)ctx + 0x34);
    ctx[0x13] = 0;
    func_0203c634(self, *(signed char *)(self + 0x20), func_ov233_020cfa8c);
}

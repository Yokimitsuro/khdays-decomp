/* Dash tick of the ov233 enemy. The +0x24 speed grows by 0.1875 per 0x88 step of
 ...
 * +0x28 heading, is added to the step. A blocked step (func_ov233_020d04dc) spawns the body's
 * effect 8 at the owner's +0x74 point and fires reaction 0x164 mode 0xe at the +4 point; a wall
 ...
 * restarts and the tick hands over to func_ov233_020d0d1c. */
...
extern int func_ov233_020d04dc(int *state);
...
extern void func_ov233_020d0d1c(int *node);

void func_ov233_020d0994(int *node)
{
    ...
    if (func_ov233_020d04dc(state) != 0) {
        item = *state;
        func_ov107_020c0b90(*(int *)(item + 0x38c), 8, *(Vec3 *)(item + 0x74), 0);
        func_ov107_020c5af8(state[0], 0x164, 0xe, (void *)state[1]);
    } else if (((struct Bits17a *)(*state + 0x17a))->b1 != 0) {
        func_ov107_020c5af8(*state, 0x164, 0xe, (void *)state[1]);
    ...
    state[8] = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov233_020d0d1c);
}

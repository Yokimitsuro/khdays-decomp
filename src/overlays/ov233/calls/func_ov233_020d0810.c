/* Dash entry of the ov233 enemy: the +0x28 heading follows the owner's +0x398
 * direction, which is also kept at +0x14; bit 0 of the owner's +0x60 high byte is raised and bits
 * 2, 3 and 7 cleared, bit 0 of its +0x388 part's flag byte is raised, reaction 0x164 mode 0xd fires
 ...
 * the tick hands over to func_ov233_020d0950. */
...
extern void func_ov233_020d0950(int *node);

void func_ov233_020d0810(int *node)
{
    ...
    *(unsigned short *)(*state + 0x60) = (unsigned short)((v & ~0xff00) | (((((unsigned int)v << 0x10) >> 0x18 | 1) << 0x18) >> 0x10));
    ...
    func_ov107_020c5af8(state[0], 0x164, 0xd, (void *)state[1]);
    ...
    *((unsigned char *)state + 0x34) = 0;
    func_0203c634(node, *(signed char *)((char *)node + 0x20), (void *)func_ov233_020d0950);
}

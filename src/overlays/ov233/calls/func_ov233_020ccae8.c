/* Hit handler of the ov233 enemy: ignored while the +0x21a stamina is spent.
 ...
 * the damage is resolved, and in sub-state 7 doubled and, for an 8-flagged hit of kind 0x4,
 * multiplied by 10/3. A 0x4000-flagged hit sets result 1 unless the actor
 * is guarded (+0x1c4 bit 1). The source and the +0x68 flag-4 bit are recorded; a frontal, unguarded
 * hit in sub-states 2/4 (with flag 0x20) or 5 fires reaction 0x164 mode 5, sets result 1 and, for
 ...
 * 0x8000 hit that is not the 8|0x80/0x80 special fires reaction 0x164 with the mode taken from the
...
extern const struct ModeTable data_ov233_020d0e08;

#define ANG2IDX(a) ((unsigned short)(((long long)(a) * 0x28be60db9391LL + 0x80000000000LL) >> 44) >> 4)

int func_ov233_020ccae8(char *actor, int other, struct ActorHitEvent *hit)
{
